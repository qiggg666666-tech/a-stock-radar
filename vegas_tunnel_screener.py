...
⚠️ 突破易假突破, 非买入保证; trailing止损是持仓后动态管理, 扫描只给初始位, 需手动管理+止损。
"""
import os
import re
import sys
...
def run_scan():
    ...
    print(f"开始Vegas通道突破扫描 {len(tasks)} 只（{PARAMS['NUM_PROCESSES']}进程, 双源, 突破窗口={PARAMS['BREAK_FRESH_BARS']}根, 含警示={PARAMS['INCLUDE_WARNINGS']}）...")
        ...
        for res in pool.imap_unordered(_process_one, tasks):
            if res:
                if "__fail__" in res:
                    ...
                    results.append(res)
                    pbar.write(f"  √ {res['代码']} {res['名称']} {res['信号']} 大趋势{res['大趋势']} 价={res['最新价']} 放量{res['放量倍数']}")
...
def build_push(df, cluster, hot):
    ...
    L = [f"**📈 Vegas通道趋势突破** | 做多{len(longs)}只 警示{len(warns)}只 🎯风口{len(reso)} (全发)",
         "*(EMA12+收盘突破中隧道144/169+大隧道576/676多头+放量; 建议位=入场那刻初始位, trailing需持仓后手动; 做空转回避; 信号稀少属正常)*", ""]
    if hot:
...
def main():
    print("=" * 70)
    print(f"📈 Vegas通道趋势突破 (日线扫描+初始建议位) | {datetime.now():%Y-%m-%d %H:%M} | 回看{PARAMS['lookback_days']}天")
    print(f"全扫={'是' if not SCAN_LIMIT else f'限{SCAN_LIMIT}'}; 双源; 预筛={'开' if PARAMS['SNAPSHOT_PRE'] else '关'}; 突破窗口={PARAMS['BREAK_FRESH_BARS']}根; 含警示={PARAMS['INCLUDE_WARNINGS']}; 不拦交易日; 推送全列+分页")
    print("=" * 70)
    df = run_scan()
    if df is None or df.empty:
        print("\n本次扫描未发现满足 Vegas通道突破 的信号。")
        print("通道突破+多重趋势过滤本就极严, 0命中属正常; 可调: BREAK_FRESH_BARS 调大(放宽突破新鲜度)")
        sys.exit(0)
    ...
    try:
        ...
        with open(os.path.join(OUTPUT_DIR, f"vegas_{tag}.json"), 'w', encoding='utf-8') as f:
            json.dump({"date": tag, "params": {"BREAK_FRESH_BARS": PARAMS['BREAK_FRESH_BARS'],
    ...
    if SERVERCHAN_KEY:
        try:
            ...
            send_serverchan(f"📈 Vegas通道突破 做多{n_long}只 🎯风口{n_reso}", build_push(df, cluster, hot))
        except Exception as e:
