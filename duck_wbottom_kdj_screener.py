...
四共振(AND): ① 周线老鸭头(周线蓄势向上) ② 日线W底(双底结构+价格在底区) ③ KDJ超卖(J≤J_MAX, 当下跌下来)
  ④ MACD零轴上(DIF>0, 中期趋势在)。= 趋势框架内"强势回踩到双底超卖"= 高质量"跌下来的蓄势/筑底票"。
  追高票被"J超卖+价格在底区"双重排除(治 first_red_wbottom 追高毛病)。

【KDJ/MACD 原样搬自 kdj_macd_screener.py 真码】calc_kdj(9,3,3,J=3K-2D)/calc_macd(12,26,9)/J≤J_MAX/
  MACD_MODE(zero=DIF>0; zero_cross=DIF>0且DIF>DEA) 一字对齐, 口径与 kdj_macd 脚本完全一致。
【周线老鸭头/日线W底 用本脚本自带 numpy 实现, 不依赖 scipy】故 requirements.txt 无需加任何东西。
  老鸭头=60周线向上+5/10上穿60+回调8~35%+短期多头+周线MACD转强(score≥DUCK_SCORE_MIN)。
  W底=近60日找两底(距离≥8, 容差≤WBOTTOM_TOL)+价格在底区(close≤颈线×NEAR_NECK 且 ≥两底×0.97);
  【填坑】W底不强求突破颈线(突破时J常已拐头, 与超卖时间冲突), 改"结构在+价格在底区", 与J超卖自洽。
【定位】老鸭头+DIF>0 均要求趋势在 -> 抓"强势回踩型跌下来", 非"跌崩抄底"; 要后者设 REQUIRE_DIF_POS=0。
【工程规格】双源baostock+东财+硬超时; 多进程+快照预筛; baostock行业本地join+聚类+东财风口🎯;
  推送全发分页+信号vs实时对齐列; 不拦交易日; 收尾防护sys.exit(0); append补丁。阈值全env可调。
【数据】老鸭头需~80周线, 故 LOOKBACK_DAYS=1800, MIN_DATA_LEN=450; 拉数据偏重, 建议 SCAN_LIMIT≤1500。
⚠️ 四共振极严, 0命中属正常(弱市可能0); 非买入保证, J≤0后仍可能钝化, 等J拐头/放量确认+止损。
⚠️ 本脚本为新建独立实现, 不修改现有 first_red_wbottom; 其老鸭头/W底阈值为本脚本自带实现, 与现有
  first_red_wbottom 可能不同; 若要"完全沿用现有 first_red_wbottom 的老鸭头/W底 + KDJ"合并进同一文件,
  需另提供 first_red_wbottom 当前源码再做合并版。
"""
import os
import re
import sys
...
# ==================== 参数 (env 可调) ====================
# ---- KDJ/MACD (口径对齐 kdj_macd_screener) ----
KDJ_N = int(os.environ.get('KDJ_N', '9'))
J_MAX = float(os.environ.get('J_MAX', '0.0'))                 # J≤此值算超卖
MACD_MODE = os.environ.get('MACD_MODE', 'zero')               # zero / zero_cross
REQUIRE_KDJ_OS = os.environ.get('REQUIRE_KDJ_OS', '1').strip() in ('1', 'true', 'True')   # 是否硬卡KDJ超卖
REQUIRE_DIF_POS = os.environ.get('REQUIRE_DIF_POS', '1').strip() in ('1', 'true', 'True')  # 是否硬卡MACD零轴上(0=允许跌深抄底)
# ---- 周线老鸭头 ----
DUCK_SCORE_MIN = int(os.environ.get('DUCK_SCORE_MIN', '4'))
# ---- 日线W底 ----
WBOTTOM_TOL = float(os.environ.get('WBOTTOM_TOL', '0.05'))    # 两底容差
NEAR_NECK = float(os.environ.get('NEAR_NECK', '1.03'))        # 价格上限=颈线×此(没飞太高)
...
# ------------------ 历史双源 (含 high/low 供 KDJ/W底; 长历史供周线老鸭头) ------------------
...
# ==================== KDJ / MACD (原样搬自 kdj_macd_screener 真码) ====================
...
# ==================== 周线老鸭头 (numpy/pandas 实现, 无 scipy) ====================
def _detect_duck(df_daily):
    """返回 (valid, score)。60周线向上+5/10上穿60+回调合适+短期多头+周线MACD转强。"""
    try:
...
# ==================== 日线W底 (numpy 找两底, 无 scipy; 结构在+价格在底区) ====================
def _detect_wbottom(df_daily):
    """返回 dict{bottoms_close, neck, low_min, in_zone}。不强求突破颈线(填坑: 与J超卖时间自洽)。"""
...
def check_one_stock(df):
    ...
    zone_score = max(0.0, 10.0 - abs(near_neck_pct))   # 越贴近颈线(底区上沿)越高
    ...
    return {"代码": None, "名称": None, "行业": "",
            "最新价": round(close, 2), "信号日期": sig_date,
...
def build_push(df, cluster, hot, spot_now=None):
    ...
    L = [f"**🦆📉 回踩筑底四共振** | 命中{len(df)}只 🎯风口{len(reso)} (全发)",
...
if __name__ == "__main__":
    print("=" * 70)
    dif_tag = ('零轴上+金叉' if MACD_MODE == 'zero_cross' else '站上0轴')
    print(f"🦆📉 回踩筑底四共振 (日线扫描) | {datetime.now():%Y-%m-%d %H:%M} | 回看{LOOKBACK_DAYS}天")
    ...
    try:
        ...
        disp = disp.drop(columns=['行业', 'resonance', 'resonance_sector', 'K', 'D', 'DEA', '底'], errors='ignore')
    ...
    if SERVERCHAN_KEY:
        try:
            ...
            send_serverchan(f"🦆 回踩筑底四共振 命中{len(df)}只 🎯风口{n_reso}", build_push(df, cluster, hot, spot_now))
        except Exception as e:
