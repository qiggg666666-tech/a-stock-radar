...
def split_first_red_notifications(header: list[str], records: list[dict[str, str]], footer: list[str], max_chars: int = 3400) -> list[str]:
    """将全部首红检测记录分段发送，避免Server酱长度截断导致候选被静默遗漏。"""
    lines = [first_red_line(item) for item in records]
    if not lines:
    ...
    if current:
        groups.append(current)
    total = len(groups)
    return ["\n".join(header + [f"- 首红明细分段：{index}/{total}"] + group + footer) for index, group in enumerate(groups, start=1)]
...
def main() -> int:
    ...
    if args.notify == "true":
        if upstream_failed:
            ...
            bodies = split_first_red_notifications(lines[:-1], first_red_records, ["", "> 全部首红检测结果已分段发送；完整CSV/JSON已随artifact保存。"])
            notifications = [notify(f"研究信号全市场：{state} | 首红{len(first_red_records)}条（{index}/{len(bodies)}）", body) for index, body in enumerate(bodies, start=1)]
            summary["notification"] = notifications[0] if len(notifications) == 1 else notifications
        else:
