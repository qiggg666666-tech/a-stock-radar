# -*- coding: utf-8 -*-
"""
ma520_bottom_screener.py —— 520日线低位 + MA5横盘 + 换手/振幅/量比 深度筑底选股
年线(MA520)历史低位 + 价格/MA5近300天低位 + MA5横盘 + 换手≥1.4% + 振幅≥4.5% + 量比≥1.4
= 深度底部蓄势; 筑底遇风口(⛏️)=价值挖掘提示。条件极严, 命中极少属正常。

【本版修复】
 1 修交易日拦截: 仅对定时触发(schedule)生效, 手动(workflow_dispatch)/本地不受限(周末可用周五数据复盘)。
 2 【性能·关键】加快照预筛砍量: 原全扫4996只×每只拉1600天(4.4年)历史, 3进程≈2.5h+, 超3h timeout被砍=白跑0结果。
   现按 价≥MIN_PRICE + 换手≥PRE_TURNOVER_MIN(1.0%, 比策略1.4宽松) 快照预筛, 砍掉策略本就不要的低价/无换手僵尸股,
   几乎不误杀(低位筑底票仍需拉历史精算, 无法纯快照判断), 4996→约2-3千, 能在timeout内跑完。预筛失败退化全扫。
 3 加收尾防护sys.exit(0): 存盘/推送异常不让job误红。
 4 策略内核(detect_full_strategy_with_vr/_safe_pct_position/各阈值)、双源(含换手率rename)、行业/风口/筑底聚类、推送 一字未动。
 5 【本版新增·价格时点+实时对齐列】原 _process_one 没存信号价/信号日期, 推送行也没现价; 现补 '最新价'+'信号日期'
   两字段(在 _process_one 取, 不碰策略内核), 推送每行末加 '🕒信号价@日期(距今N天) → 现价X@run(±Y%)'。
   实时快照限流/失败 -> 对齐列降级只显示信号时点, 绝不阻断推送/存盘。
   口径: 信号价=前复权日线收盘(用于判断), 现价=不复权实时(用于看盘); 期间除权则涨幅仅供参考。
⚠️ 深度筑底=左侧埋伏, 年线低位+横盘只表示"跌透在磨底", 不代表马上启动; 遇风口⛏️仅为催化提示, 非买入保证。
"""
import os
import sys
import time
...
SCAN_LIMIT = int(os.environ.get('SCAN_LIMIT', '0'))        # 0=全扫(预筛后); 仍擦边可设1500进一步限量
...
# 【新增】快照预筛开关与阈值(比策略硬条件宽松, 几乎不误杀)
SNAPSHOT_PRE = os.environ.get('SNAPSHOT_PRE', '1').strip() in ('1', 'true', 'True')
PRE_TURNOVER_MIN = float(os.environ.get('PRE_TURNOVER_MIN', '1.0'))
...
def _call_with_timeout(fn, *a, timeout=25, **kw):
    """通用硬超时包装(用于akshare快照等无内置超时的调用)"""
...
def snapshot_prefilter(tasks):
    """用全市场快照砍掉明显不符合的(价<MIN_PRICE / 换手<PRE_TURNOVER_MIN), 大幅减少逐只拉4年历史的量。
    预筛阈值比策略硬条件宽松(换手1.0 < 策略1.4), 几乎不误杀低位筑底票。失败/关闭则返回原列表(退化全扫)。"""
    if not SNAPSHOT_PRE:
    ...
    try:
        ...
        m = pd.Series(True, index=spot.index)
        if '最新价' in spot.columns:
            m &= (spot['最新价'] >= MIN_PRICE)
        if '换手率' in spot.columns:
            m &= (spot['换手率'] >= PRE_TURNOVER_MIN)
        keep = set(spot.loc[m, '代码'])
        filtered = [(c, n) for c, n in tasks if c[3:] in keep]
        print(f"  快照预筛: {len(tasks)} → {len(filtered)} 只 (价≥{MIN_PRICE} + 换手≥{PRE_TURNOVER_MIN}%)")
...
def _fetch_spot_now():
    ...
    收盘后跑时, 此处'现价'=当天收盘价, 对齐列正好显示'筑底信号后隔几天涨/跌多少'。"""
    try:
...
def _align_suffix(r, spot_now):
    ...
    无信号价->空; 无信号日->省略日期/距今; 无现价->省略'现价@run'段。r 可为 Series 或 dict。
    信号日期兼容 Timestamp 与字符串(统一 str[:10] 取 YYYY-MM-DD)。"""
    sig_price = r.get('最新价')
    sig_date = r.get('信号日期')
...
def _process_one(args):
    ...
    try:
        ...
        # 【本版新增】补信号价+信号日期(原 detect 没返回 close; 此处从 df 取, 不碰策略内核;
        # baostock 路径 close 可能为字符串, 故 pd.to_numeric 转一次)
        sig_price = float(pd.to_numeric(df['close'], errors='coerce').iloc[-1])
        sig_date = df['date'].iloc[-1]

        return {
            "代码": code, "名称": name, "行业": "",
            **info,
            "最新价": round(sig_price, 2),                 # 【本版新增】信号价(=最后一根K线收盘)
            "信号日期": sig_date,                          # 【本版新增】信号基于的最后一根K线日期
...
def run_ma520_bottom_scan(limit=SCAN_LIMIT):
    ...
    # 2.5) 【新增】快照预筛砍量 (价≥MIN_PRICE + 换手≥PRE_TURNOVER_MIN), 解决全扫跑不完
...
def build_push_content(df, cluster, hot, spot_now=None):
    P = PUSH_TOP
    lines = []
    lines.append("*(🕒每行末'信号价@日期→现价@run'=筑底信号日收盘 vs 本次run实时价, 含信号后涨跌幅; 信号价=前复权日线, 现价=不复权实时, 期间除权则涨幅仅供参考)*")
    lines.append("")
    if hot:
    ...
    if not meet.empty:
        ...
        for _, r in meet.head(P).iterrows():
            base = (f"- {r['名称']}（{r['代码']}）[⛏️{r['hot_sector']}] 横盘{r['sideways_days']}天 "
                    f"| 量比{r['volume_ratio']} | 换手{r['avg_turnover_%']}% | 振幅{r['avg_amplitude_%']}% "
                    f"| 价低{r['price_low_%']}% 年线低{r['ma520_low_%']}%")
            lines.append(base + _align_suffix(r, spot_now))   # 【本版新增】时点+实时对齐后缀
        lines.append("")
    lines.append(f"### 📋 全部筑底 Top{min(len(df), P)}")
    for _, r in df.head(P).iterrows():
        base = (f"- {r['名称']}（{r['代码']}）[{sec_tag(r.to_dict())}] 横盘{r['sideways_days']}天 "
                f"| 量比{r['volume_ratio']} | 换手{r['avg_turnover_%']}% | 振幅{r['avg_amplitude_%']}% "
                f"| 价低{r['price_low_%']}% 年线低{r['ma520_low_%']}%")
        lines.append(base + _align_suffix(r, spot_now))       # 【本版新增】时点+实时对齐后缀
    if len(df) > P:
...
if __name__ == "__main__":
    ...
    print(f"K线双源(baostock+东财, 均含换手率); 快照预筛={'开' if SNAPSHOT_PRE else '关'}; 条件极严, 命中可能极少属正常")
    print("【本版】推送加'信号价@日期→现价@run'对齐列(限流自动降级)")
    print("=" * 70)

    # 交易日判断仅对定时触发(schedule)生效; 手动(workflow_dispatch)/本地运行不受限, 周末可用周五数据复盘
    ...
    if df is not None and not df.empty:
        ...
        try:
            csv_path = f"{OUTPUT_DIR}/ma520_bottom_{tag}.csv"
            json_path = f"{OUTPUT_DIR}/ma520_bottom_{tag}.json"
