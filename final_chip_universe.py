...
def provider_call(label: str, timeout_seconds: float, function: Callable[[], Any]) -> Any:
    ...
    try:
        if not parent.poll(timeout_seconds):
            ...
            # 导致子进程变僵尸继续占用连接，后续 fork() 在容器里可能变慢甚至卡住，
            # 多次累积后总耗时会远超"单次超时×重试次数"的理论上限（这次job卡了30分钟
            # 被GitHub Actions强制杀掉，但按 90s×3次×2源 算最多该是9分钟左右）。
            # 换成 SIGKILL(kill) 保证一定能杀死，不给它有机会拖慢后续尝试。
            process.kill()
...
def main() -> int:
    ...
    # 新增：整个重试循环的总耗时硬上限。不管内部单次超时是否按预期生效，
    # 到点就必须退出转缓存兜底，避免像这次一样一路卡到被GitHub Actions
    # 作业级超时(30分钟)强制杀掉、什么产物都留不下。留够余量给写文件/上传步骤。
    ...
    for label, source in provider_sources():
        for attempt in range(1, max(args.retries, 1) + 1):
            ...
            if remaining <= 0:
                errors.append(f"budget_exhausted:before:{label}:{attempt}")
                break
            per_attempt_timeout = min(args.timeout_seconds, remaining)
            try:
                universe = provider_call(label, per_attempt_timeout, source)
