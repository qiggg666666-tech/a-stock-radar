...
def _baostock_universe() -> pd.DataFrame:
    ...
    try:
        result = bs.query_stock_basic(code_name="", code="", ipo_date="", out_date="", type="1", status="1")
...
def fetch_baostock_universe(timeout_seconds: float) -> pd.DataFrame:
    ...
    if source in {"auto", "akshare"}:
        try:
            ...
            if source == "akshare":
                raise
    try:
        return fetch_baostock_universe(timeout_seconds), "baostock", errors
    except Exception as exc:  # noqa: BLE001
        errors.append(str(exc)[:240])
