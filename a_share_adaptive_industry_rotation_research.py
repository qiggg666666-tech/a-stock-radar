...
class AkShareFetcher:
    ...
    def daily(self, code: str, start_date: str, signal_date: str) -> tuple[pd.DataFrame | None, str | None]:
        ...
        if missing:
            return None, f"daily_schema_missing:{','.join(missing)}"
        frame = raw.rename(
            columns={"日期": "date", "开盘": "open", "最高": "high", "最低": "low", "收盘": "close", "成交量": "volume", "成交额": "amount_yuan"}
        )[["date", "open", "high", "low", "close", "volume", "amount_yuan"]].copy()
        ...
        if frame.empty:
            return None, "empty_daily_history_before_signal_date"
...
class AdaptiveIndustryRotationResearch:
    ...
    def run(self) -> tuple[pd.DataFrame, dict[str, Any]]:
        ...
        if self.cfg.universe_file:
            ...
            shard = universe[self.cfg.shard_index :: self.cfg.shard_count]
            if self.cfg.max_total:
                shard = shard[: self.cfg.max_total]
        ...
        for position, (code, name, industry, selection_basis) in enumerate(queue, start=1):
            frame, daily_error = self.fetcher.daily(code, self.cfg.start_date, signal_date)
            if frame is None:
...
def write_outputs(output_dir: Path, candidates: pd.DataFrame, errors: list[dict[str, str]], audit: dict[str, Any]) -> None:
    ...
    if not candidates.empty:
        display = ["rank", "code", "name", "industry", "research_score", "close", "return_20d_pct", "average_turnover_20d_wan"]
        lines.extend(["", "## 前30名", candidates.head(30)[display].to_markdown(index=False, floatfmt=".4f")])
...
def parser() -> argparse.ArgumentParser:
    ...
    command.add_argument("--max-total", type=int, default=120)
