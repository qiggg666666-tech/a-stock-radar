...
from strategy_shard_notify_summary import (
    SPECS,
    artifact_health,
...
def build_snapshot(key: str, input_root: Path, run_date: str, top: int) -> dict[str, Any]:
    ...
    artifact_state, shards, missing_shards = artifact_health(paths, spec)
    run_statuses = read_run_statuses(input_root, key)
    failed_shards = sorted(shard for shard, payload in run_statuses.items() if payload.get("state") == "failed")
    if artifact_state == "missing":
        snapshot_status = "missing_artifact"
    elif failed_shards:
        snapshot_status = "upstream_failed"
