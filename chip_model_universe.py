...
def baostock_universe() -> list[dict[str, str]]:
    ...
    try:
        ...
        return normalize_baostock_rows(fields, rows)
    finally:
        bs.logout()


def normalize_baostock_rows(fields: list[str], rows: list[list[str]]) -> list[dict[str, str]]:
    """按BaoStock返回字段名取值，避免将ipoDate误当作type导致全量为空。"""
    index = {str(name): position for position, name in enumerate(fields)}
    required = {"code", "code_name", "type", "status"}
    if not required.issubset(index):
        raise RuntimeError(f"baostock_fields_missing:{','.join(sorted(required - set(index)))}")
    records: list[dict[str, str]] = []
    for row in rows:
        if len(row) <= max(index[name] for name in required):
            continue
        code = str(row[index["code"]])
        code_name = str(row[index["code_name"]]).strip()
        stock_type = str(row[index["type"]])
        status = str(row[index["status"]])
        if stock_type != "1" or status != "1" or not code.startswith(("sh.6", "sz.0", "sz.3")) or not code_name:
            continue
        records.append({"code": code.split(".", 1)[1], "name": code_name, "source": "baostock"})
    return records
