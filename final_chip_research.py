...
# 红色筹码多/绿色筹码少判定：直接复用已有的profit(获利占比)——红色=现价及以下的
# 筹码(获利盘)，绿色=现价上方的筹码(套牢盘)，两者互补(相加=100%)，所以只需一个
# 阈值，不用分别设两个。默认值参考用户给出的两个正例(获利占比57.86%和70.51%)，
# 取偏保守的下限让两例都能命中，未在更大样本上验证过，先作为起始值。
RED_HEAVY_PROFIT_MIN = 55.0
...
#
# 方向说明：daily_long_lower(长下影，现价探底回升，买入型态) 和 daily_long_upper
# (长上影，冲高回落，卖出/压力型态) 判定用的是同一套阈值，只是上下对称，仍然都会
# 计算并保留在输出里(有参考价值)。但"长影线"这个推送信号/signal字段只认
# daily_long_lower——之前用has_long_shadow(两个方向都算)会让长上影这种偏卖出的
# 信号混进一个本意是找买入机会的专区，容易误导。
...
# 布林带：与同花顺默认口径一致 (20, 2)，供红筹码/宽幅等形态叠加参考。
BOLL_PERIOD = 20
BOLL_STD_MULT = 2.0

# MACD：与同花顺默认口径一致 (12, 26, 9)；柱体按国内常见 2*(DIF-DEA)。
...
# 周线MA5/MA10金叉 + 明显低于MA20：只用已完成的周线(周五收盘)，排除进行中的本周，
# 避免未来函数。WEEKLY_MA_GAP_MIN_PCT参考用户给出的露笑科技例子(MA5比MA20低约15%)
# 取偏保守的下限，未在更大样本上验证过，先作为起始值。
WEEKLY_MA_GOLDEN_CROSS_LOOKBACK = 4
WEEKLY_MA_GAP_MIN_PCT = 8.0
...
def normalize_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    # 注意：这里不再截断到LOOKBACK行——保留全部拉取到的历史（fetch_ohlcv本来就按
    # LOOKBACK+110天去请求，只是以前在这里被提前丢弃了），把"只取最近LOOKBACK行"
    # 这一步挪到analyze()内部做，好让周线金叉信号能用到更长的原始历史，同时不影响
    # 筹码/影线/OBV/均线这些原本就基于最近LOOKBACK行计算的逻辑——它们的输入切片
    # 跟改动前完全一样。
...
def compute_boll_features(data: pd.DataFrame) -> dict[str, Any]:
    """布林带(20, 2)：与同花顺默认参数一致。输出中轨/上轨/下轨、带宽、收盘相对位置。"""
    default = {
        "boll_mid": None, "boll_upper": None, "boll_lower": None,
        "boll_width_pct": None, "boll_pos": None,
        "close_above_boll_mid": False, "close_near_boll_upper": False,
        "boll_squeeze": False,
    }
    close = data["close"].astype(float)
    n = len(close)
    if n < BOLL_PERIOD:
        return default
    mid = close.rolling(BOLL_PERIOD, min_periods=BOLL_PERIOD).mean()
    std = close.rolling(BOLL_PERIOD, min_periods=BOLL_PERIOD).std(ddof=0)
    last_mid = float(mid.iloc[-1]) if pd.notna(mid.iloc[-1]) else None
    last_std = float(std.iloc[-1]) if pd.notna(std.iloc[-1]) else None
    if last_mid is None or last_std is None or last_mid <= 0:
        return default
    upper = last_mid + BOLL_STD_MULT * last_std
    lower = last_mid - BOLL_STD_MULT * last_std
    last_close = float(close.iloc[-1])
    width_pct = (upper - lower) / last_mid * 100.0
    # 收盘在上下轨之间的相对位置：0=下轨，1=上轨
    band = upper - lower
    pos = (last_close - lower) / band if band > 1e-8 else 0.5
    # 近 20 日带宽分位，用于 squeeze 粗判（带宽落在近20日最窄 25% 内）
    width_series = (2 * BOLL_STD_MULT * std / mid * 100.0).dropna()
    squeeze = False
    if len(width_series) >= 10:
        q25 = float(width_series.tail(20).quantile(0.25))
        squeeze = bool(width_pct <= q25 + 1e-9)
    return {
        "boll_mid": round(last_mid, 2),
        "boll_upper": round(upper, 2),
        "boll_lower": round(lower, 2),
        "boll_width_pct": round(width_pct, 2),
        "boll_pos": round(float(pos), 3),
        "close_above_boll_mid": bool(last_close > last_mid),
        "close_near_boll_upper": bool(pos >= 0.85),
        "boll_squeeze": squeeze,
    }


def compute_macd_features(data: pd.DataFrame) -> dict[str, Any]:
    """MACD(12,26,9)：与同花顺默认参数一致；柱体=2*(DIF-DEA)。"""
    default = {
        "macd_dif": None, "macd_dea": None, "macd_hist": None,
        "macd_golden_cross": False, "macd_dif_above_zero": False,
    ...
    prev_dif = float(dif.iloc[-2]) if n >= 2 else last_dif
    prev_dea = float(dea.iloc[-2]) if n >= 2 else last_dea
    prev_hist = float(hist.iloc[-2]) if n >= 2 else last_hist
    golden = bool(prev_dif <= prev_dea and last_dif > last_dea)
    expanding = bool(last_hist > prev_hist and last_hist > 0)
    return {
        "macd_dif": round(last_dif, 4),
        "macd_dea": round(last_dea, 4),
        "macd_hist": round(last_hist, 4),
        "macd_golden_cross": golden,
        "macd_dif_above_zero": bool(last_dif > 0),
        "macd_hist_expanding": expanding,
    }


def compute_weekly_ma_signal(full_data: pd.DataFrame) -> dict[str, Any]:
    """周线MA5/MA10金叉、且金叉发生时两条短均线都明显低于MA20(现价还远离/低于长期
    均线)。只用已完成的周线(W-FRI)，排除进行中的本周，避免未来函数。

    full_data要求是normalize_frame()返回的、未被截断到LOOKBACK行的完整历史——
    LOOKBACK(100个交易日≈20周)对这个信号来说太短，MA20(周)连一根完整值都算不出来，
    更别说给金叉留回看余量，所以这里专门吃完整历史，跟chip/影线/OBV/均线那些用
    analyze()里截断后的data计算的字段是两条独立路径。
    """
    default = {
        "weekly_ma5": None, "weekly_ma10": None, "weekly_ma20": None,
        "weekly_golden_cross_5_10": False, "weekly_ma5_below_ma20_pct": None,
        "is_weekly_golden_cross_far_from_ma20": False,
    }
    if full_data.empty:
        return default
    indexed = full_data.copy()
    ...
    if weekly.empty:
        return default
    last_daily_date = indexed.index.max()
    if weekly.index[-1] > last_daily_date:
        weekly = weekly.iloc[:-1]  # 排除进行中的本周
    if len(weekly) < 20:
        return default
    ...
    last_ma10 = float(ma10.iloc[-1]) if pd.notna(ma10.iloc[-1]) else None
    last_ma20 = float(ma20.iloc[-1]) if pd.notna(ma20.iloc[-1]) else None
    if last_ma5 is None or last_ma10 is None or last_ma20 is None:
        return default

    n = len(weekly)
    golden_cross = False
    lookback = min(WEEKLY_MA_GOLDEN_CROSS_LOOKBACK, n - 1)
    for i in range(1, lookback + 1):
        prev5, prev10 = ma5.iloc[-1 - i], ma10.iloc[-1 - i]
        curr5, curr10 = ma5.iloc[-i], ma10.iloc[-i]
        if all(pd.notna(x) for x in (prev5, prev10, curr5, curr10)) and prev5 <= prev10 and curr5 > curr10:
            golden_cross = True
            break

    gap_pct = (last_ma20 - last_ma5) / last_ma20 * 100.0 if last_ma20 > 0 else None
    is_far = bool(
        golden_cross
        and gap_pct is not None
        and last_ma5 < last_ma20
        and last_ma10 < last_ma20
        and gap_pct >= WEEKLY_MA_GAP_MIN_PCT
    )
    return {
        "weekly_ma5": round(last_ma5, 4),
        "weekly_ma10": round(last_ma10, 4),
        "weekly_ma20": round(last_ma20, 4),
        "weekly_golden_cross_5_10": golden_cross,
        "weekly_ma5_below_ma20_pct": round(gap_pct, 2) if gap_pct is not None else None,
        "is_weekly_golden_cross_far_from_ma20": is_far,
    }
...
def analyze(code: str, name: str, frame: pd.DataFrame) -> dict[str, object]:
    ...
    full_data = normalize_frame(frame)
    data = full_data.tail(LOOKBACK).reset_index(drop=True)
    ...
    boll_feat = compute_boll_features(data)
    macd_feat = compute_macd_features(data)
    weekly_feat = compute_weekly_ma_signal(full_data)
    ...
    # 红色筹码多、绿色筹码少：整体筹码结构里获利盘(红)占主导、套牢盘(绿)很轻，
    # 跟is_chip_spike/is_below_spike不是一回事——那两个看的是某个峰是否窄而突出，
    # 这个看的是现价上下两侧筹码总量的对比，不要求有明显尖峰。
    green_chip_pct = round(100.0 - profit_pct, 2)
    is_red_heavy_chip = bool(profit_pct >= RED_HEAVY_PROFIT_MIN)
    ...
    signal = (
        ...
        else "红筹码占优·套牢盘轻" if is_red_heavy_chip
        else "周线MA5/10金叉·远离MA20" if weekly_feat["is_weekly_golden_cross_far_from_ma20"]
        else "尖峰关注·筹码主峰" if is_chip_spike
        else "长影线关注·下影买入" if shadow_feat["daily_long_lower"]
        else "观察·接近尖峰未确认" if in_zone
        else "无"
    )
    if ma_feat["ma_signal"].startswith("强多") and (
        tradeable or is_below_spike or is_wide_zone or is_red_heavy_chip
        or weekly_feat["is_weekly_golden_cross_far_from_ma20"] or is_chip_spike
        or shadow_feat["daily_long_lower"] or in_zone
    ):
        signal = "高潜力·筹码+均线共振"
    ...
    return {
        ...
        "weekly_ma5": weekly_feat["weekly_ma5"],
        "weekly_ma10": weekly_feat["weekly_ma10"],
        "weekly_ma20": weekly_feat["weekly_ma20"],
        "weekly_golden_cross_5_10": weekly_feat["weekly_golden_cross_5_10"],
        "weekly_ma5_below_ma20_pct": weekly_feat["weekly_ma5_below_ma20_pct"],
        "is_weekly_golden_cross_far_from_ma20": weekly_feat["is_weekly_golden_cross_far_from_ma20"],
        "is_red_heavy_chip": is_red_heavy_chip,
        "green_chip_pct": green_chip_pct,
        ...
        "boll_mid": boll_feat["boll_mid"],
        "boll_upper": boll_feat["boll_upper"],
        "boll_lower": boll_feat["boll_lower"],
        "boll_width_pct": boll_feat["boll_width_pct"],
        "boll_pos": boll_feat["boll_pos"],
        "close_above_boll_mid": boll_feat["close_above_boll_mid"],
        "close_near_boll_upper": boll_feat["close_near_boll_upper"],
        "boll_squeeze": boll_feat["boll_squeeze"],
        "macd_dif": macd_feat["macd_dif"],
        "macd_dea": macd_feat["macd_dea"],
        "macd_hist": macd_feat["macd_hist"],
        "macd_golden_cross": macd_feat["macd_golden_cross"],
        "macd_dif_above_zero": macd_feat["macd_dif_above_zero"],
        "macd_hist_expanding": macd_feat["macd_hist_expanding"],
...
def self_test() -> None:
    ...
    assert {"is_chip_spike", "main_peak_above_close", "daily_long_lower", "daily_long_upper",
            ...
            "obv_sticky", "obv_ma_spread_pct", "is_red_heavy_chip", "green_chip_pct"}.issubset(result)
    assert {"boll_mid", "boll_upper", "boll_lower", "boll_width_pct", "boll_pos",
            "close_above_boll_mid", "close_near_boll_upper", "boll_squeeze"}.issubset(result)
    assert {"macd_dif", "macd_dea", "macd_hist", "macd_golden_cross",
            "macd_dif_above_zero", "macd_hist_expanding"}.issubset(result)
    # 红绿互补回归：green_chip_pct必须等于100-profit_pct，且is_red_heavy_chip的判定
    # 必须跟RED_HEAVY_PROFIT_MIN阈值一致，不能两个字段各算各的、慢慢漂移。
    assert abs(result["green_chip_pct"] - (100.0 - result["profit_pct"])) < 1e-6
    assert result["is_red_heavy_chip"] == (result["profit_pct"] >= RED_HEAVY_PROFIT_MIN)
    assert "short_term_note" in result and isinstance(result["short_term_note"], str)
    assert {"weekly_ma5", "weekly_ma10", "weekly_ma20", "weekly_golden_cross_5_10",
            "weekly_ma5_below_ma20_pct", "is_weekly_golden_cross_far_from_ma20"}.issubset(result)
    ...
    # 周线MA5/10金叉+远离MA20回归：构造一段"大幅下跌后温和修复"的走势——20周均线
    # 被下跌阶段的高价拖高，5/10周均线在修复阶段形成金叉，但仍明显低于20周均线，
    # 跟露笑科技这个真实案例的形态一致。
    weekly_rng = np.random.RandomState(3)
    pre_days, decline_days, recover_days = 250, 90, 35
    weekly_n = pre_days + decline_days + recover_days
    weekly_dates = pd.bdate_range("2025-01-01", periods=weekly_n)
    weekly_close = np.concatenate([
        np.linspace(8, 20, pre_days),
        np.linspace(20, 6, decline_days),
        np.linspace(6, 7.5, recover_days),
    ])
    weekly_frame = pd.DataFrame({
        "date": weekly_dates,
        "open": weekly_close - 0.05, "high": weekly_close + 0.15, "low": weekly_close - 0.15,
        "close": weekly_close,
        "volume": np.full(weekly_n, 80000.0) + weekly_rng.normal(0, 500, weekly_n),
        "turnover": [0.02] * weekly_n,
    })
    weekly_result = analyze("002617", "周线金叉样本", weekly_frame)
    assert {"weekly_ma5", "weekly_ma10", "weekly_ma20", "weekly_golden_cross_5_10",
            "weekly_ma5_below_ma20_pct", "is_weekly_golden_cross_far_from_ma20"}.issubset(weekly_result)
    assert weekly_result["weekly_golden_cross_5_10"] is True, "未检测到周线5/10金叉"
    assert weekly_result["is_weekly_golden_cross_far_from_ma20"] is True, "未判定为远离MA20"
    assert weekly_result["weekly_ma5"] < weekly_result["weekly_ma20"]
    assert weekly_result["weekly_ma10"] < weekly_result["weekly_ma20"]
    assert weekly_result["weekly_ma5_below_ma20_pct"] >= WEEKLY_MA_GAP_MIN_PCT
