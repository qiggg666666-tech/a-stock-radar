...
工程层: 双源baostock+东财/超时/推送软导入/交易日/行业join/聚类/收尾防护sys.exit(0)。
"""
import os
import re
import sys
    ...
    try:
        ...
        with open(os.path.join(OUTPUT_DIR, f"combo_{tag}.json"), 'w', encoding='utf-8') as f:
            json.dump({"date": tag, "fr_cluster": fr_cl, "res_cluster": res_cl,
                       ...
                       "n_dual": int(df["双确认"].sum()),
                       "hits": df.drop(columns=["_detail"], errors="ignore").to_dict('records')},
