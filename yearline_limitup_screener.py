...
def main():
    ...
    with ctx.Pool(
        ...
        processed_count = 0
        save_interval = 200  # 每处理 200 只股票增量存盘一次
        ...
        for ar in async_results:
            ...
            if processed_count % save_interval == 0 and results:
                ...
                temp_df = temp_df.sort_values(by="综合评分", ascending=False).reset_index(drop=True)
                temp_path = output_dir / f"年线涨停_评分_{today}_checkpoint.csv"
                temp_df.to_csv(temp_path, index=False, encoding="utf-8-sig")
                log.info(f"💾 增量存盘: 已处理 {processed_count}/{len(async_results)}，当前命中 {len(results)} 只")
