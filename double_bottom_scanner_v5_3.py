...
【本版修复】
  ① get_all_a_share_symbols 恢复 finally: bs.logout()
     —— 主进程取完列表即登出, 避免 fork 时把活动 baostock 套接字带进 worker / 单线程重复登录
  ② 无股票/无数据源时 sys.exit(1) -> sys.exit(0), 防 Actions 瞬时失败红叉
其余沿用 v5.3: forming-only / near-neckline / list-symbols / 断点续扫 / 会话复用 / 查询超时。
用法: python double_bottom_scanner_v5_3.py --all --periods W,M,Q,Y --workers 3 --min-score 50 --forming-only --near-neckline 10 --push --top 30
"""
import argparse
import glob
import logging
...
class Config:
    ...
    NUM_PROCESSES = 1; FETCH_TIMEOUT_SEC = 30; STAGGER_DELAY_RANGE = (0.3, 1.5); MAX_RETRIES = 2
    OUTPUT_DIR = os.environ.get("OUTPUT_DIR", "output"); SCAN_LIMIT = int(os.environ.get("SCAN_LIMIT", "0"))
...
def get_all_a_share_symbols() -> List[str]:
    ...
    try:
        ...
        for c in df["code"].tolist():
            plain = str(c).split(".")[-1]
            if plain[:2] in ("60", "00", "30", "68"): out.append(plain)
        return sorted(set(out))
    except Exception as e:
        logger.warning(f"获取全市场列表失败: {e}"); return []
    finally:
        try: bs.logout()
        except Exception: pass
...
def fetch_with_timeout(fn, *args, timeout: int = Config.FETCH_TIMEOUT_SEC, **kwargs):
    ...
    with ThreadPoolExecutor(max_workers=1) as ex:
        ...
        try: return future.result(timeout=timeout)
            ...
            logger.warning(f"{fn.__name__} 超时({timeout}s)"); return None
        except Exception as e:
            logger.warning(f"{fn.__name__} 异常: {e}"); return None
...
def get_daily_data(symbol: str, is_a_share: bool = True, years: int = 12, source: str = "auto") -> Optional[pd.DataFrame]:
    if not is_a_share: return fetch_with_timeout(get_yf_data, symbol, years)
    tried = []
    if source in ("auto", "baostock") and HAS_BS:
        for attempt in range(Config.MAX_RETRIES):
            ...
            tried.append("baostock"); time.sleep(1.0)
        if source == "baostock": return None
    if source in ("auto", "akshare") and HAS_AK:
        df = fetch_with_timeout(get_ak_data, symbol, years)
        if df is not None and len(df) > 50: return df
...
def save_checkpoint(all_rows, tag, suffix=""):
    if not all_rows: return
    os.makedirs(Config.OUTPUT_DIR, exist_ok=True)
    path = os.path.join(Config.OUTPUT_DIR, f"double_bottom_{tag}{suffix}.csv")
    pd.DataFrame(all_rows).sort_values("total_score", ascending=False).to_csv(path, index=False, encoding="utf-8-sig")
    logger.info(f"💾 中间结果已保存: {path} ({len(all_rows)} 条)")
...
def load_symbols(args):
    if getattr(args, "all_market", False):
        ...
        if Config.SCAN_LIMIT > 0 and len(syms) > Config.SCAN_LIMIT:
            ...
            logger.info(f"已随机抽取前 {Config.SCAN_LIMIT} 只")
        return syms
...
def main():
    parser = argparse.ArgumentParser(description="多周期双坑底扫描器 v5.3 —— 矩阵修复版")
    ...
    parser.add_argument("--batch-save", type=int, default=100, help="每N只保存一次中间结果")
    ...
    # ==================== 单线程模式 ====================
    if args.workers <= 1:
        iterator = tqdm(enumerate(symbols), total=len(symbols), desc="扫描中", ncols=80) if HAS_TQDM else enumerate(symbols)
        for idx, sym in iterator:
            try:
                res, status = scan_symbol(sym, periods=periods, is_a_share=sym.isdigit() and len(sym) == 6,
                                          min_score=args.min_score, source=args.source)
            except Exception as e:
                logger.warning(f"[{sym}] 扫描异常: {e}"); stats["error"] += 1; continue
            if status == "no_data": stats["no_data"] += 1; continue
            if status == "error": stats["error"] += 1; continue
            if res:
                stats["success"] += 1
                for period_name, patterns in res.items():
                    for p in patterns: all_rows.append(p.to_dict())
            else: stats["no_pattern"] += 1
            if (idx + 1) % args.batch_save == 0 and all_rows:
                save_checkpoint(all_rows, tag, suffix=f"_checkpoint_{idx+1}")
    # ==================== 多进程模式 ====================
    else:
        tasks = [(sym, periods, sym.isdigit() and len(sym) == 6, args.min_score, args.source) for sym in symbols]
        with ProcessPoolExecutor(max_workers=args.workers, initializer=_worker_bs_init) as executor:
            futures = {executor.submit(_scan_worker, task): task[0] for task in tasks}
            iterator = tqdm(list(futures), desc="扫描中", ncols=80, total=len(futures)) if HAS_TQDM else futures
            for idx, fut in enumerate(iterator):
                sym = futures[fut]
                try:
                    _, (res, status) = fut.result()
                except Exception as e:
                    logger.warning(f"[{sym}] 进程异常: {e}"); stats["error"] += 1; continue
                if status == "no_data": stats["no_data"] += 1; continue
                if status == "error": stats["error"] += 1; continue
                if res:
                    stats["success"] += 1
    ...
    if all_rows:
        ...
        for pname, cnt in period_counts.items():
            print(f"   • {pname}: {cnt} 条")
        # 彩色 Top20 详细输出（直接用 DataFrame, 无对象引用）
        print("\n" + "=" * 70)
