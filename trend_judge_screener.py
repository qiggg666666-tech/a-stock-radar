# -*- coding: utf-8 -*-
"""
trend_judge_screener.py —— 综合走势判断 + 选股(多因子技术面强弱评分) · 矩阵规格
...
默认只推评分≥SCORE_MIN(偏多及以上)的票, 按评分降序。

【诚实定位-必读】本脚本是【概率性技术面强弱评分, 非预测】。它给出"当前哪些票技术面最强/最弱",
  帮你缩小盯盘范围/提高胜率, 不等于"某只一定涨"; 多空标签是当下截面状态, 数据变标签也变。
  技术信号本质是提高胜率的证据, 不是预知未来; 务必结合仓位/止损, 不构成投资建议。
【本版规格】双源baostock+东财+硬超时; 全市场多进程(每子进程独立登录baostock, 命门已修);
  append兼容补丁(防baostock在pandas2.x崩); 宽松快照预筛(失败退化全扫, 靠SCAN_LIMIT保底);
  baostock行业本地join+聚类+东财风口🎯; 推送全发分页(严格检查返回); 存output/+收尾防护+
  sys.exit(0); 不拦交易日(周末用上一交易日数据)。指标全部手写, 不依赖pandas_ta/matplotlib。
"""
import os
import re
import sys
...
def check_one_stock(df: pd.DataFrame):
    """返回 (命中dict 或 None, 失败原因 或 None)。综合5维度算走势评分+多空标签。"""
...
def run_scan():
    ...
    print(f"开始综合走势判断扫描 {len(tasks)} 只（{PARAMS['NUM_PROCESSES']}进程, 双源, 评分门槛≥{SCORE_MIN_ENV}）...")
        ...
        for res in pool.imap_unordered(_process_one, tasks):
            if res:
                if "__fail__" in res:
                    ...
                    results.append(res)
                    pbar.write(f"  {res['多空标签']} {res['代码']} {res['名称']} 评分{res['走势评分']} 趋{res['趋势分']}动{res['动量分']}量{res['量价分']}M{res['MACD分']}位{res['位置分']}")
    ...
    if not df.empty:
        df = df.sort_values('走势评分', ascending=False).reset_index(drop=True)
    return df
...
def enrich(df):
    ...
    df2 = df2.sort_values(['resonance', '走势评分'], ascending=[False, False]).reset_index(drop=True)
...
def build_push(df, cluster, hot):
    ...
    L = [f"**📊 综合走势判断** | 强多{len(strong)} 偏多{len(bull)} 🎯风口{len(reso)} (全发, 评分≥{SCORE_MIN_ENV})",
         "*(趋势+动量+量价+MACD+位置 五维合成评分0-100+多空标签; 概率性技术面强弱, 非预测; 结合仓位/止损)*", ""]
    if hot:
        L.append("🌪️ **风口**: " + "、".join(f"{n}({c}%)" for n, c in hot[:6])); L.append("")
    if cluster:
        L.append("📊 **走势强势板块**: " + "、".join(f"{n}({c})" for n, c in cluster)); L.append("")
    def line(r):
        return (f"- {r['多空标签']} **{r['名称']}({r['代码']})** [{_sec_tag(r.to_dict())}] 评分{r['走势评分']} | 现价{r['最新价']} "
                f"涨{r['近20日涨幅%']}% RSI{r['RSI']} 量比{r['量比']} | 趋{r['趋势分']}动{r['动量分']}量{r['量价分']}M{r['MACD分']}位{r['位置分']}")
    if not reso.empty:
    ...
    if not strong.empty:
        ...
        L += [line(r) for _, r in strong.iterrows()]; L.append("")
    if not bull.empty:
        L.append(f"### 🟢 偏多 共{len(bull)}只")
        L += [line(r) for _, r in bull.iterrows()]
    return "\n".join(L)
...
def main():
    print("=" * 70)
    print(f"📊 综合走势判断+选股 | {datetime.now():%Y-%m-%d %H:%M} | 回看{PARAMS['lookback_days']}天 评分门槛≥{SCORE_MIN_ENV}")
    print(f"全扫={'是' if not SCAN_LIMIT else f'限{SCAN_LIMIT}'}; 双源; 预筛={'开' if PARAMS['SNAPSHOT_PRE'] else '关'}; 不拦交易日; 推送全列+分页")
    print("⚠️ 概率性技术面强弱评分, 非预测; 结合仓位/止损, 不构成投资建议")
    print("=" * 70)
    df = run_scan()
    if df is None or df.empty:
        print(f"\n本次未发现走势评分≥{SCORE_MIN_ENV}的票(市场整体偏弱, 或门槛需下调 SCORE_MIN)。")
        sys.exit(0)
    ...
    try:
        ...
        with open(os.path.join(OUTPUT_DIR, f"trend_judge_{tag}.json"), 'w', encoding='utf-8') as f:
            json.dump({"date": tag, "params": {"SCORE_MIN": SCORE_MIN_ENV,
    ...
    if SERVERCHAN_KEY:
        try:
            ...
            send_serverchan(f"📊 走势判断 强多{n_strong}只 共{len(df)}只 🎯风口{n_reso}", build_push(df, cluster, hot))
        except Exception as e:
