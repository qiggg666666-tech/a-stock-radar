...
SENSITIVE_CONFIG_TOKENS = ("key", "token", "secret", "password", "credential", "authorization")
SENSITIVE_TEXT_PATTERN = re.compile(r"(?i)(sendkey|serverchan|token|secret|password|api[_-]?key)=?[^\s,;]+")


def redact_text(value: object, limit: int = 220) -> str:
    """将异常文本压缩为可写入artifact的脱敏诊断。"""
    compact = " ".join(str(value).replace("\n", " ").split())
    compact = SENSITIVE_TEXT_PATTERN.sub("<redacted>", compact)
    return compact[:limit] or "未提供详情"


def public_config_snapshot(cfg: Config) -> dict[str, Any]:
    """结果artifact只保存可复现实验参数，绝不保存通知或访问凭据。"""
    snapshot: dict[str, Any] = {}
    for key, value in asdict(cfg).items():
        if any(token in key.lower() for token in SENSITIVE_CONFIG_TOKENS):
            continue
...
def baostock_cursor_to_frame(response: Any) -> pd.DataFrame:
    """逐行读取BaoStock游标，避免response.get_data()内部依赖已删除的DataFrame.append。"""
...
def child_universe(result_queue: Any, source: str, cfg_dict: dict[str, Any]) -> None:
    ...
    try:
        if source == "akshare":
            ...
            try:
                raw = baostock_cursor_to_frame(bs.query_stock_basic())
            finally:
                try:
                    bs.logout()
                except Exception:
        ...
        result_queue.put({"kind": "error", "reason": redact_text(f"{type(error).__name__}:{error}")})
...
def load_shared_universe(path: Path) -> list[dict[str, str]]:
    """读取由上游任务准备的股票池，禁止四片再次并发请求同一数据。"""
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except Exception as error:
        raise DataSourceError(f"共享股票池文件不可读:{redact_text(error)}") from error
    source = str(payload.get("source", ""))
    raw_universe = payload.get("universe")
    if source not in {"akshare", "baostock", "cache"} or not isinstance(raw_universe, list):
        raise DataSourceError("共享股票池结构无效")
    ...
    for item in raw_universe:
        ...
        if re.fullmatch(r"[036]\d{5}", code) and name and "ST" not in name and "退" not in name:
            selected[code] = name
    if len(selected) < 1000:
        raise DataSourceError(f"共享股票池有效代码不足:{len(selected)}")
    return [{"代码": code, "名称": selected[code]} for code in sorted(selected)]


def isolated_universe(cfg: Config) -> list[dict[str, str]]:
    if cfg.universe_file is not None:
        return load_shared_universe(cfg.universe_file)
    ctx = context()
    cfg_dict = asdict(cfg)
    cfg_dict["output_dir"] = str(cfg.output_dir)
    reasons: list[str] = []
...
def run_scan(cfg: Config) -> int:
    ...
    try:
        ...
        safe_reason = redact_text(error)
        LOGGER.error("股票池获取失败：%s", safe_reason)
        save_outputs(
            cfg,
            0,
            set(),
            [],
            {"source_error": 1, "logic_error": 0, "timeout": 0, "no_signal": 0, "worker_crash": 0},
            "universe_unavailable",
            started_at,
            diagnostics=[{"stage": "universe", "reason": safe_reason}],
        )
        # 退出码2让strategy_shard_runner与GitHub清晰标记“业务未扫描”。
        return 2
