...
def load_universe_payload(path: Path) -> dict[str, Any]:
    return json.loads(path.read_text(encoding="utf-8"))


def load_universe(payload: dict[str, Any]) -> list[dict[str, str]]:
...
def main() -> int:
    ...
    if args.self_test:
        ...
        # 优雅跳过回归：universe状态不是ready/degraded_cache时，load_universe必须
        # 不被调用到（因为它对空/无效universe会raise），而是在main()里提前分支处理。
        for bad_status in (None, "universe_unavailable", "something_else"):
            assert bad_status not in {"ready", "degraded_cache"}
        try:
            load_universe({"status": "universe_unavailable", "universe": []})
            raise AssertionError("empty universe should still raise when called directly")
        except ValueError:
            pass
        print("SELF_TEST_OK")
        return 0
    ...
    universe_payload = load_universe_payload(args.universe_file)
    universe_status = universe_payload.get("status")
    if universe_status not in {"ready", "degraded_cache"}:
        # 跟final_chip_shard.py同一套优雅跳过：universe源(含缓存兜底)都不可用时，
        # 这个分片不应该报错崩溃——写一份空的、状态明确的产物，让summary能看出
        # 当天没有数据可用，而不是让整条workflow红掉。
        args.output_dir.mkdir(parents=True, exist_ok=True)
        status = {
            "schema_version": "dinghai-shard/v2",
            ...
            "disclosure": "定海神针独立研究分片；不读取DistilledQuant核心或520低位首红的输入输出。",
        }
        pd.DataFrame().to_csv(args.output_dir / "raw_records.csv", index=False, encoding="utf-8-sig")
    ...
    queue = shard_slice(load_universe(universe_payload), args.shard_index, args.shard_count)
    ...
    status = {"schema_version": "dinghai-shard/v2", "status": "ready" if not errors else "degraded", "signal_date_requested": signal_date.strftime("%Y-%m-%d"), "shard_index": args.shard_index, "shard_count": args.shard_count, "scan_queue_count": len(queue), "valid_record_count": len(records), "error_count": len(errors), "daily_data_source_counts": source_counts, "universe_status": universe_status, "disclosure": "定海神针独立研究分片；不读取DistilledQuant核心或520低位首红的输入输出。"}
    (args.output_dir / "status.json").write_text(json.dumps(status, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(status, ensure_ascii=False))
    return 0
