# -*- coding: utf-8 -*-
"""
dip_buy_screener.py —— 上升趋势·回调低点买入选股
"低点买入"=回调买入: 在已向上的趋势里, 等价格缩量回踩支撑/超卖时介入, 不接下跌飞刀。
逻辑=趋势保护伞(收盘>MA60 且 周/月多头, 否则排除) + 回调到位(回撤5-25%+缩量+回踩MA+RSI转弱+止跌)。
分级 🟢强买点/🟡观察; 顶背离或放量跌标⚠️风险降级。回调或转跌, 必设止损。

【本版新增·价格时点+实时对齐列】解决"推送价≠行情实时价"的困惑:
  推送每行末尾加 "🕒信号价@信号日(距今N天) → 现价X@run(±Y%)"。
  - 信号价=信号触发那天收盘(定格, 不刷新); 现价=本次run拉的一次实时/收盘快照。
  - 收盘后跑时, 现价=当天收盘, 对齐列正好显示"信号后隔几天涨/跌多少"=策略命中成绩单。
  - 实时快照限流/失败 -> 对齐列降级只显示前半段, 绝不阻断推送/存盘。
  - 口径: 信号价=前复权日线(用于判断), 现价=不复权实时(用于看盘); 若信号后除过权, 涨幅仅供参考。
  策略判定/存盘/控制台展示 一字未动。
"""
import os
import re
...
PARAMS = dict(
    ...
    LOOKBACK_DAYS=1500, MA_TREND=60, MA_SHORT=20, RSI_P=14,
    MIN_DD=5.0, MAX_DD=25.0, NEAR_MA_PCT=3.0,
    SHRINK_RATIO=0.8, HEAVY_RATIO=1.3, RSI_LOW=45, RSI_WAS=55,
...
def _fetch_spot_now():
    ...
    收盘后跑时, 此处'现价'=当天收盘价, 对齐列正好显示'信号后隔几天涨/跌多少'。"""
    try:
        d = _call_with_timeout(ak.stock_zh_a_spot_em, timeout=AK_TIMEOUT)
...
def _process_one(args):
    ...
    try:
        ...
        return {"代码": code, "名称": name, "行业": "", "最新价": round(float(h['close'].iloc[-1]), 2),
                "信号日期": str(h['date'].iloc[-1])[:10],   # 【本版新增】信号基于的最后一根K线日期
...
def build_push(df, cluster, spot_now=None):
    P = PUSH_TOP
    strong = df[df["买点"].str.contains("🟢", na=False)].sort_values("分", ascending=False)
    watch = df[~df["买点"].str.contains("🟢", na=False)].sort_values("分", ascending=False)
    L = [f"**🛒 上升趋势·回调低点买入** | 🟢强买点{len(strong)} 🟡观察{len(watch)}",
         "*(趋势向上+缩量回踩超卖; 非买绝对最低; 回调或转跌, 必设止损; 熊市假信号靠趋势伞过滤)*",
         "*(🕒每行末'信号价@日期→现价@run'=信号触发日收盘 vs 本次run实时价, 含信号后涨跌幅; 信号价=前复权日线, 现价=不复权实时, 期间除权则涨幅仅供参考)*", ""]
    if cluster:
        L.append("🛒 **回调买点扎堆板块**: " + "、".join(f"{n}({c})" for n, c in cluster))
        L.append("")
    def line(r):
        rk = " ⚠️" if r["风险"] else ""
        base = (f"- {r['买点']} **{r['名称']}({r['代码']})** [{r['行业']}] 现价{r['最新价']} 回撤{r['回撤%']}% "
                f"缩量比{r['缩量比']} RSI{r['RSI']} 周{'✓' if r['周'] else '✗'} 月{'✓' if r['月'] else '✗'}{rk}")
        return base + _align_suffix(r, spot_now)   # 【本版新增】时点+实时对齐后缀
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open(os.path.join(OUTPUT_DIR, f"dip_buy_{tag}.json"), 'w', encoding='utf-8') as f:
            json.dump({"date": tag, "params": PARAMS, "cluster": cluster,
                       "n_strong": int(df["买点"].str.contains("🟢", na=False).sum()),
                       "n_watch": int((~df["买点"].str.contains("🟢", na=False)).sum()),
    ...
    if SERVERCHAN_KEY:
        try:
            ...
            send_serverchan(f"🛒 回调低点买入 | 🟢{int(df['买点'].str.contains('🟢', na=False).sum())} 🟡{int((~df['买点'].str.contains('🟢', na=False)).sum())}",
                            build_push(df, cluster, spot_now))
        except Exception as e:
