...
def run_scan(args: argparse.Namespace, config: FetchConfig, params: Params) -> int:
    ...
    for position, row in universe.iterrows():
        ...
        if status["scanned"] >= 20 and status["failed"] / max(1, status["scanned"]) > args.max_failure_rate:
            ...
            break
        if time.monotonic() - started_monotonic >= args.max_runtime_seconds:
            status["status"] = "runtime_limit"
            status["reason"] = f"主动运行时限{args.max_runtime_seconds}秒"
            break
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-runtime-seconds", type=int, default=int(os.getenv("MAX_RUNTIME_SECONDS", "19200")), help="主动停止并写出状态的最长运行秒数")
