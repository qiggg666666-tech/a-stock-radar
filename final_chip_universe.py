#!/usr/bin/env python3
"""FINAL Chip专属A股共同股票池：双源、缓存降级和显式状态。"""
...
def baostock_universe() -> list[dict[str, str]]:
    ...
    try:
        ...
        while result.next():
            ...
            if valid_code(code) and row[fields["type"]] == "1" and row[fields["status"]] == "1" and out_date in ("", "None"):
                rows.append({"code": code, "name": row[fields["code_name"]].strip()})
        if not rows:
...
def akshare_universe() -> list[dict[str, str]]:
    ...
    for _, item in table.iterrows():
        ...
        if valid_code(code):
            rows.append({"code": code, "name": str(item.iloc[1]).strip() if len(item) > 1 else ""})
    if not rows:
...
def main() -> int:
    ...
    errors: list[str] = []
    for label, source in (("akshare", akshare_universe), ("baostock", baostock_universe)):
