...
def main():
    print("=" * 70)
    print(f"📊 综合走势判断+形态共振 | {datetime.now():%Y-%m-%d %H:%M} | PATTERN_ONLY={'开' if PATTERN_ONLY else '关'} 形态加分{PATTERN_BONUS} | Offset={SCAN_OFFSET} Limit={SCAN_LIMIT}")
    print("=" * 70)
    df = run_scan()
    if df is None or df.empty:
