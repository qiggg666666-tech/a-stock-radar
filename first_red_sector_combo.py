...
一次全市场扫描, 每只票本地同时算 ①首红 ②动量/估值 ③站MA20/短动量; 扫完按国标行业
groupby 内联出板块榜(象限+多空+强度+接力), 再把每只首红票挂到行业板块标签上 ->
交叉分级 🌟共振/🟡追高/⚠️逆风/⚪中性/缺失。内联重算保证口径一致+自包含。
"""
import os
import re
import sys
...
PARAMS = dict(
    ...
    BULLISH=65.0, BEARISH=35.0, PE_LOW=30.0, PE_HIGH=70.0,
    LOOKBACK_DAYS=4000,
...
def build_push(fr, rank):
    ...
    if not rank.empty:
        ...
        L.append("看多🐂: " + "、".join(_clean(r["行业_raw"]) for _, r in rank[rank["多空"] == "🐂"].head(6).iterrows()) or "无")
