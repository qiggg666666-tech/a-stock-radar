...
def scan_market(args: argparse.Namespace, config: Config) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    state = {"version": VERSION, "state": "completed", "finished_at": now_text(), "shard": shard, "offset": args.offset, "limit": args.limit, "backtest": bool(args.backtest), "hold_days": args.hold_days if args.backtest else None, "universe": universe_meta, "stats": stats, "candidate_count": int(len(candidates)), "coverage_complete": len(completed.intersection({t['code'] for t in tasks})) == len(tasks), "config": asdict(config)}
