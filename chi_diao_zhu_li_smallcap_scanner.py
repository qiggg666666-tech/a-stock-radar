...
def make_config() -> Config:
    ...
    return Config(
        ...
        history_days=max(700, env_int("HISTORY_DAYS", 900)), output_dir=Path(os.getenv("OUTPUT_DIR", "output")),
...
def spot_worker(result_queue: Any) -> None:
    try:
...
def get_spot_hard(config: Config) -> pd.DataFrame:
    context = ctx(); result_queue = context.Queue(maxsize=1); process = context.Process(target=spot_worker, args=(result_queue,)); process.start()
    try: payload = result_queue.get(timeout=config.ak_timeout)
    except queue.Empty as error:
        if process.is_alive(): process.terminate()
        process.join(5); result_queue.close(); raise DataSourceError(f"股票池快照超时{config.ak_timeout}秒") from error
    process.join(5)
