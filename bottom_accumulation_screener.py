...
【本版核心·保证坑底能选出】上一版把"均线粘合+振幅挤压"当硬门槛, 致29.64那种急跌见底的票
  (均线发散/振幅放大)被卡死选不出。本版改门槛为【低位区+止跌宽松+坑底兜底】, 粘合/挤压降为标签:
  门槛 = ① 低位区: 距近LOW_LOOKBACK低点 ≤ BOTTOM_ZONE_PCT(20%) AND 距高点回撤 ≥ DRAWDOWN_MIN(30%);
        ② 止跌宽松(任一): 收阳/长下影/RSI超卖/不再深跌/MACD转强/站上MA5;
        ③ 坑底兜底: 距低点 ≤ PIT_PCT(5%) 的票【无论有无止跌都进】(保证29.64急跌底选出, 标⚠️探底)。
  非坑底且非止跌且非横盘(下跌中继)才排除。
四档状态标签(不卡门槛, 只标时机): 🔴坑底止跌 / ⚠️坑底探底(接飞刀) / 🟡低位横盘蓄力 / 🟠低位止跌。
  = 覆盖"见底那一刻(29.64,🔴/️)"与"跌完横盘(35.18,🟡)"两种时机, 一个脚本全收。

【工程规格】双源baostock+东财+硬超时; 多进程并发+快照预筛砍量; baostock行业本地join+聚类+东财风口🎯;
  推送全发分页(严格检查返回)+按状态分组; 信号vs实时对齐列; 不拦交易日; 收尾防护sys.exit(0); append补丁;
  前复权。全部阈值 env 可调(BOTTOM_ZONE_PCT/PIT_PCT/DRAWDOWN_MIN/RSI_LOW/MA_SPREAD_MAX/SQUEEZE_MAX...)。
⚠️⚠️ 坑底档(🔴/⚠️)=左侧极左=接飞刀: 29.64事后是底, 事前不知是否真底, 可能继续跌; ⚠️探底档连止跌都没有,
  风险最高。本脚本只提示"到了长期低位值得盯", 非买入信号; 真埋伏须极小仓位+跌破近期低点止损。
⚠️ 截面扫描: "选出29.64"=股价处于29.64坑底状态那天跑即报警; 今天35.18跑选的是35.18状态(🟡)。
"""
import os
import re
import sys
...
# ==================== 参数配置 (全部 env 可调) ====================
# 低位区门槛 (容纳35.18横盘~18% 与 29.64坑底~0%)
BOTTOM_ZONE_PCT = float(os.environ.get('BOTTOM_ZONE_PCT', '0.20'))   # 距近低点上限(20%)
DRAWDOWN_MIN = float(os.environ.get('DRAWDOWN_MIN', '0.30'))         # 距近高点最小回撤(30%, 保证是跌下来的低位)
LOW_LOOKBACK = int(os.environ.get('LOW_LOOKBACK', '250'))            # 低位/回撤参考窗口
# 坑底兜底 (保证29.64急跌底无条件选出)
PIT_PCT = float(os.environ.get('PIT_PCT', '0.05'))                   # 距低点≤此=坑底区(无论止跌与否都进)
# 止跌信号阈值
RSI_LOW = float(os.environ.get('RSI_LOW', '35'))
# 横盘标签阈值 (降级为标签, 不卡门槛)
MA_SPREAD_MAX = float(os.environ.get('MA_SPREAD_MAX', '0.02'))       # MA5/10/20极差<此=粘合
SQUEEZE_LOOKBACK = int(os.environ.get('SQUEEZE_LOOKBACK', '10'))
SQUEEZE_MAX = float(os.environ.get('SQUEEZE_MAX', '0.06'))           # 近N日振幅<此=挤压
# 防僵尸
...
# ==================== 策略内核 (低位区+止跌宽松+坑底兜底; 粘合/挤压降为标签) ====================
def check_one_stock(df):
    """门槛=低位区+止跌宽松(坑底兜底无条件进); 返回 (命中dict 或 None, 失败原因 或 None)。"""
    ...
    # 不再深跌: 近5日最低 >= 近15日最低*0.99
    no_new_low = bool(low.iloc[-5:].min() >= low.iloc[-15:].min() * 0.99) if L >= 15 else False
    macd_turn = bool(macd_hist.iloc[L] > macd_hist.iloc[L - 1])  # 绿柱缩短/红柱放大
    ...
    # 排除下跌中继: 非坑底 且 无止跌 且 非横盘
    if not is_pit and not stop and not is_sideways:
        return None, "无信号"
    ...
    score = round((1 - min(above_low, BOTTOM_ZONE_PCT) / BOTTOM_ZONE_PCT) * 40
                  + (25 if is_pit else 0) + (15 if stop else 0) + (10 if is_sideways else 0), 1)
...
def run_scan():
    ...
    print(f"开始底部坑底/蓄势扫描 {len(tasks)} 只（{NUM_PROCESSES}进程, 双源; 低位区≤{BOTTOM_ZONE_PCT*100:.0f}%+回撤≥{DRAWDOWN_MIN*100:.0f}%; 坑底≤{PIT_PCT*100:.0f}%无条件进; 粘合/挤压=标签）...")
        ...
        for res in pool.imap_unordered(_process_one, tasks):
            if res:
                if "__fail__" in res:
                    ...
                    results.append(res)
                    pbar.write(f"  √ {res['代码']} {res['名称']} {res['阶段']} 距低点{res['距低点%']}% 价={res['最新价']}")
...
def build_push(df, cluster, hot, spot_now=None):
    ...
    L = [f"**🪨 底部坑底/蓄势** | 命中{len(df)}只 🎯风口{len(reso)} (全发)",
         "*(🔴/⚠️坑底=左侧极左接飞刀, 29.64事后是底事前未必, 可能续跌; 🟡横盘/🟠止跌=跌完磨底; 非买入保证, 必止损)*", ""]
    if hot:
...
def main():
    ...
    print(f"全扫={'是' if not SCAN_LIMIT else f'限{SCAN_LIMIT}'}; 双源; 多进程{NUM_PROCESSES}; 预筛={'开' if SNAPSHOT_PRE else '关'}; "
          f"低位区≤{BOTTOM_ZONE_PCT*100:.0f}%+回撤≥{DRAWDOWN_MIN*100:.0f}%; 坑底≤{PIT_PCT*100:.0f}%无条件进(保29.64); 不拦交易日; 推送全列+分页")
    print("=" * 70)
    df = run_scan()
    if df is None or df.empty:
        print("\n本次扫描未发现满足 底部坑底/蓄势 的信号。")
        print("可调: BOTTOM_ZONE_PCT调大(放宽低位区)/PIT_PCT调大(放宽坑底)/DRAWDOWN_MIN调小(放宽回撤)")
        sys.exit(0)
    ...
    try:
        ...
        with open(os.path.join(OUTPUT_DIR, f"bottom_acc_{tag}.json"), 'w', encoding='utf-8') as f:
            json.dump({"date": tag, "params": {"BOTTOM_ZONE_PCT": BOTTOM_ZONE_PCT, "PIT_PCT": PIT_PCT,
                                               "DRAWDOWN_MIN": DRAWDOWN_MIN, "RSI_LOW": RSI_LOW},
