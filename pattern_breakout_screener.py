...
def evaluate_worker(result_queue: Any, symbol: str, name: str, cfg_dict: dict[str, Any]) -> None:
    cfg = Config(**{**cfg_dict, "output_dir": Path(cfg_dict["output_dir"])})
    try:
        try:
...
def universe_worker(result_queue: Any, source: str) -> None:
    try:
        if source == "akshare":
            ...
            try: raw = bs.query_stock_basic().get_data().rename(columns={"code": "代码", "code_name": "名称"})
            finally:
                try: bs.logout()
                except Exception: pass
