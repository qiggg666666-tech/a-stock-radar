...
EXPORT_COLUMNS = [
    ...
    "信号评分", "股牛股", "首个粉红柱", "买进", "操纵", "趋势", "中线趋势", "相对强弱%", "数据源",
]
...
def signal_rank(row: dict[str, Any]) -> int:
    ...
    return {"首个粉红柱": 0, "买点": 1, "趋势观察": 2}.get(signal_type, 9)
...
def symbol_worker(result_queue: Any, item: dict[str, Any], benchmark: pd.DataFrame, config_dict: dict[str, Any]) -> None:
    ...
    try:
        ...
        row = {
            ...
            "最新价": round(float(latest["close"]), 2), "信号评分": score, "股牛股": int(latest["股牛股"]), "首个粉红柱": int(latest["首个粉红柱"]), "买进": int(latest["买进"]),
...
def self_test() -> int:
    ...
    ranked = sorted([{"信号类型": "趋势观察", "信号评分": 90}, {"信号类型": "买点", "信号评分": 60}, {"信号类型": "首个粉红柱", "信号评分": 40}], key=lambda row: (signal_rank(row), -int(row["信号评分"])))
    assert [row["信号类型"] for row in ranked] == ["首个粉红柱", "买点", "趋势观察"]
