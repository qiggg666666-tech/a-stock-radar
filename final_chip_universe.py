...
【重要修复】换手率单位换算 bug（_normalize_df 里 t>1.5 才 /100 的旧逻辑）：
绝大多数正常换手率(<1.5这个数字本身)一直被当成小数直接存成 0.5~5 量级，
冲垮当日筹码衰减权重(几乎每天清零)，导致 band_ratio/is_approaching 等一系列
指标失真。已改为固定 /100。此前所有基于本脚本输出做的参数分析(VOL_MULTIPLIER
扫描、is_spike阈值等)在此修复之前跑的，结论需要重新验证。
...
def _normalize_df(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    if "turn" in df.columns:
        # baostock/akshare 的换手率字段本来就是百分比数字（如 1.85 代表1.85%），
        # 统一除以100转成 0~1 小数。之前用 t>1.5 判断条件转换，导致绝大多数正常
        # 换手率(<1.5%这个数字本身，而不是1.5这个换手率百分比)被误当成小数直接
        # 存了185%这种量级，冲垮了后面的当日衰减权重计算（几乎每天筹码记忆清零）。
...
def score_break(feat, df) -> Tuple[float, bool, bool, float, float, bool, float]:
    """返回 (突破分, in_zone, tradeable, ma5, ma10, trend, vol_ratio)。"""
    close = float(df.iloc[-1]["close"])
    mp = feat["main_peak"]
    if mp <= 0:
        return 0.0, False, False, close, close, False, 0.0
    ...
    return round(float(np.clip(sc, 0, 100)), 1), in_zone, tradeable, round(ma5, 4), round(ma10, 4), trend, round(vr, 4)
...
def analyze_symbol(symbol: str, industry: str) -> dict:
    ...
    break_s, is_approaching, is_tradeable, ma5, ma10, trend, vol_ratio = score_break(feat, df)
