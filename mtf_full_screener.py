...
class ScreenerMaster:
    ...
    def load_stock_list(self):
        ...
        if self.cfg.scan_limit and len(codes) > self.cfg.scan_limit: codes = codes[:self.cfg.scan_limit]
    ...
    def run(self):
        self.logger.info("=" * 70)
        self.logger.info(f"增强多周期双共振[{self.dm.cfg_dict['label']}] | {datetime.now():%Y-%m-%d %H:%M} | 回看{self.dm.cfg_dict['lookback_days']}天 | 进程{self.cfg.num_processes}")
        self.logger.info("=" * 70)
