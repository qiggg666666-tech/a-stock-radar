...
【本版·加严默认值】alpha 默认 0.006(在原0.008基础上温和收紧, 形态更标准; 更严可0.004, 别回0.002太严常0命中);
  recent_days 默认 10(原15, 只要近10天突破的更新鲜信号)。两者仍 env 可调(ALPHA/RECENT_DAYS), workflow
  设值可覆盖默认。其余形态参数(min_gap/max_gap/vol_expand/vol_shrink/bb_std/bb_period/lookback)亦全env可调。
  ⚠️ 权衡: 容差越宽命中越多但W底形态越不标准; W底本就稀有, 收紧后也不会爆炸减少。
...
PARAMS = dict(
    ...
    alpha=float(os.environ.get('ALPHA', '0.006')),   # 【本版加严】0.008→0.006(温和收紧, 形态更标准; 更严0.004; 别回0.002太严)
    # 成交量
    ...
    # 扫描
    recent_days=int(os.environ.get('RECENT_DAYS', '10')),   # 【本版加严】15→10(只要近10天突破的更新鲜信号)
...
def main():
    ...
    if df is None or df.empty:
        print("\n本次扫描未发现符合条件的近期W底信号。")
        print("可调: ALPHA 调大(如0.008~0.01) / RECENT_DAYS 调大 / 看失败统计定位瓶颈")
        sys.exit(0)
