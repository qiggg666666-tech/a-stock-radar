...
⚠️ VOL_RATIO=0.85 为"纯度优先"折中(比0.75活、比0.9纯): 缩量仅辅助确认, 方向靠前三重
  (零轴+金叉+底背离); 0.85砍掉量缩比0.85~1的"平量层"(洗盘证据最弱), 信号更纯;
  命中仍作观察池, 等放量金叉右侧确认+轻仓止损。
"""
import os
import re
import sys
...
PARAMS = dict(
    ...
    # 0.85=纯度优先折中(原0.75太严出0, 0.9噪声多); 砍平量层保洗盘证据, 方向仍靠前三重
    VOL_SHORT=5, VOL_LONG=20, VOL_RATIO=0.85,
...
def volume_shrinking(volume: pd.Series, short=5, long=20, ratio=0.85):
    if len(volume) < long:
        return False
...
def run_scan():
    ...
    print(f"开始MACD零轴上+ZigZag底背离扫描 {len(tasks)} 只（{PARAMS['NUM_PROCESSES']}进程, 双源, 阈值{PARAMS['ZIGZAG_THRESHOLD']*100:.0f}%, 缩量比<{PARAMS['VOL_RATIO']}）...")
...
def build_push(df, cluster, hot):
    ...
    L = [f"**📉 MACD零轴上+ZigZag底背离** | 命中{len(df)}只 🎯风口{len(reso)} (全发)",
         "*(零轴上方+即将金叉+已确认底背离+缩量+均线支撑=左侧反转提示; 缩量门槛0.85(适中), 命中作观察池, 等放量金叉确认+轻仓止损)*", ""]
    if hot:
...
def main():
    print("=" * 70)
    print(f"📉 MACD零轴上+ZigZag底背离 | {datetime.now():%Y-%m-%d %H:%M} | 回看{PARAMS['LOOKBACK_DAYS']}天 阈值{PARAMS['ZIGZAG_THRESHOLD']*100:.0f}% 缩量比<{PARAMS['VOL_RATIO']}")
