...
处理"现价下方长红柱"(is_below_spike)与"宽幅堆积区"(is_wide_zone)两条线——
不再计算/排序/推送 is_tradeable 那套"可交易"榜单。全部命中分批推送，避免单条消息过长被截断。

下方长红柱阈值（由内核 final_chip_research 计算，汇总侧只读 is_below_spike）已于
2026-08-30 按金瑞矿业类形态放宽：BELOW_SPIKE_BAND_MIN=0.10, GAP_MIN=1.0, RATIO_MIN=0.003。
"""
