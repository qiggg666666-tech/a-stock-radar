#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""独立DistilledQuant核心任务的共同股票池准备器。

该股票池是运行时快照，不应被描述为历史时点成分池。AkShare不可用时使用
BaoStock ``query_stock_basic()`` 的当前兼容调用；两源都失败时明确退出，不伪造池。
它仅由DistilledQuant核心工作流消费，不与其他研究任务共享artifact。
"""
...
def baostock_universe() -> pd.DataFrame:
    ...
    try:
        ...
        if "status" in raw.columns:
            # query_stock_basic()可返回历史标的；仅保留明确标记为上市中的行。
...
def main() -> int:
    parser = argparse.ArgumentParser(description="独立DistilledQuant核心共同股票池")
    parser.add_argument("--signal-date", default="")
    parser.add_argument("--output", type=Path, default=Path("distilled_quant_core_universe.json"))
    parser.add_argument("--timeout-seconds", type=float, default=45.0)
    parser.add_argument("--retries", type=int, default=2)
    parser.add_argument("--self-test", action="store_true")
    args = parser.parse_args()
    if args.self_test:
        sample = pd.DataFrame({"code": ["000001", "300001", "600001", "830001"], "name": ["甲", "乙", "丙", "退市样本"]})
        assert [row["code"] for row in normalize_universe(sample)] == ["000001", "300001", "600001"]
        status_sample = pd.DataFrame({"code": ["sz.000002", "sh.600002"], "code_name": ["丁", "戊"], "status": ["1", "0"]})
        assert status_sample[status_sample["status"].astype(str) == "1"]["code_name"].tolist() == ["丁"]
        print("SELF_TEST_OK")
        return 0
    ...
    payload = {
        "schema_version": "distilled-quant-core-universe/v1",
        ...
        "universe": universe,
        "disclosure": "仅供独立DistilledQuant核心使用的运行时A股股票池快照；排除ST/退市名称和非沪深创业板前缀，不与其他研究任务共享，不代表历史时点成分池。",
    }
