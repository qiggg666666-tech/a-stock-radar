...
3. 两源暂不可用时可使用不超过限定天数的工作流缓存或仓库种子快照；
4. 所有降级来源均写入状态artifact，过期或缺失时以退出码2结束，禁止伪装为成功。
"""
...
def baostock_response_to_frame(response: Any) -> pd.DataFrame:
    """将BaoStock游标逐行转换为DataFrame，避免其get_data()内部的DataFrame.append兼容性问题。"""
    fields = list(getattr(response, "fields", []) or [])
    if not fields:
        raise UniverseError("BaoStock股票池字段为空")
    rows: list[list[str]] = []
    while response.next():
        row = list(response.get_row_data())
        if len(row) != len(fields):
            raise UniverseError(f"BaoStock行字段数量异常:{len(row)}/{len(fields)}")
        rows.append(row)
    if not rows:
        raise UniverseError("BaoStock股票池查询无数据")
...
def fetch_baostock() -> list[dict[str, str]]:
    ...
    try:
        ...
        return clean_universe(baostock_response_to_frame(response), "baostock")
    finally:
        try:
            bs.logout()
        except Exception:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--seed-file", default="a_share_universe_seed.json", help="可提交到仓库的最后回退股票池快照")
    parser.add_argument("--seed-max-age-days", type=int, default=3)
...
def self_test() -> int:
    ...
    bao_rows = [[f"sh.6{i:05d}", "样本股"] for i in range(1001)] + [["sz.000001", "平安银行"], ["sz.300750", "宁德时代"], ["sz.002888", "*ST样本"]]
    bao_frame = baostock_response_to_frame(FakeBaoStockResponse(bao_rows))
    bao_cleaned = clean_universe(bao_frame, "baostock")
    bao_codes = {item["代码"] for item in bao_cleaned}
    assert "000001" in bao_codes and "300750" in bao_codes
    assert "002888" not in bao_codes
    assert len(bao_cleaned) == 1003
    print("SELF_TEST_OK")
    return 0
...
def main() -> int:
    ...
    if args.self_test:
        return self_test()
    output, status_output, cache_file, seed_file = Path(args.output), Path(args.status_output), Path(args.cache_file), Path(args.seed_file)
    ...
    seeded = load_fresh_cache(seed_file, args.seed_max_age_days)
    if seeded is not None:
        universe, seeded_at = seeded
        payload = {
            "schema_version": SCHEMA_VERSION,
            "generated_at": now_iso(),
            "source": "seed_snapshot",
            "degraded": True,
            "seed_generated_at": seeded_at,
            "universe": universe,
        }
        write_json(output, payload)
        write_json(cache_file, payload)
        ...
        print(f"UNIVERSE_READY source=seed_snapshot size={len(universe)}")
        return 0
