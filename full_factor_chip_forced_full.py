#!/usr/bin/env python3
"""
完整合并版：全市场分片 + HighOpen/CloseLow/VwapClose/M0
+ 强制完整筹码过滤（红筹码占优 ∩ 宽幅堆积区）
+ 横截面回归因子加权 + SVR 回测 + Server酱推送

依赖：
  - 同目录必须有 final_chip_research.py
  - pip install akshare baostock pandas numpy scikit-learn statsmodels matplotlib pyarrow requests

用法：
  python full_factor_chip_forced_full.py --shard 1
  python full_factor_chip_forced_full.py --shard all
...
def apply_full_chip_filter(df: pd.DataFrame, code: str, name: str = "") -> pd.DataFrame:
    """使用完整 analyze 打上 is_red_heavy_chip & is_wide_zone 标签"""
...
def merge_and_backtest() -> None:
    ...
    for i in range(LOOKBACK_MONTHS, len(months) - 1):
        train = dataset[dataset["ym"].isin(months[i - LOOKBACK_MONTHS : i])]
        ...
        selected = test.nlargest(TOP_N, "pred")
...
def main() -> None:
    ...
    parser.add_argument("--shard", type=str, default="1", help="1-8 / all / backtest")
