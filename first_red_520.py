...
def retain_latest_production_signals(candidates: pd.DataFrame, backtest: bool) -> tuple[pd.DataFrame, str | None, int]:
    """实盘只保留全市场最新可用信号日；回测保留完整历史信号序列。"""
...
def scan_market(args: argparse.Namespace, config: Config) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    candidates, latest_signal_date, historical_signal_records = retain_latest_production_signals(candidates, bool(args.backtest))
    stats["historical_signal_records"] = historical_signal_records
    stats["candidate_records"] = int(len(candidates))
    state = {"version": VERSION, "state": "completed" if stop_reason == "completed" else stop_reason, "finished_at": now_text(), "shard": shard, "offset": args.offset, "limit": args.limit, "backtest": bool(args.backtest), "hold_days": args.hold_days if args.backtest else None, "latest_signal_date": latest_signal_date, "universe": universe_meta, "stats": stats, "candidate_count": int(len(candidates)), "coverage_complete": len(completed.intersection({t['code'] for t in tasks})) == len(tasks), "config": asdict(config), "stop_reason": stop_reason, "elapsed_seconds": round(time.monotonic() - started, 2)}
...
def run_self_test() -> None:
    ...
    filtered, latest_date, historical_count = retain_latest_production_signals(production_candidates, False)
    assert latest_date == "2026-08-17" and historical_count == 3 and list(filtered["code"]) == ["000002", "000003"]
    backtest_candidates, backtest_date, _ = retain_latest_production_signals(production_candidates, True)
