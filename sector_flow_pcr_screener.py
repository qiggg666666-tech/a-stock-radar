...
【本版·修两次崩+隐藏雷·整条外部调用链全保护】
  ① 期权接口 option_daily_stats_sse 限流返回空/非JSON 致 r.json() 抛 JSONDecodeError 拖死脚本
     -> get_pcr_emotion 的 for 循环内 try/except 隔离, 单天炸跳下一天, 全炸走中性不崩;
  ② _call_with_timeout 误把 label= 当 kw 透传给 akshare fn(fn 不接受 label) -> TypeError, 一旦期权
     不崩就会在 get_sector_flow/hist/members 连环崩(隐藏第二雷) -> _call_with_timeout 内 pop 掉 label;
  ③ get_sector_flow/hist/members 的 _call_with_timeout 调用补 try/except, 限流时降级(板块流空跳过/
     板块特征0/成分股空), 不再拖死。任一接口限流/炸JSON 均降级不崩, 不再"改一个崩下一个"。
...
def _call_with_timeout(fn, *a, timeout=AK_TIMEOUT, **kw):
    # 【修】label 仅供调用者标注日志意图, akshare fn 不接受 label; 不透传否则 TypeError
    # (隐藏第二雷: 期权不崩时会在 get_sector_flow/hist/members 连环 TypeError 崩)
    kw.pop('label', None)
    with cf.ThreadPoolExecutor(max_workers=1) as ex:
...
def get_pcr_emotion():
    ...
    for back in range(0, 7):
        ...
        # 【修】期权接口限流返回空/非JSON 会抛 JSONDecodeError 等, 单天炸跳下一天, 不拖死
        try:
            df = _call_with_timeout(ak.option_daily_stats_sse, date=d, timeout=AK_TIMEOUT)
        except Exception as e:
            print(f"  期权PCR: 取{d}异常({type(e).__name__}), 试下一天")
            df = None
            continue
...
def get_sector_flow():
    ...
    for fn in attempts:
        # 【修】某接口限流/异常时试下一个 lambda, 不拖死(label 透传已在 _call_with_timeout 内 pop)
        try:
            df = _call_with_timeout(fn, label="sector_flow")
        except Exception as e:
            print(f"  板块资金流: 某接口异常({type(e).__name__}), 试下一个")
            continue
        if df is not None and not df.empty:
            return df.copy()
    return pd.DataFrame()
