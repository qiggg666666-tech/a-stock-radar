...
SHARDS = ("a", "b", "c", "d")
FINAL_COLUMNS = ["代码", "名称", "信号日期", "信号类型", "流通市值(亿)", "当日成交额(亿)", "最新价", "信号评分", "股牛股", "首个粉红柱", "买进", "操纵", "趋势", "中线趋势", "相对强弱%", "数据源"]
...
def signal_rank(value: Any) -> int:
    """首个粉红柱优先，其次买点，再次趋势观察。"""
    return {"首个粉红柱": 0, "买点": 1, "趋势观察": 2}.get(str(value or ""), 9)
...
def main() -> int:
    ...
    priority_events = pd.concat([first_pink_bars, buy_points], ignore_index=True).drop_duplicates("代码", keep="first") if not observations.empty else pd.DataFrame(columns=FINAL_COLUMNS)
    ...
    notification = send_serverchan(
        args.sendkey,
        f"吃掉主力·{args.run_label}：{state} | 首粉{len(first_pink_bars)} | 可发布{len(publishable)}条",
        "# 吃掉主力研究：每日全市场汇总\n\n"
