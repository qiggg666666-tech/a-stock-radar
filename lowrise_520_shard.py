...
DEFAULT_PRIORITY_DISTANCE_PCT = 5.0
...
def provider_call(label: str, timeout: int, function: Callable[[], Any]) -> Any:
    ...
    try:
        if not parent.poll(timeout):
            ...
            # 子进程变僵尸继续占用连接。改用SIGKILL(kill)保证一定能杀死，跟
            # final_chip_universe.py/dinghai_universe.py/lowrise_520_universe.py
            # 里同一个真实事故修复口径一致。
            process.kill(); process.join(3); raise TimeoutError(f"provider_timeout:{label}:{timeout}s")
...
def observation(frame: pd.DataFrame, signal: pd.Timestamp, max_stale: int, priority_distance_pct: float) -> dict[str, Any]:
    ...
    is_priority = bool(distance < priority_distance_pct)
    return {"data_last_date": last.strftime("%Y-%m-%d"), "low_520":low520, "close":float(current["close"]), "distance_to_520_low_pct":distance, "volume_ratio_previous_5":vr, "first_red":first_red, "above_ma5":bool(current["close"]>ma5), "distance_priority_group":"priority_under_5pct" if is_priority else "standard_5pct_or_more", "lowrise_observation":bool(first_red and current["close"]>ma5)}


def write_skip_status(output_dir: Path, signal_date: str, shard_index: int, shard_count: int, universe_status: object) -> dict[str, Any]:
    """跟dinghai_shard.py/final_chip_shard.py同一套优雅跳过：universe源(含缓存兜底)
    都不可用时，这个分片不应该报错崩溃——写一份空的、状态明确的产物，让summary能
    看出当天没有数据可用，而不是让整条workflow红掉。"""
    output_dir.mkdir(parents=True, exist_ok=True)
    status = {
        "schema_version": "lowrise-520-shard/v2",
        "status": "skipped:universe_unavailable",
    ...
    pd.DataFrame().to_csv(output_dir / "raw_records.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(columns=["code", "name", "stage", "error_type", "error_message", "attempts"]).to_csv(
...
def main() -> int:
    ...
    parser.add_argument("--universe-file",type=Path); parser.add_argument("--signal-date"); parser.add_argument("--shard-index",type=int,default=0); parser.add_argument("--shard-count",type=int,default=4); parser.add_argument("--start-date",default="2023-01-01"); parser.add_argument("--priority-distance-pct",type=float,default=DEFAULT_PRIORITY_DISTANCE_PCT,help="仅用于排序置顶，不作为候选硬过滤"); parser.add_argument("--timeout-seconds",type=int,default=35); parser.add_argument("--retries",type=int,default=2); parser.add_argument("--max-stale-days",type=int,default=3); parser.add_argument("--output-dir",type=Path,default=Path("lowrise_shard_output")); parser.add_argument("--self-test",action="store_true")
    args=parser.parse_args()
    if args.self_test:
        items=[str(i) for i in range(16)]; assert len([x for n in range(4) for x in items[n::4]])==16
        # 优雅跳过回归：universe状态不是ready/degraded_cache时必须走跳过分支，不能
        # 尝试对空/无效universe发起扫描。
        for bad_status in (None, "universe_unavailable", "something_else"):
            assert bad_status not in {"ready", "degraded_cache"}
        print("SELF_TEST_OK: lowrise_shard_partition"); return 0
    if not args.universe_file or not args.signal_date: parser.error("--universe-file and --signal-date are required")
    signal=pd.Timestamp(args.signal_date).normalize(); payload=json.loads(args.universe_file.read_text(encoding="utf-8"))

    universe_status = payload.get("status")
    if universe_status not in {"ready", "degraded_cache"}:
        status = write_skip_status(args.output_dir, args.signal_date, args.shard_index, args.shard_count, universe_status)
        print(json.dumps(status, ensure_ascii=False))
        return 0

    universe=sorted(payload.get("universe",[]),key=lambda x: str(x["code"])); queue=universe[args.shard_index::args.shard_count]
    ...
    for item in queue:
        ...
        try:
            records.append({"code":code,"name":str(item.get("name","")),"daily_data_source":source,**observation(frame,signal,args.max_stale_days,args.priority_distance_pct)}); sources[source]+=1
    ...
    status={"schema_version":"lowrise-520-shard/v2","status":"ready" if not errors else "degraded","signal_date_requested":args.signal_date,"shard_index":args.shard_index,"shard_count":args.shard_count,"scan_queue_count":len(queue),"valid_record_count":len(records),"error_count":len(errors),"daily_data_source_counts":sources,"universe_status":universe_status}
