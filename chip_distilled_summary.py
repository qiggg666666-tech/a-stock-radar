...
def main() -> int:
    ...
    report = ['# 筹码尖峰蒸馏汇总', '', f'- 完成分片：{len(completed)}/{args.shard_total}', f'- 有效记录：{len(records)}', f'- 数据错误：{len(errors)}', f'- 排序记录（artifact）：{len(candidates)}', f'- 推送展示：{len(notification_preview)}（最多50条）', '', '## 推送预览（最多50条）']
