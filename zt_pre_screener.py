...
【本版完善】
  ...
  4. 【空跑也推送】原逻辑空结果时(快照+兜底均空/逐只全失败/候选池空)直接 return 不推送,
     致定时或手动空跑时微信"石沉大海", 误以为没跑。现三处空跑出口均推一条"空跑通知",
     说明原因(多限流/门槛严, 非脚本故障); 有货时推送不变。手动(workflow_dispatch)与定时
     空跑皆可见, 满足"手动跑也能收到消息"。
...
def _push_empty(reason):
    """空结果时推一条"空跑通知", 让定时/手动空跑不再静默(否则微信收不到, 误以为没跑)。
    无 SERVERCHAN_KEY 时静默不推; 推送本身包防护, 失败不影响主流程。"""
    if not SERVERCHAN_KEY:
        return
    try:
        msg = (f"**⚠️ 涨停候选池 · 空跑** | {_bj_now():%m-%d %H:%M}\n\n"
               f"**原因**: {reason}\n\n"
               f"*(多为东财限流致快照/兜底无数据, 或当日形态门槛无命中, **非脚本故障**。"
               f"交易日错峰单跑通常恢复; 用 Actions 的 `Run workflow` 按钮手动重跑亦可验证。)*")
        send_serverchan(f"涨停候选池 {_bj_now():%m-%d} | ⚠️空跑", msg)
    except Exception as e:
...
def run_candidate():
    ...
    if not cands:
        ...
        _push_empty("快照与兜底均无候选(东财限流或 FALLBACK_FULL=0); 交易日错峰或手动重跑通常恢复")  # 【本版】空跑也推
        return pd.DataFrame()
    ...
    if not rows:
        print("⚠️ 无有效日线, 候选池为空")
        _push_empty("有候选但逐只拉日线全失败/无有效形态命中(限流或门槛严)")  # 【本版】空跑也推
        return pd.DataFrame()
    ...
    # 【本版】推送: 有货推有货; 空(pool为空)也推空跑通知, 不再静默
    if SERVERCHAN_KEY:
        if not pool.empty:
            ...
            _push_empty(f"有效日线{len(df)}只, 但形态打分排序后候选池为空(门槛严)")  # 【本版】空跑也推
