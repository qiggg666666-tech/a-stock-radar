...
def main() -> int:
    ...
    parser.add_argument("--intraday-retries", type=int, default=2, help="首红候选二次确认的重试次数")
    parser.add_argument("--skip-intraday-confirm", action="store_true", help="跳过首红候选的盘中二次确认，只保留日线 is_first_red_daily")
    parser.add_argument("--intraday-request-interval", type=float, default=1.2, help="连续两次盘中确认调用之间的等待秒数，避免打得太密被接口限流/断连")
    ...
    for position, item in enumerate(selected, 1):
        ...
        try:
            ...
            if row.get("is_first_red_daily") and not args.skip_intraday_confirm:
                if intraday_confirm_count > 0: time.sleep(args.intraday_request_interval)
