...
def main():
    print("=" * 70)
    mode = "年线+季线" if SCAN_BOTH else _ma_name(MA_PERIOD)
    print(f"📈 趋势启动确认 | {datetime.now():%Y-%m-%d %H:%M} | FRESH_ONLY={'开' if FRESH_ONLY else '关'} 复核={'开' if REALTIME_RECHECK else '关'} | Offset={SCAN_OFFSET} Limit={SCAN_LIMIT}")
    print("=" * 70)
    df = run_scan()
    if df is None or df.empty:
