...
def process_task(payload: tuple[dict[str, Any], bool, int]) -> dict[str, Any]:
    ...
    base["data_last_date"] = data_last_date
    try:
...
def retain_production_signals(candidates: pd.DataFrame, backtest: bool, only_latest_signal_date: bool, live_only: bool) -> tuple[pd.DataFrame, str | None, int]:
    """实盘默认只保留信号日等于最后可用日线的记录，杜绝历史信号混入当日名单。"""
    historical_signal_records = int(len(candidates))
    if backtest or candidates.empty:
        return candidates, None, historical_signal_records
    dates = pd.to_datetime(candidates["signal_date"], errors="coerce")
    if dates.isna().all():
        raise ValueError("生产候选缺少可解析的signal_date")
    if live_only:
        data_dates = pd.to_datetime(candidates.get("data_last_date"), errors="coerce")
        if data_dates.isna().all():
            raise ValueError("生产候选缺少可解析的data_last_date")
        asof_date = data_dates.max().strftime("%Y-%m-%d")
        filtered = candidates.loc[
            (dates.dt.strftime("%Y-%m-%d") == data_dates.dt.strftime("%Y-%m-%d"))
            & (data_dates.dt.strftime("%Y-%m-%d") == asof_date)
        ].reset_index(drop=True)
        return filtered, asof_date, historical_signal_records
    latest_date = dates.max().strftime("%Y-%m-%d")
    if not only_latest_signal_date:
        return candidates.reset_index(drop=True), latest_date, historical_signal_records
    filtered = candidates.loc[dates.dt.strftime("%Y-%m-%d") == latest_date].reset_index(drop=True)
    return filtered, latest_date, historical_signal_records
...
def scan_market(args: argparse.Namespace, config: Config) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    stats = {"universe": len(universe), "slice": len(tasks), "resumed_completed": len(completed.intersection({t['code'] for t in tasks})), "processed": 0, "candidate_records": 0, "historical_signal_records": 0, "source_error": 0, "logic_error": 0, "timeout": 0, "no_signal": 0}
    records: list[dict[str, Any]] = []
    ...
    candidates, latest_signal_date, historical_signal_records = retain_production_signals(candidates, bool(args.backtest), bool(args.only_latest_signal_date), bool(args.live_only))
    stats["historical_signal_records"] = historical_signal_records
    stats["candidate_records"] = int(len(candidates))
    state = {"version": VERSION, "state": "completed" if stop_reason == "completed" else stop_reason, "finished_at": now_text(), "shard": shard, "offset": args.offset, "limit": args.limit, "backtest": bool(args.backtest), "hold_days": args.hold_days if args.backtest else None, "signal_scope": "same_day_only" if args.live_only else "latest_signal_date_only" if args.only_latest_signal_date else "all_detected", "latest_signal_date": latest_signal_date, "max_data_last_date": max(data_dates) if data_dates else None, "data_last_date_counts": dict(sorted(data_dates.items())), "universe": universe_meta, "stats": stats, "candidate_count": int(len(candidates)), "coverage_complete": len(completed.intersection({t['code'] for t in tasks})) == len(tasks), "config": asdict(config), "stop_reason": stop_reason, "elapsed_seconds": round(time.monotonic() - started, 2)}
...
def write_outputs(output: Path, shard: str, candidates: pd.DataFrame, state: dict[str, Any]) -> dict[str, Path]:
    ...
    lines = [f"# 520天首红扫描：分片 {shard.upper()}", "", f"- 完成时间：{state['finished_at']}", f"- 股票池：{state['universe']['count']}只（{state['universe']['source']}）", f"- 分片处理：{state['stats']['processed']} / {state['stats']['slice']}", f"- 候选记录：{state['candidate_count']}", f"- 检测范围：{'仅最新信号日' if state.get('signal_scope') == 'latest_signal_date_only' else '全部检测结果'}", f"- 最新检测信号日：{state.get('latest_signal_date') or '回测模式：保留完整历史序列'}", f"- 原始历史信号：{state['stats'].get('historical_signal_records', state['candidate_count'])}", f"- 双源失败：{state['stats']['source_error']}；超时：{state['stats']['timeout']}；逻辑错误：{state['stats']['logic_error']}", "", "> 自动化筛选记录，不构成投资建议。", ""]
...
def run_self_test() -> None:
    ...
    production_candidates = pd.DataFrame({"signal_date": ["2026-08-16", "2026-08-17", "2026-08-17"], "code": ["000001", "000002", "000003"]})
    production_candidates["data_last_date"] = ["2026-08-17", "2026-08-17", "2026-08-17"]
    all_detected, latest_date, historical_count = retain_production_signals(production_candidates, False, False, False)
    assert latest_date == "2026-08-17" and historical_count == 3 and list(all_detected["code"]) == ["000001", "000002", "000003"]
    filtered, _, _ = retain_production_signals(production_candidates, False, True, False)
    assert list(filtered["code"]) == ["000002", "000003"]
    same_day, same_day_date, _ = retain_production_signals(production_candidates, False, False, True)
    assert same_day_date == "2026-08-17" and list(same_day["code"]) == ["000002", "000003"]
    backtest_candidates, backtest_date, _ = retain_production_signals(production_candidates, True, False, True)
    assert backtest_date is None and len(backtest_candidates) == 3
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--only-latest-signal-date", action="store_true", help="仅在明确需要时裁剪为最新信号日；默认保留全部检测结果")
    parser.add_argument("--live-only", action=argparse.BooleanOptionalAction, default=True, help="实盘仅保留信号日等于最后可用日线的记录；默认启用")
