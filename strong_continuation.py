...
# 【v4新增】防"全扫撞timeout白干"两道闸:
#  SOFT_CAP    : 预筛失败(限流)时, 不退化全扫4996, 截断到此数(强势票成交额大, 基本不漏)
#  SOFT_TIMEOUT: 扫描软超时(秒), 到点主动收工正常出结果(保绿勾), 必须 < workflow的timeout(60min)
SOFT_CAP = int(os.environ.get('SOFT_CAP', '2500'))
SOFT_TIMEOUT = int(os.environ.get('SOFT_TIMEOUT', '2700'))   # 45分钟, 留15分钟给enrich+存盘+推送

SNAPSHOT_PRE = os.environ.get('SNAPSHOT_PRE', '1').strip() in ('1', 'true', 'True')
PRE_AMOUNT_MIN = float(os.environ.get('PRE_AMOUNT_MIN', str(1.0e8)))
PRE_60D_MIN = float(os.environ.get('PRE_60D_MIN', '-15'))
...
def is_trading_day():
    """保留备用; 【v4】main 不再调用, 即不限交易日(周末用上一交易日数据复盘强势票)"""
    try:
...
def _init_worker():
    atexit.unregister(_save_partial)   # 抢救只在主进程做, 子进程退出勿触发
...
# ------------------ 快照宽松预筛 (v4: 失败/空时截断兜底, 不退化全扫) ------------------
def _snapshot_prefilter(codes_with_prefix):
    """预筛成功=按快照真实筛选(不截断); 预筛失败/快照空(限流)=截断到 SOFT_CAP 兜底,
    防全扫4996撞timeout白干。强势票成交额大, 截断基本不漏(限流时按baostock原序, 偏沪市属次优)。"""
    if not SNAPSHOT_PRE:
        print(f"  快照预筛: 关闭(SNAPSHOT_PRE=0), 全扫{len(codes_with_prefix)}只(软超时{SOFT_TIMEOUT//60}分钟兜底)")
        return codes_with_prefix
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            d = ex.submit(ak.stock_zh_a_spot_em).result(timeout=25)
        if d is None or d.empty or '代码' not in d.columns:
            print(f"  快照预筛: 快照空(限流), 截断兜底{SOFT_CAP}只(按原序, 防全扫撞timeout)")
            return codes_with_prefix[:SOFT_CAP]
        d['代码'] = d['代码'].astype(str).str.zfill(6)
        for c in ['最新价', '成交额']:
            if c in d.columns:
                d[c] = pd.to_numeric(d[c], errors='coerce')
        col60 = next((c for c in d.columns if '60日' in c), None)
        if col60:
        ...
        if out:
            return out
        print(f"  快照预筛: 条件砍空, 截断兜底{SOFT_CAP}只")
        return codes_with_prefix[:SOFT_CAP]
    except Exception as e:
        print(f"  快照预筛失败(限流), 截断兜底{SOFT_CAP}只(按原序, 防全扫撞timeout): {e}")
        return codes_with_prefix[:SOFT_CAP]
...
def run_strong_continuation_scan(limit=SCAN_LIMIT):
    ...
    codes = _snapshot_prefilter(codes)   # v4: 预筛失败时截断兜底, 不退化全扫
    ...
    results = []
    fail_count = 0
    print(f"开始检测 {len(tasks)} 只股票（{NUM_PROCESSES} 个进程并行, K线=baostock+东财双源, 软超时{SOFT_TIMEOUT//60}分钟）...")
        ...
        for res in pool.imap_unordered(_process_one, tasks):
            if res:
                if "__error__" in res:
                    ...
                    results.append(res)
                    _PARTIAL["results"].append(res)
                    pbar.write(f"✅ 命中: {res['代码']} {res['名称']}（近20日涨幅{res['近20日涨幅%']}%）")
            pbar.update(1)
            pbar.set_postfix(命中=len(results), 失败=fail_count)
            # 【v4主防线】软超时到点主动收工, 已命中正常出结果(保绿勾), 不再撞job timeout白干
            if time.time() - _scan_start > SOFT_TIMEOUT:
                print(f"\n⏱️ 软超时{SOFT_TIMEOUT//60}分钟到, 主动收工(已扫{pbar.n}/{len(tasks)}), "
                      f"已命中{len(results)}只 → 正常出结果(保绿勾)")
                break
...
if __name__ == "__main__":
    ...
    print(f"【v4】快照预筛={'开' if SNAPSHOT_PRE else '关'} | 回看{LOOKBACK_DAYS}天 | 软超时{SOFT_TIMEOUT//60}分钟主动收工 | 不拦交易日 | 超时抢救兜底")
    print("=" * 70)

    # 【v4】不再拦交易日: 周末用上一交易日数据复盘强势票(原 is_trading_day 拦截已移除)
    ...
    if df is not None and not df.empty:
        ...
        _PARTIAL["saved"] = True   # 正常保存完成, 关闭抢救(防 atexit 重复存 PARTIAL)
        print(f"\n结果已保存: {csv_path}")
