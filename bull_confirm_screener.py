...
====================================================================
定位(与 strong/mtf 不重复): 抓"多头确认刚发生"的票 = 今天/近期多个指标【同时发出翻多拐点】。
  strong_continuation = 已强(领涨逼新高); mtf = 将强未确认(多周期临界);
  本脚本 = 刚翻多(拐点事件刚发生)。故打分【拐点事件高权重+状态低权重】,
  且入选硬要求【至少1个拐点事件】, 排除"已多头很久但无新信号"(那是strong)与"还没翻"(那是mtf)。

信号(日线单周期体检): 均线(站上200/MA50>MA200/金叉/刚突破200) + MACD(金叉/柱转正/DIF>0)
  + RSI(>50/上穿50/超买超卖) + 量(放量/缩量/量比) -> 多头得分 + 综合倾向 + 拐点事件数。
  拐点事件: 刚突破200日线/均线金叉/MACD金叉/MACD柱由负转正/RSI上穿50。

【矩阵规格】双源baostock+东财+硬超时; baostock多进程命门已修(_bs_logout无条件重置+
  _init_worker无条件清零再登录); 快照预筛砍量(右侧确认票活跃, 宽松预筛不用涨幅过滤);
  行业join+板块聚类+风口共振🎯; 推送软导入; 收尾三段防护+sys.exit(0); 不拦交易日
  (应用户"取消非交易日不出结果"诉求, 周末可用周五数据复盘)。
⚠️ 右侧确认≠买入保证; 翻多后仍可能假突破, 需结合量能/止损。csv 存全信号字段供细看。
====================================================================
"""
import os
import re
...
PARAMS = dict(
    ...
    # 入选门槛: 多头得分≥SCORE_MIN 且 拐点事件数≥EVENT_MIN( niche 核心, 排除"已强无新信号")
    SCORE_MIN=6.0, EVENT_MIN=1, STRONG_MIN=8.0,
    # 通用/初筛
    KEEP_PREFIX=("0", "3", "6"), EXCLUDE_NAME=("ST", "退"), MIN_PRICE=3.0,
    PRE_AMOUNT_MIN=1.0e8, PRE_TURNOVER_MIN=1.0,
...
def compute_signal(df):
    """返回 命中dict 或 None(数据不足/未达门槛)。打分=拐点事件高权重+状态低权重。"""
    ...
    d['RSI'] = 100 - 100 / (1 + ag / al.replace(0, 1e-9))   # 除零防护
    ...
    dif = _f(L['DIF']); dea = _f(L['DEA']); macd = _f(L['MACD']); rsi = _f(L['RSI'])
    vr = _f(L['量比'])
    if close is None or ma200 is None:
        return None
    p_close = _f(P['close']); p_ma50 = _f(P['MA200']); p_ma200 = _f(P['MA200'])
    ...
    vol_strong = (vr is not None and vr >= 2.0)
    vol_shrink = (vr is not None and vr <= 0.7)
    ...
    if score < PARAMS["SCORE_MIN"] or n_events < PARAMS["EVENT_MIN"]:
        return None   # 未达门槛 或 无拐点事件(排除"已强无新信号")
    if score >= PARAMS["STRONG_MIN"]:
        level = "强多确认"
    elif score >= PARAMS["SCORE_MIN"]:
        level = "偏多确认"
    else:
        level = "中性"
    ...
    return {"代码": None, "名称": None, "行业": "", "最新价": round(close, 2),
            ...
            "MA200": round(ma200, 2), "DIF": round(dif, 4) if dif is not None else None,
            "DEA": round(dea, 4) if dea is not None else None,
            "MACD柱": round(macd, 4) if macd is not None else None,
            "RSI": round(rsi, 2) if rsi is not None else None,
            "量比": round(vr, 2) if vr is not None else None,
            "多头得分": round(score, 1), "综合倾向": level, "拐点数": n_events,
            "tags": tags,
            "刚突破200": just_break200, "均线金叉": ma_golden, "MACD金叉": macd_golden,
            "MACD转正": hist_pos, "RSI上穿50": rsi_cross50, "站上200": above200,
            "放量": vol_up, "缩量": vol_shrink,
...
def build_push(df, cluster, hot):
    ...
    L = [f"**🚀 多指标共振翻多·右侧确认** | 命中{len(df)}只 🎯风口{len(reso)} 强多{len(strong)}",
         "*(抓'多头确认刚发生': 多指标同时翻多拐点; 与strong已强/mtf将强不重叠; 右侧确认≠买入保证, 防假突破)*", ""]
    if hot:
        L.append("🌪️ **风口**: " + "、".join(f"{n}({c}%)" for n, c in hot[:6]))
        L.append("")
    if cluster:
        L.append("🚀 **翻多确认板块**: " + "、".join(f"{n}({c})" for n, c in cluster))
        L.append("")
    def line(r):
        return (f"- **{r['名称']}({r['代码']})** [{_sec_tag(r.to_dict())}] 现价{r['最新价']} 分{r['多头得分']} "
                f"RSI{r['RSI']} 量比{r['量比']} | {'·'.join(r['tags']) or '-'}")
    if not reso.empty:
        L.append(f"### 🎯 翻多遇风口 Top{min(len(reso), P)} (翻多确认+板块催化)")
        L += [line(r) for _, r in reso.head(P).iterrows()]
        L.append("")
    L.append(f"### 🚀 强多确认 Top{min(len(strong), P)} (拐点事件多+得分高)")
    L += [line(r) for _, r in strong.head(P).iterrows()] or ["今日无强多确认"]
    L.append("")
    if len(df) > len(strong):
        rest = df[df['综合倾向'] != '强多确认']
        L.append(f"### 📋 偏多确认 Top{min(len(rest), 6)}")
        L += [line(r) for _, r in rest.head(6).iterrows()]
    if len(df) > P:
        L.append(f"\n*…另有{len(df)-P}只, 见output(含全信号字段)*")
    return "\n".join(L)
...
def main():
    ...
    print(f"全扫={'是' if not SCAN_LIMIT else f'限{SCAN_LIMIT}'}; 门槛 得分≥{PARAMS['SCORE_MIN']}+拐点≥{PARAMS['EVENT_MIN']}; 不拦交易日")
    print("=" * 70)
    ...
    try:
        ...
        disp = disp.drop(columns=['行业', 'resonance', 'resonance_sector', 'tags',
                                  '刚突破200', '均线金叉', 'MACD金叉', 'MACD转正', 'RSI上穿50',
                                  '站上200', '放量', '缩量', 'MA50', 'MA200', 'DIF', 'DEA', 'MACD柱'], errors='ignore')
