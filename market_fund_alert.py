...
【本版·修北向监控静默空转(假绿勾)】akshare 新版 stock_hsgt_fund_flow_summary_em 的"类型"列
  返回'沪港通'/'深港通'(即沪股通/深股通的北向通道总称), 原匹配模式'沪股通|深股通|北向'匹配0行
  -> north_rows 空 -> "本次不做阈值判断" -> 北向监控空转(脚本仍绿勾, 但北向这条腿没干活)。
  修: 匹配模式扩充为'沪股通|深股通|北向|沪港通|深港通'; 南向'港股通'不含这些子串自然排除;
  匹配成功时打印实际匹配取值供核对; 沿用原 json 快照(north.net_raw/net_yi)显式化口径不确定性。
  ⚠️ 沪港通/深港通行的净买额口径(是否纯北向)第一次跑后请核对 json 的 north.net_raw 与日志。
"""
...
def check_north_flow():
    ...
    try:
        ...
        # 【修北向空转】akshare 新版"类型"列返回'沪港通'/'深港通'(沪股通/深股通的北向通道总称),
        # 原模式'沪股通|深股通|北向'匹配0行致空转; 扩充含'沪港通|深港通'。
        # 南向'港股通'不含这些子串, 自然排除不会混入。
        north_pat = '沪股通|深股通|北向|沪港通|深港通'
        north_rows = df[df[id_col].astype(str).str.contains(north_pat, na=False)]
        snapshot["matched_rows"] = len(north_rows)
        if north_rows.empty:
            uniq = df[id_col].astype(str).unique().tolist()
            print(f"⚠️ 在 {id_col} 列中未匹配到沪股通/深股通/北向/沪港通/深港通相关行，本次不做阈值判断")
            print(f"   该列现有取值(供核对): {uniq}")
            return alerts, snapshot
        matched_types = north_rows[id_col].astype(str).unique().tolist()
        print(f"  北向匹配到的类型取值: {matched_types} (沪港通/深港通=北向通道, 港股通南向已排除; 请核对json的north.net_raw确认口径/单位)")
