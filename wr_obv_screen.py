...
【本版新增·防接飞刀(ANTI_KNIFE, 默认开)】
  ① 企稳K线: 阳线 或 下影线占比≥KNIFE_LOWER_SHADOW(买盘承接), 排除光头大阴线(自由落体);
  ② 非崩盘: 近5日累计跌幅≥KNIFE_MAX_5D_DROP(更负=更宽松), 排除连续跌停式崩盘;
  ③ 非跌停: 当日不接近跌停(买不进且仍在暴跌)。
  被剔票计入"防飞刀过滤", 日志可见; ANTI_KNIFE=0 全关。
【矩阵化】output/存盘+baostock东财双源(preclose改close.shift(1)统一)+query_stock_basic股票池+
  快照预筛+行业本地join+风口🎯+推送serverchan_sdk/requests双通道分页+失败统计+收尾防护+append补丁+参数env可调。
【信号逻辑·忠于原Pine】bull_cross/bull_reverse/OBV多头/涨停排除 一字未动, 仅加排序分。
...
KNIFE_LOWER_SHADOW = float(os.environ.get('KNIFE_LOWER_SHADOW', '0.3'))   # 非阳线时, 下影线占振幅最低比例
KNIFE_MAX_5D_DROP = float(os.environ.get('KNIFE_MAX_5D_DROP', '-0.28'))   # 近5日最大允许跌幅(更负=更宽松)
PARAMS = dict(
...
def compute_signals(df, limit_pct):
    """df 按日期升序, 含 open/high/low/close/volume。触发返回 dict, 否则 None / {"__knife__":...}。"""
    ...
    if ANTI_KNIFE:
        ...
        # ① 企稳K线: 阳线 或 下影线占比≥阈值(买盘承接), 排除光头大阴线(自由落体)
...
def main():
    ...
    print(f"WR({WR_LENGTH},{WR_SMOOTH},{WR_SLOW}) OBV{OBV_LENGTH} 反转阈值{REV_THRESHOLD}; 防飞刀={'开' if ANTI_KNIFE else '关'}")
    print("=" * 70)
    df = run_scan()
