#!/usr/bin/env python3
"""FINAL Chip 洗盘 + 首红确认 + 红筹码占优 + 长影线 + 均线强多 汇总器。
...
5. 均线强多：ma_signal以"强多"开头、但没有命中以上任一专区的股票(比如靠均线金叉+放量涨停，
   跟筹码形态/长影线无关)，按ma_score从高到低排序，让这类此前只体现在signal字段里、
   四个专区都捞不到的股票也有地方推送。
五类互不排斥，同一只股票可能同时命中多类。每类各自全部命中分批推送。
...
def main() -> int:
    ...
    if not completed:
        result = {
            "schema_version": "final-chip-summary/v8-ma-strong",
    ...
    # 直接判断四个已有信号本身，而不是读signal字段——signal内部判断"高潜力"还是
    # "均线强多"用的是is_wide_zone/is_chip_spike/is_below_spike/in_zone，跟这四个
    # 真正的推送专区口径不完全一样，直接对着四个专区的判定条件取反才准确。
    ...
    lines = [
        "# FINAL Chip 汇总（洗盘 + 首红确认 + 红筹码占优 + 长影线 + 均线强多）",
        "",
    ...
    notifications: dict[str, list[dict[str, object]]] = {
        "washout": [], "first_red_confirmed": [], "red_heavy": [], "long_shadow": [], "ma_strong": [],
    }
    if args.notify:
    ...
    result = {
        ...
        "disclosure": (
            ...
            "长影线(has_long_shadow)、均线强多(ma_signal以'强多'开头且未落入以上任一专区) 五类股票，"
            "互不排斥；通知失败不阻断 artifact。"
        ),
    }
    (args.output_dir / "final_chip_summary.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8"
