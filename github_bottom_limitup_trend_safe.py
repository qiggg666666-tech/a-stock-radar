...
def epoch_to_cn(value: Any) -> Optional[datetime]:
    """兼容东方财富秒级/毫秒级 f124 更新时间字段。"""
    epoch = finite_or_none(value)
    if epoch is None or epoch <= 0: return None
    if epoch > 10_000_000_000: epoch /= 1000
    try: return datetime.fromtimestamp(epoch, tz=timezone.utc).astimezone(CN_TZ)
...
def get_fund_rank(cfg: Config, cache: JsonCache) -> list[dict[str, Any]]:
    ...
    for row in raw_rows:
        ...
        result.append({
            ...
            "updated_at": epoch_to_cn(row.get("f124")),
        })
    if not result: raise DataSourceError("资金流排行清洗后为空")
    return result

def verify_fund_freshness(funds: list[dict[str, Any]], cfg: Config, now: datetime) -> datetime:
    """将盘前、休市或接口残留的上一交易日资金流降级为正常跳过。"""
    timestamps = [value for value in (item.get("updated_at") for item in funds) if isinstance(value, datetime)]
    if not timestamps:
        raise DataSourceError("资金流数据缺少更新时间(f124)，无法确认是否为当日数据")
    newest = max(timestamps)
    age_minutes = (now - newest).total_seconds() / 60
    if newest.date() != now.date():
        raise DataSourceError(f"资金流非当日数据：最新{newest:%Y-%m-%d %H:%M}，当前{now:%Y-%m-%d %H:%M}")
    if age_minutes < -10 or age_minutes > cfg.max_stale_minutes:
        raise DataSourceError(f"资金流数据不新鲜：最新{newest:%H:%M}，延迟{age_minutes:.0f}分钟，阈值{cfg.max_stale_minutes}分钟")
    return newest
...
def write_status_reports(cfg: Config, started: datetime, status: str, reason: str) -> dict[str, Path]:
    """无数据/数据未就绪时仍输出同名三类产物，供Actions artifact与日报安全读取。"""
    cfg.output_dir.mkdir(parents=True, exist_ok=True)
    stamp = started.strftime("%Y%m%d_%H%M%S")
    csv_path = cfg.output_dir / f"bottom_limitup_trend_{stamp}.csv"
    json_path = cfg.output_dir / f"bottom_limitup_trend_{stamp}.json"
    markdown_path = cfg.output_dir / f"bottom_limitup_trend_{stamp}.md"
    with csv_path.open("w", encoding="utf-8-sig", newline="") as file:
        csv.DictWriter(file, fieldnames=list(CSV_FIELDS), extrasaction="ignore").writeheader()
    payload = {
        "generated_at": started.isoformat(timespec="seconds"), "version": VERSION,
        "run_status": status, "reason": reason, "config": json_safe(asdict(cfg)),
        "counts": {"spot_rows": 0, "fund_rank_rows": 0, "base_filtered": 0, "technical_candidates": 0, "technical_available": 0, "selected": 0},
        "data_quality": {"global_data_source": 1}, "selected": [],
    }
    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2, allow_nan=False), encoding="utf-8")
    markdown_path.write_text(
        "# A股底部涨停趋势筛选报告\n\n"
        f"生成时间：{started:%Y-%m-%d %H:%M}\n\n"
        f"运行状态：`{status}`\n\n"
        f"原因：{reason}\n\n"
        "本次未使用过期或不完整的实时资金流数据生成候选；任务已安全结束并保留空产物。\n",
        encoding="utf-8",
    )
    return {"csv": csv_path, "json": json_path, "markdown": markdown_path}
...
def parse_args() -> argparse.Namespace:
    ...
    runtime.add_argument("--workers", type=int, default=max(1, int(os.environ.get("WORKERS", 2))))
    runtime.add_argument("--retries", type=int, default=max(1, int(os.environ.get("RETRIES", 2))))
    runtime.add_argument("--timeout", type=float, default=max(1.0, float(os.environ.get("REQUEST_TIMEOUT", 12))))
    ...
    runtime.add_argument("--force-refresh", action="store_true", default=env_bool("FORCE_REFRESH", False))
    runtime.add_argument("--output-dir", type=Path, default=Path("output"))
    runtime.add_argument("--run-mode", choices=("intraday", "close"), default=os.environ.get("RUN_MODE", "close"))
    runtime.add_argument("--max-stale-minutes", type=int, default=int(os.environ.get("MAX_STALE_MINUTES", 180)))
    runtime.add_argument("--allow-unexpected-success", action="store_true", help="未知程序错误也写状态文件后以0退出；默认保留非0退出供CI发现代码问题")
...
def build_config(args: argparse.Namespace) -> Config:
    return Config(
        ...
        run_mode=args.run_mode, max_stale_minutes=max(1, args.max_stale_minutes),
        fail_on_unexpected=not args.allow_unexpected_success,
    )

def run_self_test() -> int:
    assert clean_code("600519.0") == "600519"
    assert epoch_to_cn(1_700_000_000) is not None
    assert epoch_to_cn(1_700_000_000_000) is not None
    LOGGER.info("离线自检通过。")
    return 0
...
def main() -> int:
    ...
    LOGGER.info("开始（%s模式）：资金流 + 肯纳特 + 520日布林 + 120周首红 + 底部涨停趋势筛选", cfg.run_mode)
    try:
        spot = get_spot(cfg, cache)
        funds = get_fund_rank(cfg, cache)
        latest_fund_at = verify_fund_freshness(funds, cfg, started)
        base_filtered = build_candidates(spot, funds, cfg)
        technical_candidates = base_filtered[: cfg.candidate_cap]
        LOGGER.info("行情 %s 只；资金流 %s 只（最新%s）；基础条件通过 %s 只；技术计算 %s 只", len(spot), len(funds), latest_fund_at.strftime("%H:%M"), len(base_filtered), len(technical_candidates))
        ...
        counts = {
            ...
            "selected": len(selected), "fund_latest_at": latest_fund_at.isoformat(),
        }
        paths = write_reports(selected, cfg, counts, issues, started)
    except DataSourceError as exc:
        reason = str(exc)
        LOGGER.warning("全局数据未就绪或不可用，安全跳过：%s", reason)
        paths = write_status_reports(cfg, started, "skipped_data_unavailable", reason)
        for name, path in paths.items(): print(f"{name.upper():<9}: {path}")
        return 0
    except Exception as exc:
        LOGGER.exception("未预期的程序错误")
        paths = write_status_reports(cfg, started, "unexpected_error", f"{type(exc).__name__}:{exc}")
        for name, path in paths.items(): print(f"{name.upper():<9}: {path}")
        return 1 if cfg.fail_on_unexpected else 0
