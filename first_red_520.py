...
def run_first_red_520_scan(limit=SCAN_LIMIT):
    ...
    results = []
    fail_count = 0
    print(f"开始520天首红扫描 {len(tasks)} 只股票（{NUM_PROCESSES} 进程并行, K线双源, 全部推送）...")
...
def build_push_content(df, cluster, spot_now=None):
    lines = []
    lines.append("*(🕒每行末'信号价@日期→现价@run'=首红信号日收盘 vs 本次run实时价)*")
    lines.append("")
    if cluster:
...
if __name__ == "__main__":
    ...
    print(f"K线双源(baostock+东财); 板块=baostock国标行业本地join; 命中全部推送; 不拦交易日; 推送全列+分页")
    print("=" * 70)
    ...
    if SERVERCHAN_KEY:
        try:
            ...
            title = f"520天首红 命中 {len(df)} 只（全发, 含板块）"
