...
【本版加固·防1h扫描白干】
 6 run_scan 扫描循环每只结果获取包try(防单只数据/pickle异常炸掉整个池子)。
 7 main 扫描完【先存原始命中】(保1h成果), 再 enrich 包try(崩了降级空聚类/风口, 原始命中与推送保留)。
   原漏洞: enrich未包try且存盘在其后, enrich一崩->1h白干+红叉+无推送。加固后无论收尾何错皆绿勾+有结果。
【矩阵接入·分段扫描】
 8 新增 SCAN_OFFSET 环境变量支持，允许通过 offset 跳过前 N 只股票，配合 SCAN_LIMIT 实现分段扫描。
⚠️ 概率性形态+背离+板块综合判断, 非预测; 老鸭头/W底均可能失败, 务必结合仓位/止损, 不构成投资建议。
⚠️ 周线老鸭头需~80周线, 故拉~1800天日线 resample 周线, min_data_len=450; 信号极严, 0命中属正常。
⚠️ 性能: 每只拉1800天+scipy极值偏慢, 3500只≈1h+; 建议 workflow 设 SCAN_LIMIT=1500 压到~30min。
"""
import os
import re
import sys
...
PUSH_MEDIUM = os.environ.get('PUSH_MEDIUM', '1').strip() in ('1', 'true', 'True')
...
def run_scan():
    ...
    # 【矩阵新增】分段扫描逻辑：先按 SCAN_OFFSET 跳过，再按 SCAN_LIMIT 截断
    ...
    with mp.Pool(processes=NUM_PROCESSES, initializer=_init_worker) as pool:
        ...
        for res in pool.imap_unordered(_process_one, tasks):
            # 【加固】单只结果获取包try: 防某只数据/pickle/worker异常炸掉整个扫描池
            try:
                if res:
                    if "__fail__" in res:
                ...
                fail_count += 1
                print(f"  ⚠️ 单只结果获取异常(已跳过, 不影响其余): {type(e).__name__}: {e}")
...
def build_push(df, cluster, hot, spot_now=None):
    ...
    def line(r):
        ...
        return (f"- {flag} **{r['名称']}({r['代码']})** [{sec_tag(r.to_dict())}] {r['信号强度']} 总分{r['总分']} "
                ...
                f" 周:{r.get('周线理由','—')}<br> 日:{r.get('日线理由','—')}")
    if not reso.empty:
...
if __name__ == "__main__":
    ...
    print(f"全扫={'是' if not SCAN_LIMIT else f'限{SCAN_LIMIT}'}; Offset={SCAN_OFFSET}; 双源; 多进程{NUM_PROCESSES}; 预筛={'开' if SNAPSHOT_PRE else '关'}; "
    ...
    # 【加固】先存原始命中, 保住1h扫描成果, 防后面 enrich/推送 崩了白干
    try:
        ...
        print(f"\n📁 已存原始命中 output/duck_head_wbottom_{tag}.* (1h扫描成果已落盘)")
    except Exception as e:
        print(f"\n⚠️ 原始存盘异常: {type(e).__name__}: {e}"); traceback.print_exc()

    # 【加固】enrich包try: 崩了降级空聚类/空风口, 原始命中与推送仍保留(不再白干)
    cluster, hot = [], []
    try:
        df, cluster, hot = enrich(df.to_dict('records'))
    except Exception as e:
        print(f"\n⚠️ 行业/风口增强异常(不影响原始命中与推送, 已降级): {type(e).__name__}: {e}"); traceback.print_exc()
        cluster, hot = [], []

    # 增强后再存一次(带行业/风口列); 失败也无妨, 原始已存
    try:
        df.to_csv(os.path.join(OUTPUT_DIR, f"duck_head_wbottom_{tag}.csv"), index=False, encoding="utf-8-sig")
        df.to_json(os.path.join(OUTPUT_DIR, f"duck_head_wbottom_{tag}.json"), orient='records', force_ascii=False, indent=2)
        print(f"📁 已存增强后 output/duck_head_wbottom_{tag}.*")
    except Exception as e:
        print(f"⚠️ 增强存盘异常(原始已存, 不影响): {type(e).__name__}: {e}"); traceback.print_exc()
