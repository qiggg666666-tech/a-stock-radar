#!/usr/bin/env python3
"""FINAL Chip 洗盘 + 首红确认汇总器。

只处理两类信号：
1. final_chip_research.py 里移植自筹码尖峰蒸馏引擎 classify_stage() 判定出的
   "洗盘"阶段(is_washout/stage=='洗盘')，按 washout_score 从高到低排序。
2. 移植自"全市场首红+10分钟MA5上扬"选股脚本的 is_first_red_confirmed
   （日线首红 is_first_red_daily + 盘中MA5拐头都确认），按盘中MA5斜率从高到低排序。
不再输出买入/高优先级/宽幅堆积区等其它信号。两类各自全部命中分批推送。
...
def main() -> int:
    parser = argparse.ArgumentParser(description="FINAL Chip 汇总（洗盘 + 首红确认）")
    ...
    if not completed:
        result = {
            "schema_version": "final-chip-summary/v5-washout-firstred",
    ...
    notifications: dict[str, list[dict[str, object]]] = {"washout": [], "first_red_confirmed": []}
    if args.notify:
    ...
    result = {
        ...
        "disclosure": (
            "FINAL Chip 汇总：只输出/推送 is_washout(stage=='洗盘') 与 is_first_red_confirmed"
            "（日线首红+盘中MA5拐头都确认）两类股票；通知失败不阻断 artifact。"
        ),
    }
    (args.output_dir / "final_chip_summary.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8"
