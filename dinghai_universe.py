...
def main() -> int:
    ...
    except Exception as exc:  # Keep a failure artifact for workflow-level audit.
        failure = {"schema_version": "dinghai-universe/v1", "status": "universe_unavailable", "generated_at": datetime.now().astimezone().isoformat(timespec="seconds"), "signal_date_requested": args.signal_date, "error": f"{type(exc).__name__}:{str(exc)[:1200]}", "count": 0, "universe": []}
    ...
    if args.status_output:
        args.status_output.parent.mkdir(parents=True, exist_ok=True)
        args.status_output.write_text(json.dumps({**payload, "status": "ready"}, ensure_ascii=False, indent=2), encoding="utf-8")
