# -*- coding: utf-8 -*-
"""
多周期 MACD 综合扫描脚本（baostock + 优化版 W 底）v2.1 · 矩阵规格
条件: 周DIF<-1 + 年W底(周线近似) + 月DEA零轴 + 月金叉 + 季W底 + 月W底
v2.1: 评分系统/成交量验证/年线过滤/消除MACD重复计算/列序修复/worker登录快速失败/失败CSV
【矩阵化】+Server酱分页推送+行业本地join+腾讯实时价对齐+存output/+空跑推送。扫描逻辑一字未动。
【本版修复】_align_suffix 改为纯字符串拼接(无花括号), 修复 SyntaxError: unmatched '}'。
依赖: baostock pandas numpy tqdm requests
"""
...
ENABLE_MA250_FILTER           = True
ENABLE_SCORE_SORTING          = True
...
SCORE_WEEKLY_DIF        = 1
SCORE_YEARLY_W_BOTTOM   = 2
SCORE_MONTHLY_GOLDEN    = 2
SCORE_QUARTERLY_W       = 2
SCORE_MONTHLY_W         = 2
SCORE_MONTHLY_DEA_ZERO  = 1
SCORE_VOLUME_CONFIRM    = 1
SCORE_MA250_SUPPORT     = 1

MA250_MAX_DISTANCE_PCT = 0.15
VOLUME_SHRINK_RATIO    = 0.85
VOLUME_EXPAND_RATIO    = 1.20

# ==================== 矩阵 env ====================
STOCK_POOL = os.getenv("STOCK_POOL", "hs300")
CUSTOM_CODES = []
...
def _align_suffix(r, spot_now):
    """实时价对齐后缀。用纯字符串拼接(无花括号), 避免 f-string 粘贴损坏导致 unmatched '}'。"""
...
def _worker_init():
    ...
    _LOGGED_IN = _login_with_retry()
    if not _LOGGED_IN:
        print("[警告] 本 worker 登录失败, 其股票将快速失败不逐个超时。")
...
def _fetch_hist_raw(code):
    global _LOGGED_IN
    if not _LOGGED_IN:
        raise RuntimeError("baostock 未登录（worker 初始化失败）")
...
def check_ma250_filter(daily_df):
    if len(daily_df) < 250:
        return False, None, None, None
    ma250 = daily_df["收盘"].rolling(window=250).mean().iloc[-1]
    current_price = daily_df["收盘"].iloc[-1]
    if pd.isna(ma250) or ma250 <= 0:
        return False, current_price, ma250, None
    distance_pct = (current_price - ma250) / ma250
    if distance_pct <= MA250_MAX_DISTANCE_PCT:
        return True, current_price, ma250, round(distance_pct, 4)
    return False, current_price, ma250, round(distance_pct, 4)

def find_w_bottom(dif_series, volume_series=None, min_dist=6, max_dist=40, tol=0.20,
                  min_rebound=0.15, max_age=12, require_below_zero=True, min_rebound_vol_mult=1.5):
    vol_pass = False
    if len(dif_series) < 30:
        return False, None, None, None, None, vol_pass
    ...
    for k in range(len(bottoms) - 1, 0, -1):
        ...
        if (mid_high_idx - b1["idx"] < 2) or (b2["idx"] - mid_high_idx < 2):
            continue
        if volume_series is not None and ENABLE_VOLUME_CHECK:
            vol = volume_series.iloc[-lookback:].values
            vol_b1 = vol[b1["idx"]]; vol_b2 = vol[b2["idx"]]
            vol_mid = np.mean(vol[mid_high_idx:mid_high_idx + 3]) if mid_high_idx + 3 <= n else np.mean(vol[mid_high_idx:])
            if vol_b1 > 0 and vol_b2 > 0:
                vol_pass = (vol_b2 < vol_b1 * VOLUME_SHRINK_RATIO) and (vol_mid > vol_b1 * VOLUME_EXPAND_RATIO)
        return True, round(b1["val"], 3), round(b2["val"], 3), b1["date"], b2["date"], vol_pass
    return False, None, None, None, None, vol_pass
...
def scan_one(code, name):
    try:
        ...
        dif_w, dea_w = calc_macd(weekly["收盘"])
        dif_m, dea_m = calc_macd(monthly["收盘"])
        dif_q, dea_q = calc_macd(quarterly["收盘"])
        score = 0
        result = {"代码": code, "名称": name, "收盘价": round(daily["收盘"].iloc[-1], 2),
                  "评分": 0, "年距%": None, "信号日期": daily.index[-1].strftime("%Y-%m-%d")}

        if ENABLE_MA250_FILTER:
            ma250_ok, cur, ma250, dist_pct = check_ma250_filter(daily)
            if not ma250_ok:
                return None, None
            result["年距%"] = dist_pct
            score += SCORE_MA250_SUPPORT

        if ENABLE_WEEKLY_DIF_LT_MINUS1:
            if len(dif_w) < 30 or dif_w.iloc[-1] >= WEEKLY_DIF_THRESHOLD:
                return None, None
            result["周DIF"] = round(dif_w.iloc[-1], 3)
            score += SCORE_WEEKLY_DIF
        else:
            result["周DIF"] = None

        if ENABLE_YEARLY_W_BOTTOM:
            vol_w = weekly["成交量"] if ENABLE_VOLUME_CHECK else None
            ok, b1, b2, d1, d2, vol_ok = find_w_bottom(dif_w, vol_w, YEARLY_W_MIN_DIST, YEARLY_W_MAX_DIST, W_TOL, W_MIN_REBOUND, YEARLY_W_MAX_AGE, W_REQUIRE_BELOW_ZERO)
            if not ok:
                return None, None
            result["年W前"], result["年W后"], result["年W后日期"] = b1, b2, d2.strftime("%Y-%m-%d")
            score += SCORE_YEARLY_W_BOTTOM
            if vol_ok:
                score += SCORE_VOLUME_CONFIRM; result["年W量"] = "✓"
            else:
                result["年W量"] = "✗" if ENABLE_VOLUME_CHECK else "-"
        else:
            result["年W前"] = result["年W后"] = result["年W后日期"] = result["年W量"] = None

        if ENABLE_MONTHLY_DEA_NEAR_ZERO:
            if len(dea_m) < 24 or abs(dea_m.iloc[-1]) >= MONTHLY_DEA_THRESHOLD:
                return None, None
            result["月DEA"] = round(dea_m.iloc[-1], 3)
            score += SCORE_MONTHLY_DEA_ZERO
        else:
            result["月DEA"] = None
        ...
        if ENABLE_MONTHLY_GOLDEN_CROSS:
            ...
            if not crossed:
                return None, None
            score += SCORE_MONTHLY_GOLDEN
        else:
            result["月金叉DIF"] = result["月金叉DEA"] = None

        if ENABLE_QUARTERLY_W_BOTTOM:
            vol_q = quarterly["成交量"] if ENABLE_VOLUME_CHECK else None
            ok, b1, b2, d1, d2, vol_ok = find_w_bottom(dif_q, vol_q, QUARTERLY_W_MIN_DIST, QUARTERLY_W_MAX_DIST, W_TOL, W_MIN_REBOUND, QUARTERLY_W_MAX_AGE, W_REQUIRE_BELOW_ZERO)
            if not ok:
                return None, None
            result["季W前"], result["季W后"], result["季W后日期"] = b1, b2, d2.strftime("%Y-%m-%d")
            score += SCORE_QUARTERLY_W
            if vol_ok:
                score += SCORE_VOLUME_CONFIRM; result["季W量"] = "✓"
            else:
                result["季W量"] = "✗" if ENABLE_VOLUME_CHECK else "-"
        else:
            result["季W前"] = result["季W后"] = result["季W后日期"] = result["季W量"] = None

        if ENABLE_MONTHLY_W_BOTTOM:
            vol_m = monthly["成交量"] if ENABLE_VOLUME_CHECK else None
            ok, b1, b2, d1, d2, vol_ok = find_w_bottom(dif_m, vol_m, MONTHLY_W_MIN_DIST, MONTHLY_W_MAX_DIST, W_TOL, W_MIN_REBOUND, MONTHLY_W_MAX_AGE, W_REQUIRE_BELOW_ZERO)
            if not ok:
                return None, None
            result["月W前"], result["月W后"], result["月W后日期"] = b1, b2, d2.strftime("%Y-%m-%d")
            score += SCORE_MONTHLY_W
            if vol_ok:
                score += SCORE_VOLUME_CONFIRM; result["月W量"] = "✓"
            else:
                result["月W量"] = "✗" if ENABLE_VOLUME_CHECK else "-"
        else:
            result["月W前"] = result["月W后"] = result["月W后日期"] = result["月W量"] = None
...
def build_push(df, spot_now=None):
    lines = [f"**📊 多周期MACD底部共振(v2.1评分版)** | 命中{len(df)}只 (现价=实时价)",
             "*(周DIF<-1+年/季/月W底+月金叉+月DEA零轴+成交量验证+年线过滤; 按评分排序; 非预测, 必止损)*", ""]
    def line(r):
        vol = "".join(x for x in [f"年量{r.get('年W量','')}", f"季量{r.get('季W量','')}", f"月量{r.get('月W量','')}"] if x and not x.endswith('-'))
        return (f"- **{r['名称']}({r['代码']})** [{r.get('行业', '—')}] 评分{r['评分']} 现价{r.get('最新价')} | "
                f"周DIF{r.get('周DIF')} 月DEA{r.get('月DEA')} 年距{r.get('年距%')}% | "
                f"年W{r.get('年W后日期','—')} 季W{r.get('季W后日期','—')} 月W{r.get('月W后日期','—')} {vol}{_align_suffix(r, spot_now)}")
...
def main():
    ...
    fail_csv = os.path.join(OUTPUT_DIR, f"multi_period_macd_fail_{tag}.csv")

    if failures:
        print(f"\n有 {len(failures)} 只抓取/处理失败：")
        for code, name, reason in failures[:20]:
            print(f"  - {code} {name}: {reason}")
        pd.DataFrame(failures, columns=["代码", "名称", "失败原因"]).to_csv(fail_csv, index=False, encoding="utf-8-sig")

    if not results:
        print("\n没有股票同时满足所有启用条件。")
        pd.DataFrame().to_csv(out_csv, index=False, encoding="utf-8-sig")
        if SERVERCHAN_KEY:
            send_serverchan("📊 多周期MACD底部共振 | 0命中", "**多周期MACD底部共振(v2.1)** | 本次无同时满足全部条件的票(长周期左侧条件极严, 0命中属正常)。")
        return

    df = pd.DataFrame(results)
    sort_cols = ["评分"]; ascending = [False]
    if "周DIF" in df.columns and df["周DIF"].notna().any():
        sort_cols.append("周DIF"); ascending.append(True)
    df = df.sort_values(by=sort_cols, ascending=ascending)
    preferred = ["评分", "代码", "名称", "收盘价"]
