...
【v5.1 改动】不生成 Excel，只存 CSV；推送改为全部结果(自动分页防超长)。
数据源：baostock (http://baostock.com)
依赖：pip install baostock pandas numpy tqdm requests
================================================================================
"""
...
class Config:
    ...
    MAX_RETRIES: int = 3
    QUERY_TIMEOUT_SEC: int = 20         # 单次 baostock 查询超时
    LOGIN_STAGGER_SEC: float = 1.5      # 各子进程启动登录错峰延迟
...
def _get_query_executor() -> ThreadPoolExecutor:
...
def call_with_timeout(func, *args, timeout=None, **kwargs):
    timeout = timeout or cfg.QUERY_TIMEOUT_SEC
    fut = _get_query_executor().submit(func, *args, **kwargs)
    return fut.result(timeout=timeout)


def retry_call(func, *args, max_retries=None, base_delay=1.3, **kwargs):
    n = max_retries if max_retries is not None else cfg.MAX_RETRIES
    last_exc = None
    for attempt in range(n):
        try:
            return call_with_timeout(func, *args, **kwargs)
        except Exception as e:
            last_exc = e
            time.sleep(base_delay * (1.7 ** attempt) + np.random.uniform(0, 0.4))
    raise last_exc
...
def fetch_stock_hist(bs_code: str, days: int = None) -> Optional[pd.DataFrame]:
    ...
    df["isST"] = pd.to_numeric(df.get("isST", 0), errors="coerce").fillna(0)
    if "tradestatus" in df.columns:
        df = df[df["tradestatus"] == "1"]
    df = df.dropna(subset=["close", "volume", "open"]).reset_index(drop=True)

    if len(df) < cfg.MIN_LIST_DAYS:
        return None
    return df
...
def get_stock_pool() -> List[Tuple[str, str]]:
    log.info("[1/5] 获取A股股票池 (baostock query_stock_basic)...")
    import baostock as bs
    lg = bs.login()
    if lg.error_code != "0":
    ...
    try:
        ...
        if rs.error_code != "0":
            log.error(f"获取股票列表失败: {rs.error_msg}")
            sys.exit(1)
        rows = []
        while (rs.error_code == "0") and rs.next():
            rows.append(rs.get_row_data())
    ...
    log.info(f"  原始列表: {len(df)} 只")

    df = df[(df["type"] == "1") & (df["status"] == "1")]
    log.info(f"  仅保留正常上市股票: {len(df)} 只")

    df["plain_code"] = df["code"].str.split(".").str[-1]

    if cfg.EXCLUDE_KCB:
        df = df[~df["plain_code"].str.startswith(("688", "689"))]
    if cfg.EXCLUDE_CHUANGYE:
        df = df[~df["plain_code"].str.startswith(("300", "301"))]
    if cfg.EXCLUDE_BJ:
        df = df[~df["plain_code"].str.startswith(("8", "4"))]

    df = df[df["plain_code"].str.match(r"^(00|30|60|68|8|4)", na=False)]

    log.info(f"  ✅ 最终股票池: {len(df)} 只")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="年线涨停选股系统 v5.1-baostock版")
    ...
    p.add_argument("--query-timeout", type=int, default=cfg.QUERY_TIMEOUT_SEC, help="单次baostock查询超时(秒)")
...
def main():
    ...
    cfg.QUERY_TIMEOUT_SEC = args.query_timeout
    ...
    log.info(f"[2/5] 扫描 {len(stock_list)} 只股票 | {cfg.NUM_PROCESSES} 进程 | 预计视网络情况需数分钟到数十分钟...")
    ...
    with ctx.Pool(
        ...
        initargs=(industry_map,),
    ) as pool:
        async_results = []
        for i, (code, name) in enumerate(stock_list):
            async_results.append(pool.apply_async(process_one_stock, (code, name)))

        pbar = tqdm(total=len(async_results), desc="扫描进度", ncols=90,
                    bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]")
        for ar in async_results:
            try:
                res = ar.get(timeout=cfg.QUERY_TIMEOUT_SEC * (cfg.MAX_RETRIES + 2) + 30)
                if res:
                    if "__error__" in res:
                ...
                if len(error_samples) < _MAX_ERROR_SAMPLES:
                    error_samples.append(f"pool.get 超时/异常: {e}")
            pbar.update(1)
        pbar.close()
    ...
    print("📈 选股统计摘要")
    print("-" * 66)
    print(f"  主板涨停       : {(df['涨停类型'].str.contains('主板')).sum()} 只")
    print(f"  创业板涨停     : {(df['涨停类型'].str.contains('创业板')).sum()} 只")
    print(f"  科创板涨停     : {(df['涨停类型'].str.contains('科创板')).sum()} 只")
