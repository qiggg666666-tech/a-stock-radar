...
def compute_indicator(stock: pd.DataFrame, benchmark: Optional[pd.DataFrame], params: Parameters) -> pd.DataFrame:
    ...
    pre_pink_bar = (~bull_state) & medium_up & relative_leader & control.ge(trend) & pink_ready_count.ge(3)
...
def write_outputs(result: pd.DataFrame, quality: dict[str, str], config: RuntimeConfig) -> dict[str, Path]:
    ...
    recent = export.tail(15)[["date", "close", "股牛股", "首个粉红柱", "预备首粉", "预备首粉缺失条件", "操纵", "趋势", "买进", "信号评分"]].copy()
...
def run_self_test() -> int:
    ...
    required = {"股牛股", "粉色柱", "首个粉红柱", "粉红准备度", "预备首粉", "预备首粉缺失条件", "操纵", "趋势", "买进", "信号评分", "相对强弱%"}
    ...
    assert (result.loc[result["预备首粉"] == 1, "粉红准备度"] >= 3).all(), "预备首粉必须已满足至少三项原始粉红条件"
