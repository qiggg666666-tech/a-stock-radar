...
def main() -> int:
    ...
    parser.add_argument("--fund-flow-timeout-seconds", type=float, default=20.0, help="洗盘/红筹码占优/长影线候选的主力资金二次确认单次超时")
    parser.add_argument("--fund-flow-retries", type=int, default=2, help="主力资金二次确认的重试次数")
    parser.add_argument("--skip-fund-flow-confirm", action="store_true", help="跳过洗盘/红筹码占优/长影线候选的主力资金二次确认")
    ...
    for position, item in enumerate(selected, 1):
        ...
        try:
            ...
            # 主力资金二次确认：只对已经命中洗盘/红筹码占优/长影线任一初筛信号的候选
            # 调用，跟盘中MA5拐头确认走同一套"先初筛、再对少数候选发起昂贵的逐股票
            # 请求"的模式，避免全市场逐一调用资金流接口。
            #
            # 收紧记录：is_wide_zone和ma_signal强多这两条曾经短暂加进来过，实测命中率
            # 太高(接近半个市场)，导致单分片资金流调用次数冲到几百次，4×2秒的请求间隔
            # 加上真实网络耗时，把scan job的180分钟硬顶直接跑穿、4个分片里3个被强制
            # 取消，summary因此完全没有真实数据可推——这就是"看不到推送"那次事故的根因。
            # 现在退回到只用这三个确实比较少见的信号，同时下面加了次数硬顶兜底，以后
            # 不管信号组合怎么调，单分片的资金流调用耗时都有可预测上限，不会再靠"这次
            # 命中率会不会太高"去赌。
            # 长影线这条改成只认daily_long_lower(长下影买入型态)——has_long_shadow
            # 两个方向都算，会把长上影(卖出/压力型态)也拉进资金流确认，跟"长影线"这个
            # 专区现在的买入定位对不上。
    ...
    status = {
        ...
        "intraday_confirm_calls": intraday_confirm_count,
        "fund_flow_confirm_calls": fund_flow_confirm_count,
        "disclosure": "FINAL Chip专属分片；没有合成记录或分片推送；仅对is_first_red_daily候选调用盘中二次确认(有单分片次数硬顶)，仅对洗盘/红筹码占优/长影线候选调用主力资金二次确认(同样有单分片次数硬顶)。",
    }
    (args.output_dir / "status.json").write_text(
        json.dumps(status, ensure_ascii=False, indent=2), encoding="utf-8"
    )
