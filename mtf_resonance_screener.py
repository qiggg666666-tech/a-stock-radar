...
【本版完善】
  8 交易日拦截仅对定时触发(schedule)生效, 手动(workflow_dispatch)/本地不受限(周末可用周五数据看趋势共振)。
  9 收尾(存盘/控制台/推送)各包 try/except + sys.exit(0), 任何收尾异常不让job误红。
...
def main():
    ...
    # 【改1】交易日拦截仅对定时触发(schedule)生效; 手动(workflow_dispatch)/本地不受限, 周末可用周五数据看趋势共振
    if os.environ.get('GITHUB_EVENT_NAME') == 'schedule' and not is_trading_day():
        logger.info("非交易日且为定时触发(schedule), 跳过; 手动/本地运行不受此限")
        return
    ...
    reso = [r for r in results if r['resonance']]   # 提到收尾防护前, 控制台/推送共用

    # 【改2】收尾全部包防护: 任何收尾IO/展示/推送异常都不应让job误红
    ...
    try:
        ...
        logger.error(f"⚠️ 控制台展示异常: {type(e).__name__}: {e}")
