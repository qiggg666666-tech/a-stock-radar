...
# 2026-08-30 按金瑞矿业(600714)类形态放宽：图里是「下方清晰红柱簇」而非「单点碾压3倍」
# 的极端尖刺。原 0.12/3.0/0.008 会漏掉 band≈12%、gap≈1.02、ratio≈0.32% 的典型红柱。
# gap=1.0：下方最高局部峰至少不低于第二峰即可（多柱并列也算清晰红柱）。
BELOW_SPIKE_BAND_MIN, BELOW_SPIKE_GAP_MIN = 0.10, 1.0
# 占比阈值：这根"下方峰"至少要占现价下方总筹码的多少比例，才算"长"，不是随便一根
# 小尖刺。放宽到 0.003，覆盖多柱分摊但仍明显的红柱形态。
BELOW_SPIKE_RATIO_MIN = 0.003
