...
为日线批量扫描。5重过滤的趋势确认做多信号:
  ① 趋势强度: ADX ≥ adxMin
  ② 波动充足: 布林带宽度% ≥ minBBWidthPct
  ③ 多头趋势: EMA20>EMA50 且 收盘>SMA200 且 +DI>-DI
  ④ RSI确认 : RSI(14) ≥ rsiBullMin
...
【本版加严·解决"做多87只偏多/追高"】原5重过滤只确认"趋势在", 无过热保护 -> RSI74/BB宽118%/已涨一段
  的追高票混入。本版加3道过热上限(全env可调): RSI≤RSI_BULL_MAX(70) + BB宽≤MAX_BB_WIDTH_PCT(60) +
  近20日涨幅≤MAX_RET20(30%); 并把 ADX_MIN(20→25)/RSI_BULL_MIN/MIN_BB_WIDTH_PCT 改读env(原写死PARAMS,
  加workflow env不生效, 故必须改码)。FAIL_STATS 加"过热追高"细分, 看日志知砍在哪。
  默认值即加严, 替换本文件即生效; env 为可选微调入口。
...
⚠️ 5重过滤+过热保护叠加, 信号稀少(宁缺毋滥), 全市场0命中属正常, 非bug。
...
# ==================== 【本版加严】env 可调阈值 (解决"做多偏多/追高", 必须在PARAMS前定义) ====================
ADX_MIN = float(os.environ.get('ADX_MIN', '25'))            # 趋势强度下限(原20→25, 砍弱趋势)
RSI_BULL_MIN = float(os.environ.get('RSI_BULL_MIN', '50'))  # RSI多头下限(原50)
RSI_BULL_MAX = float(os.environ.get('RSI_BULL_MAX', '70'))  # RSI过热上限(新增, 砍RSI74追高)
MIN_BB_WIDTH_PCT = float(os.environ.get('MIN_BB_WIDTH_PCT', '4'))   # 布林宽下限(原4)
MAX_BB_WIDTH_PCT = float(os.environ.get('MAX_BB_WIDTH_PCT', '60'))  # 布林宽上限(新增, 砍BB宽118%暴涨)
MAX_RET20 = float(os.environ.get('MAX_RET20', '0.30'))      # 近20日涨幅上限(新增, 砍已涨一段)


# ==================== 参数配置 (对应Pine inputs) ====================
PARAMS = dict(
    fastLen=20, slowLen=50, trendLen=200,
    rsiLen=14, rsiBullMin=RSI_BULL_MIN, rsiBearMax=50,
    pivotLen=5,
    atrLen=14, stopATRMult=2.0, tp1ATRMult=1.5, tp2ATRMult=3.0,
    adxLen=14, adxSmooth=14, adxMin=ADX_MIN,
    bbLen=20, bbMult=2.0, minBBWidthPct=MIN_BB_WIDTH_PCT,
    rsiBullMax=RSI_BULL_MAX, maxBBWidthPct=MAX_BB_WIDTH_PCT, maxRet20=MAX_RET20,
...
_BS_LOGGED = False
_INDUSTRY_MAP = {}
FAIL_STATS = {"抓取失败": 0, "数据不足": 0, "无信号": 0, "过热追高": 0}
...
def check_one_stock(df: pd.DataFrame):
    ...
    # 【本版加严】近20日涨幅(追高保护用)
    ret20 = (c / float(close.iloc[-21]) - 1.0) if (len(close) >= 21 and close.iloc[-21] and close.iloc[-21] > 0) else 0.0
    ...
    longBase = trendOK and volOK and bullTrend and rsiBull and (maBullCross or bosUp)
    shortSignal = trendOK and volOK and bearTrend and rsiBear and (maBearCross or bosDn)
    # 【本版加严】过热/追高保护: RSI过高 / 布林过宽(已暴涨) / 近20日涨幅过大 -> 砍
    overheat = (r > PARAMS['rsiBullMax']) or (bw > PARAMS['maxBBWidthPct']) or (ret20 > PARAMS['maxRet20'])
    is_long = bool(longBase and not overheat)
    is_warn = bool(shortSignal)
    if not is_long:
        if PARAMS['INCLUDE_WARNINGS'] and is_warn:
            pass  # 走空头警示分支
        elif longBase and overheat:
            return None, "过热追高"
        else:
            return None, "无信号"
    ...
    return {"代码": None, "名称": None, "行业": "",
            ...
            "RSI": round(r, 1), "BB宽度%": round(bw, 2), "近20日涨幅%": round(ret20 * 100, 1),
...
def run_scan():
    ...
    print(f"开始MA+RSI+ADX+BB+SMC扫描 {len(tasks)} 只（{PARAMS['NUM_PROCESSES']}进程, 双源, 5重过滤+过热保护, 含警示={PARAMS['INCLUDE_WARNINGS']}）...")
        ...
        for res in pool.imap_unordered(_process_one, tasks):
            if res:
                if "__fail__" in res:
                    ...
                    results.append(res)
                    pbar.write(f"  √ {res['代码']} {res['名称']} {res['信号']} 触发:{res['触发']} ADX={res['ADX']} RSI={res['RSI']} 涨{res['近20日涨幅%']}% 价={res['最新价']}")
...
def build_push(df, cluster, hot):
    ...
    L = [f"**📈 MA+RSI+ADX+BB+SMC 趋势确认(加严)** | 做多{len(longs)}只 警示{len(warns)}只 🎯风口{len(reso)} (全发)",
         f"*(加严: ADX≥{PARAMS['adxMin']}+RSI[{PARAMS['rsiBullMin']},{PARAMS['rsiBullMax']}]+BB宽≤{PARAMS['maxBBWidthPct']}%+涨≤{PARAMS['maxRet20']*100:.0f}%; 建议位=入场初始位, trailing/TP1半仓/保本需持仓后手动; 信号稀少属正常)*", ""]
    if hot:
    ...
    def line(r):
        ...
        return (f"- {flag} **{r['名称']}({r['代码']})** [{_sec_tag(r.to_dict())}] 触发:{r['触发']} | 现价{r['最新价']} "
                f"ADX={r['ADX']} RSI={r['RSI']} BB宽={r['BB宽度%']}% 涨{r.get('近20日涨幅%','—')}% | {pos}")
    if not reso.empty:
...
def main():
    print("=" * 70)
    print(f"📈 MA+RSI+ADX+BB+SMC 趋势确认(加严) | {datetime.now():%Y-%m-%d %H:%M} | 回看{PARAMS['lookback_days']}天")
    ...
    print(f"【加严】ADX≥{PARAMS['adxMin']} RSI[{PARAMS['rsiBullMin']},{PARAMS['rsiBullMax']}] BB宽[{PARAMS['minBBWidthPct']},{PARAMS['maxBBWidthPct']}]% 近20日涨≤{PARAMS['maxRet20']*100:.0f}%")
    print("=" * 70)
    df = run_scan()
    if df is None or df.empty:
        print("\n本次扫描未发现满足5重过滤+过热保护的趋势确认信号。")
        print("过滤极严, 0命中属正常; 看上面统计(过热追高/无信号各多少); 放宽: RSI_BULL_MAX调大/MAX_RET20调大/ADX_MIN调小")
        sys.exit(0)
    ...
    try:
        ...
        with open(os.path.join(OUTPUT_DIR, f"ma_rsi_adx_{tag}.json"), 'w', encoding='utf-8') as f:
            json.dump({"date": tag, "params": {"adxMin": PARAMS['adxMin'], "minBBWidthPct": PARAMS['minBBWidthPct'],
                                               "rsiBullMin": PARAMS['rsiBullMin'], "rsiBullMax": PARAMS['rsiBullMax'],
                                               "maxBBWidthPct": PARAMS['maxBBWidthPct'], "maxRet20": PARAMS['maxRet20'],
    ...
    if SERVERCHAN_KEY:
        try:
            ...
            send_serverchan(f"📈 趋势确认(加严) 做多{n_long}只 🎯风口{n_reso}", build_push(df, cluster, hot))
        except Exception as e:
