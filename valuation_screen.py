# -*- coding: utf-8 -*-
"""
valuation_screen.py —— PE/PB 估值筛选 + 估值洼地聚类💎 + 估值遇催化💰
主路径=东财快照一次向量化(秒级, 不依赖交易日, 周末可用周五估值数据); 兜底=baostock逐只(东财挂时)。
PE/PB合理区间 + 价格≥MIN_PRICE + 非ST; 最便宜Top补行业→洼地板块聚类💎; 低估值+板块风口=估值遇催化💰(价值兑现提示, 非时机信号)。

【本版完善】
 1 主路径调用包try/except: 东财向量化若抛异常(非返回空), 优雅降级baostock兜底, 不崩。
 2 修兜底路径月初拉数据bug: 原start_date=本月1号, 月初(1-2号)本月数据不足→大量拉空; 改拉最近40天确保跨月有数据。
 3 加收尾防护sys.exit(0): 存盘/展示/推送异常不让job误红。
 4 主路径无交易日拦截(设计如此, 周末可跑周五估值), 故无需改; 策略内核/阈值/洼地聚类/💰遇催化/推送 一字未动。
 5 【核心·修撞timeout白干】兜底逐只原全扫全A~5000只, 限流时每只卡满15s→数小时→撞150分钟timeout白干且0存盘。
   现加 兜底截断保底(SCAN_LIMIT, 默认1500) + 软超时主动收工(SOFT_TIMEOUT, 默认50分钟<150): 限流时到点break,
   保住已算命中正常存盘+推送拿绿勾, 不再白干。截断/软超时仅作用于兜底逐只, 主路径东财快照不截断(秒级全A)。
 6 【修隐藏hang】fetch_industry/get_industry_heat 的 akshare 调用原为裸调用无超时, 限流时可能hang几十分钟拖死enrich;
   新增 _call_with_timeout 工具, 给这两处包20s硬超时, enrich阶段最坏几分钟可控。
⚠️ 估值为安全垫维度, 非时机信号; 低PE/PB警惕价值陷阱, 需结合基本面。
⚠️ 兜底截断按列表序(偏沪市)+软超时限流时只扫部分=降级态取舍, 保"有结果"优先于"全扫"; 主路径正常时不受影响。
"""
import os
import sys
import time
...
SCAN_LIMIT = int(os.environ.get('SCAN_LIMIT', '1500'))      # 【本版】兜底逐只截断(主路径东财快照不截断; 0=不截断)
SOFT_TIMEOUT = int(os.environ.get('SOFT_TIMEOUT', '3000'))  # 【本版】兜底逐只软超时(秒, 默认50分钟<150 timeout)
...
# ------------------ 【本版新增】通用硬超时(给裸akshare调用包超时, 防限流hang) ------------------
def _call_with_timeout(fn, *a, timeout=20, **kw):
    """给无内置超时的 akshare 调用包硬超时; 超时/异常返回 None, 由调用方降级, 绝不 hang。
    用于 fetch_industry / get_industry_heat 等逐只/单次裸调用, 限流时砍在 timeout 而非 hang 几十分钟。"""
...
def fetch_industry(symbol):
    for attempt in range(2):
        try:
            info = _call_with_timeout(ak.stock_individual_info_em, symbol=symbol, timeout=20)  # 【本版】包20s硬超时
...
def get_industry_heat():
    for i in range(3):
        try:
            d = _call_with_timeout(ak.stock_board_industry_name_em, timeout=20)  # 【本版】包20s硬超时
...
def _process_one(args):
    code, name = args
    try:
        # 修月初拉数据bug: 拉最近40天(跨月)确保有数据, 而非本月1号(月初数据不足)
        start_date = (datetime.now() - timedelta(days=40)).strftime('%Y-%m-%d')
        df = _query_with_timeout(code, "date,close,peTTM,pbMRQ", start_date=start_date)
...
def _screen_via_bs():
    """baostock 逐只兜底 (修登录 bug + 列表双源 + 【本版】截断保底+软超时); 仅东财主路径失败时调用"""
    ...
    # 【本版】兜底截断保底: 东财挂时 baostock 逐只全A~5000 太慢, 截断到 SCAN_LIMIT(降级, 按列表序偏沪市)
    if SCAN_LIMIT and len(tasks) > SCAN_LIMIT:
        print(f"  [兜底] 全A {len(tasks)} 只 > 截断{SCAN_LIMIT}(降级保底, 防逐只太慢撞timeout)")
        tasks = tasks[:SCAN_LIMIT]

    results = []
    fail_count = 0
    print(f"  [兜底] 估值筛选 {len(tasks)} 只（{NUM_PROCESSES} 进程并行, 软超时{SOFT_TIMEOUT//60}分钟）...")
    _t0 = time.time()
    with mp.Pool(processes=NUM_PROCESSES, initializer=_init_worker) as pool:
        ...
        for res in pool.imap_unordered(_process_one, tasks):
            ...
            # 【本版】软超时主动收工: 限流时逐只卡满15s, 到点 break 保已命中, 防撞 timeout 白干
            if time.time() - _t0 > SOFT_TIMEOUT:
                print(f"\n  ⏱️ 兜底软超时{SOFT_TIMEOUT//60}分钟到, 主动收工(已扫{pbar.n}/{len(tasks)}), 保住已命中{len(results)}只")
                break
...
if __name__ == "__main__":
    ...
    print(f"主路径=东财快照向量化(秒级, 周末可跑周五估值); 兜底=baostock逐只(截断{SCAN_LIMIT}+软超时{SOFT_TIMEOUT//60}分钟); 范围={UNIVERSE}")
    print("=" * 70)

    # 主路径: 东财快照 (不依赖交易日, 周末亦可); 异常则优雅降级兜底, 不崩
    try:
        results = _screen_via_em()
        via = 'em'
    except Exception as e:
        print(f"  东财主路径异常: {type(e).__name__}: {e}, 切换 baostock 兜底 ...")
        results = []
        via = 'em'
    if not results:
    ...
    if results:
        ...
        try:
            csv_path = f"{OUTPUT_DIR}/valuation_screen_{tag}.csv"
            json_path = f"{OUTPUT_DIR}/valuation_screen_{tag}.json"
