...
def main() -> int:
    ...
    notification_preview = candidates.head(20)
    ...
    report = ['# 筹码尖峰蒸馏汇总', '', f'- 完成分片：{len(completed)}/{args.shard_total}', f'- 有效记录：{len(records)}', f'- 数据错误：{len(errors)}', f'- 排序记录（artifact）：{len(candidates)}', f'- 推送展示：{len(notification_preview)}（最多20条）', '', '## 推送预览（最多20条）']
    preview_fields = [field for field in ['code', 'name', 'is_tradeable', 'is_approaching', 'dist_to_peak_pct'] if field in notification_preview.columns]
    if preview_fields:
        for index, (_, row) in enumerate(notification_preview[preview_fields].iterrows(), start=1):
            report.append(f"{index:02d}. {row.get('code', '')} {row.get('name', '')}｜可交易{row.get('is_tradeable', '')}｜接近尖峰{row.get('is_approaching', '')}｜距尖峰{row.get('dist_to_peak_pct', '')}")
    else:
        report.append('当日没有可展示的研究记录；完整状态和错误审计见artifact。')
    report.extend(['', '仅为既有筹码尖峰规则的研究输出。'])
