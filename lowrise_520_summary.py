...
def notification_body(summary: dict[str, Any], observations: pd.DataFrame) -> str:
    ...
    fields = [field for field in ["priority_rank", "code", "name", "signal_date", "distance_to_520_low_pct", "volume_ratio_previous_5"] if field in observations.columns]
    if fields:
        lines.extend(["", "## 低位首红观察"])
        for _, row in observations[fields].head(30).iterrows():
            lines.append(f"{row.get('priority_rank', '-')}. {row.get('code', '')} {row.get('name', '')} / 距520低点 {row.get('distance_to_520_low_pct', '')}")
    else:
...
def main() -> int:
    ...
    if not observations.empty:
        observations = observations.sort_values(["distance_to_520_low_pct", "volume_ratio_previous_5"], ascending=[True, False]).reset_index(drop=True)
