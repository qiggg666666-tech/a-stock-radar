...
# ------------------ 阈值参数 ------------------
LOW_WINDOW = int(os.environ.get('LOW_WINDOW', '120'))   # 【120日首红】半年新低后首阳(比520日更敏感, 命中更多)
...
def _fetch_spot_now():
    ...
    收盘后跑时, 此处'现价'=当天收盘价, 对齐列正好显示'首红信号后隔几天涨/跌多少'。"""
    try:
...
def run_first_red_120_scan(limit=SCAN_LIMIT):
    ...
    if _bs_login_ok():
        try:
            ...
            stock_df = pd.DataFrame()
        # 顺手一次拿全市场行业映射(同一登录态, 零额外登录, 零逐只接口 -> 命中再多也无限流)
        try:
...
# ------------------ 行业标注(本地join) + 见底聚类 ------------------
def enrich(results):
    """用 _INDUSTRY_MAP 本地填行业(零接口, 命中再多也瞬间完成) -> 见底板块聚类(本地 groupby)"""
    if not results:
        return pd.DataFrame(), []
...
def build_push_content(df, cluster, spot_now=None):
    """全列所有首红命中(不截断); 超长由 send_serverchan 自动分页发多条。
    每行末加'信号价@日期→现价@run'对齐列(spot_now 由 main 传入, 限流自动降级)。"""
    lines = []
    lines.append("*(🕒每行末'信号价@日期→现价@run'=首红信号日收盘 vs 本次run实时价, 含信号后涨跌幅; 信号价=前复权日线, 现价=不复权实时, 期间除权则涨幅仅供参考)*")
    lines.append("")
    if cluster:
        lines.append("🔻 **见底板块**(首红扎堆, 板块级反转): " +
                     "、".join(f"{n}({c}只)" for n, c in cluster))
        lines.append("")
    lines.append(f"### 📋 120天首红 共{len(df)}只 (全发, 含板块)")
    for _, r in df.iterrows():
        five = f" | 5日最低{r['5日最低价']}（距5日低点{r['距5日低点%']}%）" if pd.notna(r.get('5日最低价')) else ""
        base = (f"- {r['名称']}（{r['代码']}）[{sec_tag(r.to_dict())}] {r['信号日期']} 最新价 {r['最新价']} "
                f"| 量比 {r['量比']} | 距120日低点 {r['距120日低点%']}%{five}")
        lines.append(base + _align_suffix(r, spot_now))   # 时点+实时对齐后缀
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    print(f"K线双源(baostock+东财); 板块=baostock国标行业一次拿全+本地join; 不拦交易日; 推送全列+分页")
    print("【120日首红】半年新低后首次收阳, 比520日首红更敏感、命中更多; 推送带'信号价@日期→现价@run'对齐列")
    print("=" * 70)

    # 不拦交易日: 周末/节假日也可用上一交易日数据复盘首红信号
    ...
    # ---- 收尾全部包防护: 任何收尾IO/展示/推送异常都不应让job误红 ----
    try:
    ...
    if SERVERCHAN_KEY:
        try:
            spot_now = _fetch_spot_now()   # 推送前拉一次实时价(限流降级, 不阻断)
