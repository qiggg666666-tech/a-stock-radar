...
def main() -> int:
    ...
    for position, item in enumerate(selected, 1):
        ...
        try:
            ...
            # 主力资金二次确认：只对已经命中洗盘/红筹码占优/长影线/均线强多任一初筛
            # 信号的候选调用，跟盘中MA5拐头确认走同一套"先初筛、再对少数候选发起昂贵
            # 的逐股票请求"的模式，避免全市场逐一调用资金流接口。
            # 同步记录：is_chip_spike/is_below_spike已被is_red_heavy_chip取代(summary
            # 那边的"尖峰筹码柱"专区已经换成"红筹码占优")，这里跟着换；均线强多没有
            # 独立的布尔字段，直接用ma_signal前缀判断，跟summary里的口径一致。
            is_fund_flow_candidate = bool(
                row.get("is_washout")
                or row.get("is_red_heavy_chip")
                or row.get("has_long_shadow")
                or str(row.get("ma_signal", "")).startswith("强多")
            )
    ...
    status = {
        ...
        "intraday_confirm_calls": intraday_confirm_count,
        "fund_flow_confirm_calls": fund_flow_confirm_count,
        "disclosure": "FINAL Chip专属分片；没有合成记录或分片推送；仅对is_first_red_daily候选调用盘中二次确认，仅对洗盘/红筹码占优/长影线/均线强多候选调用主力资金二次确认。",
    }
    (args.output_dir / "status.json").write_text(
        json.dumps(status, ensure_ascii=False, indent=2), encoding="utf-8"
    )
