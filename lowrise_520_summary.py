...
def notification_body(summary: dict[str, Any], observations: pd.DataFrame) -> str:
    ...
    if fields:
        ...
        for _, row in observations[fields].head(20).iterrows():
            label = "优先＜阈值" if row.get("distance_priority_group", "") == "priority_under_5pct" else "原版保留"
...
def main() -> int:
    ...
    parser.add_argument("--priority-distance-pct", type=float, default=DEFAULT_PRIORITY_DISTANCE_PCT, help="距520日低点优先排序阈值；须与workflow_dispatch传给shard的值一致")
    ...
    priority_distance_pct = args.priority_distance_pct
