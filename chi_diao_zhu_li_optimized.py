...
def write_outputs(result: pd.DataFrame, quality: dict[str, str], config: RuntimeConfig) -> dict[str, Path]:
    ...
    payload = {"generated_at": pd.Timestamp.now(tz=CN_TZ).isoformat(), "code": config.code, "mode": config.mode, "parameters": asdict(config.params), "data_quality": quality, "latest": latest, "recent_buy_dates": buy_dates, "recent_first_pink_dates": first_pink_dates, "rows": len(export)}
    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    state = "首个粉红柱出现" if int(latest.get("首个粉红柱") or 0) else ("买点触发" if int(latest.get("买进") or 0) else ("多项趋势条件成立" if int(latest.get("股牛股") or 0) else "无买点"))
    relative_text = "未计算（基准不可用或已关闭）" if pd.isna(latest.get("相对强弱%")) else f"{float(latest['相对强弱%']):.2f}%"
    recent = export.tail(15)[["date", "close", "股牛股", "首个粉红柱", "操纵", "趋势", "买进", "信号评分"]].copy()
...
def run_self_test() -> int:
    ...
    required = {"股牛股", "粉色柱", "首个粉红柱", "操纵", "趋势", "买进", "信号评分", "相对强弱%"}
