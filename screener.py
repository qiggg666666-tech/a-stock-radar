...
def _check_first_red(symbol: str) -> dict | None:
    try:
        time.sleep(random.uniform(1.0, 2.0)) # 增加随机等待，更像人类访问
        end = datetime.now()
...
def screen_first_red(limit=None):
    ...
    # 分批次扫描，每批后强制休息，极大地降低被封 IP 的风险
    batch_size = 50
    for i in range(0, len(stock_list), batch_size):
        batch = stock_list.iloc[i:i+batch_size]
        print(f"-> 正在处理第 {i} 到 {i+batch_size} 只股票...")
...
def screen_long_term_reversal(min_pct_change=7.0, limit=None):
    ...
    return pd.DataFrame(results).sort_values("距历史高点回撤%", ascending=False) if not results == [] else pd.DataFrame()
