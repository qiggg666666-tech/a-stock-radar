...
def consolidate(input_root: Path, output: Path, notify: bool) -> dict[str, Any]:
    ...
    summary = {"generated_at": datetime.now().astimezone().isoformat(timespec="seconds"), "state": state, "quality_diagnostics": quality, "candidates": len(candidates), "artifacts": {"csv": "first_red_520_original_daily.csv", "json": "first_red_520_original_daily.json"}, "prepare_status": prepare_status, "disclaimer": "自动化研究筛选结果，不构成投资建议。"}
    quality_label = "仅记录，不阻断" if universe_ready else "无可用股票池，无法扫描"
    lines = ["# 原版520首红：全市场当日汇总", "", f"- 状态：`{state}`", f"- 统一数据与信号日期：`{asof_date or '无'}`", f"- 覆盖：{processed}/{universe}（{coverage}%）", f"- 数据源错误：{errors}（{error_rate}%）", f"- 当日观察候选：{len(candidates)}", f"- 质量状态：`{quality_label}`", "", "> 质量指标仅用于说明数据完整度；不阻断已有同日候选的CSV生成或通知。宽松原版定义仍强制信号日等于标的最后可用日线，避免历史信号混入。"]
    if notify:
