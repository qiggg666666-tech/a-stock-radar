...
推送内容：
1. 高优先级综合榜（筹码+均线+PE 综合信号）
2. 现价下方长红柱 · 买入 / 洗盘
3. 宽幅堆积区 · 买入 / 洗盘

兼容内核字段：pe_ttm / pe_score / total_score / signal / ma_signal 等。
...
def main() -> int:
    ...
    if not completed:
        result = {
            "schema_version": "final-chip-summary/v3",
    ...
    lines = [
        ...
        f"- 每条消息最多{args.batch_size}只；兼容 pe_ttm / total_score",
        "",
        "## 高优先级综合榜（按综合分从高到低，推送）",
    ]
    ...
    result = {
        ...
        "disclosure": (
            "FINAL Chip 汇总 v3：高优先级综合榜 + 下方长红柱 + 宽幅区；"
            "推送高优先级/买入/洗盘；兼容 pe_ttm/total_score；通知失败不阻断 artifact。"
        ),
    }
    (args.output_dir / "final_chip_summary.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8"
