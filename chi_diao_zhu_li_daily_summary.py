...
SHARDS = ("a", "b", "c", "d")
FINAL_COLUMNS = ["代码", "名称", "信号日期", "信号类型", "流通市值(亿)", "当日成交额(亿)", "最新价", "信号评分", "股牛股", "首个粉红柱", "首粉确认", "多周期首粉确认", "预备首粉", "多周期预粉", "预备首粉等级", "预备首粉评分", "粉红准备度", "预备首粉缺失条件", "多周期通过数", "多周期评分", "多周期缺失条件", "周线修复", "月线支持", "季线支持", "年线支持", "高位过热排除", "量比5", "三日涨幅%", "买进", "操纵", "趋势", "中线趋势", "相对强弱%", "数据源"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-confirmed-first-pink", type=int, default=15)
    parser.add_argument("--max-multi-pre-pink", type=int, default=10)
    parser.add_argument("--max-pre-pink", type=int, default=10)
    parser.add_argument("--max-publishable", type=int, default=30)
...
def signal_rank(value: Any) -> int:
    """多周期确认优先，其次日线确认、买点、多周期预粉、单日预粉与观察。"""
    return {"多周期确认首粉": 0, "确认首粉": 1, "买点": 2, "多周期预粉M": 3, "预备首粉A": 4, "首粉观察": 5, "趋势观察": 6}.get(str(value or ""), 9)
...
def main() -> int:
    ...
    if not raw.empty and "信号日期" in raw.columns:
        ...
        if pd.notna(latest):
            ...
            if "首个粉红柱" not in final.columns:
                final["首个粉红柱"] = 0
            if "首粉确认" not in final.columns:
                final["首粉确认"] = 0
            if "多周期首粉确认" not in final.columns:
                final["多周期首粉确认"] = 0
            if "预备首粉" not in final.columns:
                final["预备首粉"] = 0
            if "多周期预粉" not in final.columns:
                final["多周期预粉"] = 0
    ...
    multi_confirmed_first_pinks = observations.loc[observations.get("多周期首粉确认", pd.Series(index=observations.index, dtype=int)) == 1].copy() if not observations.empty else observations.copy()
    confirmed_first_pinks = observations.loc[(observations.get("首粉确认", pd.Series(index=observations.index, dtype=int)) == 1) & (observations.get("多周期首粉确认", pd.Series(index=observations.index, dtype=int)) != 1)].copy() if not observations.empty else observations.copy()
    multi_pre_pinks = observations.loc[observations.get("多周期预粉", pd.Series(index=observations.index, dtype=int)) == 1].copy() if not observations.empty else observations.copy()
    pre_pink_bars = observations.loc[(observations.get("预备首粉", pd.Series(index=observations.index, dtype=int)) == 1) & (observations.get("多周期预粉", pd.Series(index=observations.index, dtype=int)) != 1)].copy() if not observations.empty else observations.copy()
    ...
    multi_confirmed_first_pinks = multi_confirmed_first_pinks.sort_values(["多周期评分", "信号评分"], ascending=[False, False]).head(max(0, args.max_confirmed_first_pink)) if not multi_confirmed_first_pinks.empty else multi_confirmed_first_pinks
    confirmed_first_pinks = confirmed_first_pinks.sort_values(["信号评分"], ascending=[False]).head(max(0, args.max_confirmed_first_pink))
    multi_pre_sort = [column for column in ["多周期评分", "预备首粉评分", "信号评分", "量比5"] if column in multi_pre_pinks.columns]
    multi_pre_pinks = multi_pre_pinks.sort_values(multi_pre_sort, ascending=[False] * len(multi_pre_sort)).head(max(0, args.max_multi_pre_pink)) if multi_pre_sort else multi_pre_pinks.head(max(0, args.max_multi_pre_pink))
    pre_pink_sort = [column for column in ["预备首粉评分", "信号评分", "量比5"] if column in pre_pink_bars.columns]
    pre_pink_bars = pre_pink_bars.sort_values(pre_pink_sort, ascending=[False] * len(pre_pink_sort)).head(max(0, args.max_pre_pink)) if pre_pink_sort else pre_pink_bars.head(max(0, args.max_pre_pink))
    priority_events = pd.concat([multi_confirmed_first_pinks, confirmed_first_pinks, buy_points, multi_pre_pinks, pre_pink_bars], ignore_index=True).drop_duplicates("代码", keep="first") if not observations.empty else pd.DataFrame(columns=FINAL_COLUMNS)
    priority_events["信号优先级"] = priority_events["信号类型"].map(signal_rank) if not priority_events.empty else pd.Series(dtype=int)
    priority_events = priority_events.sort_values(["信号优先级", "信号评分"], ascending=[True, False]).drop(columns=["信号优先级"], errors="ignore").head(max(0, args.max_publishable))
    ...
    report = {
        ...
        "multi_confirmed_first_pinks": int(len(multi_confirmed_first_pinks)),
        "confirmed_first_pinks": int(len(confirmed_first_pinks)),
        "multi_pre_pinks": int(len(multi_pre_pinks)),
    ...
    notification = send_serverchan(
        args.sendkey,
        f"吃掉主力·{args.run_label}：{state} | 多周确认{len(multi_confirmed_first_pinks)} | 多周预粉{len(multi_pre_pinks)} | 可发布{len(publishable)}条",
        "# 吃掉主力研究：每日全市场汇总\n\n"
        ...
        f"- 原始首粉观察：{len(first_pink_bars)}；多周期确认首粉：{report['multi_confirmed_first_pinks']}；日线确认首粉：{report['confirmed_first_pinks']}；多周期预粉M：{report['multi_pre_pinks']}；预备首粉A：{report['pre_pink_bars']}；买点：{report['buy_points']}\n"
        f"- 可发布研究候选（确认首粉每层≤{args.max_confirmed_first_pink}、多周期预粉M≤{args.max_multi_pre_pink}、预备A≤{args.max_pre_pink}、总计≤{args.max_publishable}）：{len(publishable)}\n"
