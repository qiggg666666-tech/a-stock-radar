...
def main() -> int:
    ...
    # 洗盘专区：仅保留 state 以"洗盘"开头的记录（洗盘·未确认 / 洗盘·蓄势中），
    # 与"买入"、"观察·已远离"区分开，各自按对应分数从高到低排序输出。
    below_washout = pd.DataFrame()
    below_buy = pd.DataFrame()
    if not below_spike.empty and "below_state" in below_spike.columns:
        below_washout = below_spike[below_spike["below_state"].astype(str).str.startswith("洗盘")].sort_values("below_score", ascending=False)
        below_buy = below_spike[below_spike["below_state"].astype(str) == "买入"].sort_values("below_score", ascending=False)
    below_washout.to_csv(args.output_dir / "final_chip_below_washout.csv", index=False, encoding="utf-8-sig")
    below_buy.to_csv(args.output_dir / "final_chip_below_buy.csv", index=False, encoding="utf-8-sig")

    wide_washout = pd.DataFrame()
    wide_buy = pd.DataFrame()
    if not wide_zone.empty and "wide_state" in wide_zone.columns:
        wide_washout = wide_zone[wide_zone["wide_state"].astype(str).str.startswith("洗盘")].sort_values("wide_score", ascending=False)
        wide_buy = wide_zone[wide_zone["wide_state"].astype(str) == "买入"].sort_values("wide_score", ascending=False)
    wide_washout.to_csv(args.output_dir / "final_chip_wide_washout.csv", index=False, encoding="utf-8-sig")
    wide_buy.to_csv(args.output_dir / "final_chip_wide_buy.csv", index=False, encoding="utf-8-sig")
    ...
    lines = [
        ...
        f"- 现价下方长红柱(is_below_spike)：{len(below_spike)}只（买入{len(below_buy)}只，洗盘{len(below_washout)}只，其余不推送）",
        f"- 宽幅堆积区(is_wide_zone)：{len(wide_zone)}只（买入{len(wide_buy)}只，洗盘{len(wide_washout)}只，其余不推送）",
        f"- 买入/洗盘分别推送，每条消息最多{args.batch_size}只，其它状态不推送", "",
        "## 现价下方长红柱·买入（按评分从高到低排序）",
    ]
    lines.extend(below_buy_lines if below_buy_lines else ["当日无买入状态记录。"])
    lines.extend(["", "## 现价下方长红柱·洗盘专区（按评分从高到低排序）"])
    lines.extend(below_washout_lines if below_washout_lines else ["当日无洗盘状态记录。"])
    lines.extend(["", "## 宽幅堆积区·买入（按评分从高到低排序）"])
    lines.extend(wide_buy_lines if wide_buy_lines else ["当日无买入状态记录。"])
    lines.extend(["", "## 宽幅堆积区·洗盘专区（按评分从高到低排序）"])
    lines.extend(wide_washout_lines if wide_washout_lines else ["当日无洗盘状态记录。"])
    lines.extend(["", "## 现价下方长红柱（全部，按评分排序，仅供参考，不推送）"])
    lines.extend(below_lines if below_lines else ["当日无命中记录。"])
    lines.extend(["", "## 宽幅堆积区（全部，按评分排序，仅供参考，不推送）"])
    ...
    # 只推送"买入"和"洗盘"两类状态，其余状态（如"观察·已远离"）不推送。
    # 每个信号维度(below_spike/wide_zone) x 每个状态(买入/洗盘) 各自独立分批推送。
    notifications: dict[str, list[dict[str, object]]] = {"below_buy": [], "below_washout": [], "wide_buy": [], "wide_washout": []}
    if args.notify:
        ...
        for key in notifications: notifications[key] = [{"status": "not_requested"}]

    result = {"schema_version": "final-chip-summary/v1", "generated_at": datetime.now(timezone.utc).isoformat(), "state": "ready" if len(completed) == args.shard_total else "partial", "completed_shards": sorted(int(item.get("shard_index", -1)) for item in completed), "record_count": len(records), "error_count": len(errors), "below_spike_count": len(below_spike), "wide_zone_count": len(wide_zone), "below_buy_count": len(below_buy), "below_washout_count": len(below_washout), "wide_buy_count": len(wide_buy), "wide_washout_count": len(wide_washout), "batch_size": args.batch_size, "notifications": notifications, "disclosure": "FINAL Chip专属汇总(现价下方长红柱+宽幅堆积区)；只推送买入/洗盘状态，通知失败不阻断artifact。"}
