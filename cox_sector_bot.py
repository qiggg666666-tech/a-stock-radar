...
【本版完善】
  6 交易日拦截仅对定时触发(schedule)生效, 手动(workflow_dispatch)/本地不受限(周末可用周五数据看超卖)。
  7 run() 收尾 _save/_notify/_print 各包 try/except, 任何收尾IO/推送/打印异常不让job误红。
  8 【修快照限流空跑】信号引擎全用历史日线、不依赖当日快照, 快照仅用于初筛砍量; 故东财快照挂时
    退化到股票列表兜底(baostock优先+akshare兜底, 截断MAX_CANDIDATES, 代码前缀过滤), 逐只拉历史日线
    照常算信号, 不再"快照一空就退出"。兜底缺当日活跃度过滤(换手/市值/涨停), 质量略降但信号判定不受影响。
    注: 此为保守兜底(不动 as_completed/ak_retry 结构, 主路径一字未改); 救"快照挂但K线接口正常"的常见
    限流场景; 东财K线也全限流的极端场景仍可能慢/撞timeout(原版该场景亦空跑, 不更糟)。
  信号引擎/技术指标/支撑阻力/共振打星/推送/md报告 一字未动。
...
class Universe:
    ...
    @classmethod
    def fallback_universe(cls):
        """【本版新增】东财快照失败时的兜底候选池: baostock股票列表优先 + akshare兜底, 截断 MAX_CANDIDATES。
        信号引擎只用历史日线(不用快照当日字段), 故兜底只需 代码+名称; 当日活跃度过滤(换手/市值/涨停)缺失=质量略降,
        但信号判定本身不受影响。代码前缀过滤 ^(60|00|30|68) 与主路径一致(排北交所)。"""
        logger.warning("  [兜底] 东财快照失败, 退化到股票列表(信号用历史日线, 不依赖快照当日字段)")
        codes = []   # [(code6, name)]
        # baostock 优先
        if BsClient.login():
            try:
                basic = bs.query_stock_basic().get_data()
                if basic is not None and not basic.empty and 'code' in basic.columns:
                    basic = basic[basic['code'].str.startswith(('sh.', 'sz.')) &
                                  (basic['type'] == '1') & (basic['status'] == '1')]
                    basic = basic[~basic['code_name'].astype(str).str.contains('ST|退', na=False, regex=True)]
                    codes = [(str(r['code']).split('.')[-1].zfill(6), str(r['code_name'])) for _, r in basic.iterrows()]
                    logger.info(f"  [兜底] baostock 列表 {len(codes)} 只")
            except Exception as e:
                logger.warning(f"  [兜底] baostock 列表异常: {e}")
            BsClient.logout()
        # akshare 兜底
        if not codes:
            d = ak_retry(ak.stock_info_a_code_name, desc="akshare列表")
            if d is not None and not d.empty and 'code' in d.columns:
                nc = 'name' if 'name' in d.columns else d.columns[1]
                d['code'] = d['code'].astype(str).str.zfill(6)
                d = d[~d[nc].astype(str).str.contains('ST|退', na=False, regex=True)]
                codes = [(str(r['code']), str(r[nc])) for _, r in d.iterrows()]
                logger.info(f"  [兜底] akshare 列表 {len(codes)} 只")
        if not codes:
            return []
        # 代码前缀过滤(排北交所, 与主路径一致)
        codes = [(c, n) for c, n in codes if c.startswith(('60', '00', '30', '68'))]
        # 截断(按列表序, 偏沪市; 防全A逐只拉K线撞timeout)
        if len(codes) > Config.MAX_CANDIDATES:
            logger.warning(f"  [兜底] {len(codes)} 只 > 截断{Config.MAX_CANDIDATES}(按列表序偏沪市, 降级保底)")
            codes = codes[:Config.MAX_CANDIDATES]
        logger.info(f"  [兜底] 候选池 {len(codes)} 只(无当日活跃度过滤, 质量略降但信号判定不受影响)")
        return [{"代码": c, "名称": n, "最新价": None, "涨跌幅": None, "换手率": None, "成交额": None}
                for c, n in codes]
...
class Picker:
    ...
    def run(self):
        ...
        # 【改1】交易日拦截仅对定时触发(schedule)生效; 手动(workflow_dispatch)/本地不受限, 周末可用周五数据看超卖
        if os.environ.get('GITHUB_EVENT_NAME') == 'schedule' and not is_trading_day():
            logger.info("非交易日且为定时触发(schedule), 跳过; 手动/本地运行不受此限"); return
        ...
        if not cands:
            # 【本版改2】东财快照失败 -> 股票列表兜底(信号用历史日线, 不依赖快照), 不再"快照一空就退出"
            cands = Universe.fallback_universe()
        if not cands:
            logger.error("快照与列表均失败, 无候选股票, 退出"); return
        ...
        # 【改3】收尾全部包防护: 任何收尾IO/推送/打印异常都不应让job误红
        try:
            self._save()
        except Exception as e:
            logger.error(f"⚠️ 存盘异常(结果已在内存, 不影响推送): {type(e).__name__}: {e}")
        try:
            self._notify()
        except Exception as e:
            logger.error(f"⚠️ 推送异常: {type(e).__name__}: {e}")
        try:
            self._print()
        except Exception as e:
            logger.error(f"⚠️ 打印异常: {type(e).__name__}: {e}")
        return self.results
