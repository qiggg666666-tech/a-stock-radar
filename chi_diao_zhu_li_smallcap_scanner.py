...
EXPORT_COLUMNS = [
    ...
    "信号评分", "股牛股", "首个粉红柱", "首粉确认", "多周期首粉确认", "预备首粉", "多周期预粉", "预备首粉等级", "预备首粉评分", "粉红准备度", "预备首粉缺失条件", "多周期通过数", "多周期评分", "多周期缺失条件", "周线修复", "月线支持", "季线支持", "年线支持", "高位过热排除", "量比5", "三日涨幅%", "买进", "操纵", "趋势", "中线趋势", "相对强弱%", "数据源",
]
...
def signal_rank(row: dict[str, Any]) -> int:
    ...
    signal_type = str(row.get("信号类型") or "")
    return {"多周期确认首粉": 0, "确认首粉": 1, "买点": 2, "多周期预粉M": 3, "预备首粉A": 4, "首粉观察": 5, "趋势观察": 6}.get(signal_type, 9)
...
def symbol_worker(result_queue: Any, item: dict[str, Any], benchmark: pd.DataFrame, config_dict: dict[str, Any]) -> None:
    ...
    try:
        ...
        if int(latest["多周期首粉确认"]) == 1:
            signal_type = "多周期确认首粉"
        elif int(latest["首粉确认"]) == 1:
            signal_type = "确认首粉"
        elif int(latest["买进"]) == 1:
            signal_type = "买点"
        elif int(latest["多周期预粉"]) == 1:
            signal_type = "多周期预粉M"
        elif int(latest["预备首粉"]) == 1:
            signal_type = "预备首粉A"
        elif int(latest["首个粉红柱"]) == 1:
            signal_type = "首粉观察"
        ...
        row = {
            ...
            "最新价": round(float(latest["close"]), 2), "信号评分": score, "股牛股": int(latest["股牛股"]), "首个粉红柱": int(latest["首个粉红柱"]), "首粉确认": int(latest["首粉确认"]), "多周期首粉确认": int(latest["多周期首粉确认"]), "预备首粉": int(latest["预备首粉"]), "多周期预粉": int(latest["多周期预粉"]), "预备首粉等级": str(latest["预备首粉等级"] or ""), "预备首粉评分": int(latest["预备首粉评分"]), "粉红准备度": int(latest["粉红准备度"]), "预备首粉缺失条件": str(latest["预备首粉缺失条件"] or ""), "多周期通过数": int(latest["多周期通过数"]), "多周期评分": int(latest["多周期评分"]), "多周期缺失条件": str(latest["多周期缺失条件"] or ""), "周线修复": int(latest["周线修复"]), "月线支持": int(latest["月线支持"]), "季线支持": int(latest["季线支持"]), "年线支持": int(latest["年线支持"]), "高位过热排除": int(latest["高位过热排除"]), "量比5": round(float(latest["量比5"]) if pd.notna(latest["量比5"]) else np.nan, 3), "三日涨幅%": round(float(latest["三日涨幅%"]) if pd.notna(latest["三日涨幅%"]) else np.nan, 3), "买进": int(latest["买进"]),
...
def save_outputs(config: Config, universe_size: int, processed: set[str], results: list[dict[str, Any]], stats: dict[str, int], reason: str, started: float, universe_as_of: str, universe_contract: dict[str, Any] | None = None, source_error_ledger: list[dict[str, Any]] | None = None) -> None:
    ...
    ordered = sorted(results, key=lambda row: (signal_rank(row), -int(row["信号评分"])))
...
def notify(config: Config, results: list[dict[str, Any]], reason: str) -> None:
    ...
    top = sorted(results, key=lambda row: (signal_rank(row), -int(row["信号评分"])))[:20]
...
def self_test() -> int:
    ...
    ranked = sorted([{"信号类型": "趋势观察", "信号评分": 90}, {"信号类型": "首粉观察", "信号评分": 80}, {"信号类型": "预备首粉A", "信号评分": 70}, {"信号类型": "多周期预粉M", "信号评分": 66}, {"信号类型": "买点", "信号评分": 60}, {"信号类型": "确认首粉", "信号评分": 50}, {"信号类型": "多周期确认首粉", "信号评分": 40}], key=lambda row: (signal_rank(row), -int(row["信号评分"])))
    assert [row["信号类型"] for row in ranked] == ["多周期确认首粉", "确认首粉", "买点", "多周期预粉M", "预备首粉A", "首粉观察", "趋势观察"]
