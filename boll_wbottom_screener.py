...
PARAMS = dict(
    ...
    # W底形态
    lookback=75, min_gap=8, max_gap=45, alpha=0.002,
    # 成交量
    volume_ma_period=10, volume_shrink_ratio=0.85, volume_expand_ratio=1.5,
    # 扫描
    recent_days=15, min_data_len=100, lookback_days=400,
...
def run_scan():
    ...
    print(f"开始布林带+W底扫描 {len(tasks)} 只（{PARAMS['NUM_PROCESSES']}进程, 双源, 近{PARAMS['recent_days']}天信号）...")
...
def build_push(df, cluster, hot):
    ...
    L = [f"**📈 布林带+W底突破** | 命中{len(df)}只 🎯风口{len(reso)} (全发, 近{PARAMS['recent_days']}天信号)",
...
def main():
    print("=" * 70)
    print(f"📈 布林带+W底突破 | {datetime.now():%Y-%m-%d %H:%M} | 回看{PARAMS['lookback_days']}天 近{PARAMS['recent_days']}天信号")
    print(f"全扫={'是' if not SCAN_LIMIT else f'限{SCAN_LIMIT}'}; 双源baostock+东财; 预筛={'开' if PARAMS['SNAPSHOT_PRE'] else '关'}; 不拦交易日; 推送全列+分页")
    print("=" * 70)
    df = run_scan()
    if df is None or df.empty:
        print("\n本次扫描未发现符合条件的近期W底信号。")
        print("可调: recent_days 调大 / alpha 放宽 / 看失败统计定位瓶颈")
        sys.exit(0)
