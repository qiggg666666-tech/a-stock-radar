...
def main() -> int:
    ...
    universe_as_of = datetime.now().strftime("%Y-%m-%d")
    universe_sha256 = sha256_file(args.output)
    payload = {
        "state": "completed", "generated_at": datetime.now(timezone.utc).isoformat(),
        "universe_as_of": universe_as_of,
        "universe_count": int(len(universe)),
        "universe_file": args.output.name,
        "universe_sha256": universe_sha256,
        "universe_run_id": f"{universe_as_of}-{universe_sha256[:16]}",
        "code_count": int(universe["代码"].nunique()),
        "first_code": str(universe.iloc[0]["代码"]),
        "last_code": str(universe.iloc[-1]["代码"]),
        "filters": {
