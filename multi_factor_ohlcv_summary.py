...
def format_notification_candidate_lines(rows: list[dict[str, Any]]) -> list[str]:
    """Keep every candidate in one mobile-readable ServerChan message."""
    lines = ["## 全部候选（代码｜名称｜研究评分）"]
    for index, row in enumerate(rows, start=1):
        code = str(row.get("code", "")).zfill(6)
        name = str(row.get("name", ""))
        score = int(float(row.get("research_score", 0) or 0))
...
def main() -> int:
    ...
    notification_lines = lines[:9] + ["", "> 完整指标见运行artifact；以下清单完整覆盖全部候选。", ""] + format_notification_candidate_lines(candidate_rows)
    notification_content = "\n".join(notification_lines)
    snapshot["notification_parts"] = 1
    snapshot["notification_candidate_count"] = len(candidate_rows)
