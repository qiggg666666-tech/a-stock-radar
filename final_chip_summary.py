#!/usr/bin/env python3
"""FINAL Chip 洗盘 + 首红确认 + 红筹码占优 + 长影线 + 宽幅堆积区 + 均线强多 汇总器。
...
5. 宽幅堆积区：is_wide_zone（现价贴近或刚脱离下方一块宽幅筹码堆积区上沿），按wide_score从高到低排序。
6. 均线强多：ma_signal以"强多"开头、但没有命中以上任一专区的股票(比如靠均线金叉+放量涨停，
   跟筹码形态/长影线无关)，按ma_score从高到低排序，让这类此前只体现在signal字段里、
   其它专区都捞不到的股票也有地方推送。
六类互不排斥（除了均线强多本身定义为"不与前五类重叠"），同一只股票可能同时命中多类。
每类各自全部命中分批推送。
...
def format_wide_zone_lines(df: pd.DataFrame) -> list[str]:
    lines = []
    for number, (_, row) in enumerate(df.iterrows(), 1):
        lines.append(
            f"{number:03d}. {row.get('code', '')} {row.get('name', '')}"
...
def main() -> int:
    ...
    if not completed:
        result = {
            "schema_version": "final-chip-summary/v9-wide-zone",
    ...
    # 改成只认daily_long_lower：has_long_shadow两个方向都算，会把长上影(冲高回落/
    # 卖出压力型态)也混进来，导致这个专区实际推送出来的大多是卖出信号，跟"长影线"
    # 本意想找的买入机会对不上。
    ...
    if not records.empty:
        wide_zone_mask = _bool_col(records, "is_wide_zone")
        candidate = records[wide_zone_mask].copy()
        if not candidate.empty:
            rank = _numeric_col(candidate, "wide_score")
            wide_zone = candidate.assign(_rank=rank).sort_values(
                "_rank", ascending=False
            ).drop(columns="_rank")
    wide_zone.to_csv(args.output_dir / "final_chip_wide_zone.csv", index=False, encoding="utf-8-sig")

    # ---------- 均线强多，但没有落进上面任何一个筹码/首红专区 ----------
    # 直接判断五个已有信号本身，而不是读signal字段——signal内部判断"高潜力"还是
    # "均线强多"用的是is_wide_zone/is_chip_spike/is_below_spike/in_zone，跟这五个
    # 真正的推送专区口径不完全一样，直接对着五个专区的判定条件取反才准确。
    # is_wide_zone这次也加进排除条件——之前没加时，三孚股份/光电股份这类"均线强多
    # +宽幅堆积区"的股票会被算进均线强多专区，跟新开的宽幅堆积区专区重复。
    ...
    wide_zone_lines = format_wide_zone_lines(wide_zone)
    ma_strong_lines = format_ma_strong_lines(ma_strong)

    lines = [
        "# FINAL Chip 汇总（洗盘 + 首红确认 + 红筹码占优 + 长影线 + 宽幅堆积区 + 均线强多）",
        "",
        ...
        f"- 宽幅堆积区(is_wide_zone)：{len(wide_zone)}只，按wide_score从高到低全部推送",
    ...
    lines.extend(["", "## 宽幅堆积区专区（现价贴近或刚脱离下方宽幅筹码堆积区上沿，按wide_score从高到低排序）"])
    lines.extend(wide_zone_lines if wide_zone_lines else ["当日无宽幅堆积区记录。"])
    ...
    notifications: dict[str, list[dict[str, object]]] = {
        "washout": [], "first_red_confirmed": [], "red_heavy": [], "long_shadow": [], "wide_zone": [], "ma_strong": [],
    }
    if args.notify:
        ...
        notifications["wide_zone"] = push_batches("宽幅堆积区专区", wide_zone_lines)
    ...
    result = {
        ...
        "disclosure": (
            ...
            "长影线(daily_long_lower，长下影买入型态)、宽幅堆积区(is_wide_zone)、均线强多(ma_signal以'强多'开头且"
            "未落入以上任一专区) 六类股票，互不排斥（均线强多本身定义为不重叠）；通知失败不阻断 artifact。"
        ),
    }
    (args.output_dir / "final_chip_summary.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8"
