...
⚠️ 相比原始版本的修复：
1. 数据源改用baostock，用正确的指数代码格式（如 sh.000001），
   避免了"用个股接口查指数代码"这种接口用错的问题。
2. 不使用 trendln 库——它维护不算活跃，容易和numpy/scipy版本冲突，
   而且原代码里 import失败时会静默返回None，容易在无人值守的自动化环境里
   "看似正常运行、实际部分功能已经失效"却不易察觉。
3. 用 scipy.signal.argrelextrema 找局部高低点（需要在requirements.txt里加scipy），
   同时检测顶背离和底背离，扩展到日/周/月三个周期。
4. 去掉了matplotlib画图，自动化环境里没有意义，改成纯文字推送。

⚠️ 依赖提醒：这个脚本用到了 scipy，如果 requirements.txt 里还没有，需要加一行 scipy。
"""
...
RECENT_DIVERGENCE_DAYS = {'daily': 30, 'weekly': 90, 'monthly': 365}  # 各周期"近期"预警的时间窗口
PEAK_ORDER = {'daily': 5, 'weekly': 3, 'monthly': 2}  # 各周期找局部高低点的左右窗口
...
def resample_to_period(daily_df, period):
    ...
    rule = 'W-FRI' if period == 'weekly' else 'ME'  # pandas 2.2+ 用ME替代已废弃的M
