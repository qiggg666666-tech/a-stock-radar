...
def normalize_baostock_listed_equities(frame: pd.DataFrame) -> pd.DataFrame:
    required = {"code", "code_name"}
    if frame.empty or not required.issubset(frame.columns):
        return pd.DataFrame(columns=["code", "name"])
    cleaned = frame.copy()
    if "type" in cleaned.columns:
        cleaned = cleaned[cleaned["type"].astype(str) == "1"]
    if "status" in cleaned.columns:
        cleaned = cleaned[cleaned["status"].astype(str) == "1"]
    cleaned = cleaned[cleaned["code"].astype(str).str.startswith(("sh.", "sz."), na=False)]
    return pd.DataFrame({
        "code": cleaned["code"].astype(str).str.split(".").str[-1],
        "name": cleaned["code_name"].astype(str),
    })
...
def _baostock_universe() -> pd.DataFrame:
    ...
    try:
        ...
        if frame.empty:
            return pd.DataFrame(columns=["code", "name"])
        return normalize_baostock_listed_equities(frame)
    finally:
        bs.logout()
...
def main() -> int:
    ...
    if args.self_test:
        ...
        baostock_sample = pd.DataFrame({
            "code": ["sh.000001", "sz.000002", "sz.300001", "sh.600001"],
            "code_name": ["上证综合指数", "样本甲", "样本乙", "已退市样本"],
            "type": ["2", "1", "1", "1"],
            "status": ["1", "1", "1", "0"],
        })
        assert normalize_baostock_listed_equities(baostock_sample)["code"].tolist() == ["000002", "300001"]
