...
【本版·治1579刷屏】真码诊断: STRICT_NON_PIT已收紧🟡🟠, 1579刷屏真凶=⚠️坑底探底(坑底无条件进+
  阴跌无止跌票海量)。故加 PUSH_STAGES 推送阶段白名单(默认'0'=只推🔴坑底止跌, 砍⚠️刷屏; csv仍全量不丢),
  标题改"推X只(全量Y只存csv)"; 另加 REQUIRE_PIT_MACD(默认0; 设1则🔴也要MACD转强, 治"MACD未转"飞刀)。
【本版B·🔴叠加周线老鸭头硬过滤】在🔴坑底止跌上再叠"周线老鸭头"(60周线向上+5/10上穿60+回调合适+
  短期多头+周线MACD转强, score≥DUCK_SCORE_MIN)。筛"周线蓄势+日线坑底"=强势股回踩精华; 跌崩/阴跌票
  因周线趋势坏被砍。REQUIRE_DUCK=0 一键关闭(回退看全🔴)。仅🔴算周线(省计算, 非坑底不算)。
  ⚠️ 周线老鸭头与急跌坑底常矛盾 -> B下🔴可能大降甚至0(无强势回踩时), 非bug; 太少设REQUIRE_DUCK=0/DUCK_SCORE_MIN=3。
  ⚠️ 周线老鸭头需≥450日线 -> MIN_DATA_LEN默认450, LOOKBACK_DAYS默认1100(原120/700不够周线); 拉数据偏重,
  全扫建议SCAN_LIMIT≤1500防超时。
...
# 【本版新增·治1579刷屏】
REQUIRE_PIT_MACD = os.environ.get('REQUIRE_PIT_MACD', '0').strip() in ('1', 'true', 'True')  # 坑底🔴也要MACD转强(治"MACD未转"飞刀; 默认0保29.64/急跌底)
PUSH_STAGES = set(int(x) for x in os.environ.get('PUSH_STAGES', '0').split(',') if x.strip().isdigit()) or {0}  # 推送阶段白名单(0🔴/1⚠️/2🟡/3); 默认只推坑底止跌(砍⚠️刷屏); csv仍全量
# 【本版B·周线老鸭头硬过滤🔴】
REQUIRE_DUCK = os.environ.get('REQUIRE_DUCK', '1').strip() in ('1', 'true', 'True')  # 🔴坑底止跌叠加周线老鸭头硬过滤(只推周线蓄势+日线坑底; 设0关闭)
DUCK_SCORE_MIN = int(os.environ.get('DUCK_SCORE_MIN', '4'))   # 周线老鸭头score阈值(≥此算成立; 太严设3)
TURNOVER_MIN = float(os.environ.get('TURNOVER_MIN', '0.3'))
# 【本版B改】周线老鸭头需≥450日线(原120不够周线60+预热); LOOKBACK 700→1100 给足周线根数+余量
...
_BS_LOGGED = False
_INDUSTRY_MAP = {}
FAIL_STATS = {"抓取失败": 0, "数据不足": 0, "无信号": 0, "周线老鸭头未成": 0}
...
# ==================== 【本版B】周线老鸭头 (numpy/pandas, 无scipy; 与duck系列同实现) ====================
...
def _detect_duck(df_daily):
    """周线老鸭头: 60周线向上+5/10上穿60+回调8~35%+短期多头+周线MACD转强。返回(valid, score)。
    需 df_daily 含 date(datetime)/open/high/low/close/volume 且 ≥约90周线(≈450日线)。"""
    try:
        ...
        if peak > 0 and 0.08 <= (peak - trough) / peak <= 0.35:    # 回调幅度合适
            score += 1
        if w['ma5'].iloc[-1] > w['ma10'].iloc[-1] and w['ma10'].iloc[-1] > w['ma60'].iloc[-1] * 0.98:  # 短期多头
            score += 1
        dif, dea, hist = _macd_series(w['close'])                  # 周线MACD转强
...
# ==================== 策略内核 (低位+止跌+坑底兜底 + 非坑底档MACD/布林硬门槛 + 坑底🔴可选MACD门槛 + 【本版B】🔴周线老鸭头硬过滤) ====================
def check_one_stock(df):
    ...
    duck_score = None   # 【本版B】仅🔴坑底止跌才计算周线老鸭头(省计算)

    # 【本版改】分档硬门槛: 坑底无条件进; 非坑底档按 STRICT_NON_PIT 收紧(粘合与急跌矛盾, 故坑底不卡粘合)
    if is_pit:
        if stop:
            if REQUIRE_PIT_MACD and not macd_strong:
                return None, "无信号"
            # 【本版B】周线老鸭头硬过滤: 仅🔴坑底止跌叠加(周线蓄势+日线坑底=强势回踩精华; 跌崩票周线坏被砍)
            if REQUIRE_DUCK:
    ...
    score = round((1 - min(above_low, BOTTOM_ZONE_PCT) / BOTTOM_ZONE_PCT) * 40
                  ...
                  + (10 if macd_strong else 0) + (10 if bb_narrow else 0) + (10 if ma_spread < MA_SPREAD_MAX else 0)
                  + (duck_score * 2 if duck_score else 0), 1)   # 【本版B】老鸭头强度加权(🔴排前)
    return {"代码": None, "名称": None, "行业": "",
            ...
            "鸭头分": duck_score,                                                            # 【本版B】周线老鸭头分(仅🔴有值)
...
def run_scan():
    ...
    print(f"开始底部坑底/蓄势扫描 {len(tasks)} 只（{NUM_PROCESSES}进程, 双源; 非坑底收紧={'开' if STRICT_NON_PIT else '关'}; 坑底≤{PIT_PCT*100:.0f}%无条件进; 🔴老鸭头={'开' if REQUIRE_DUCK else '关'}）...")
        ...
        for res in pool.imap_unordered(_process_one, tasks):
            if res:
                if "__fail__" in res:
                    ...
                    results.append(res)
                    dk = f" 鸭头{res['鸭头分']}" if res.get('鸭头分') is not None else ""
                    pbar.write(f"  √ {res['代码']} {res['名称']} {res['阶段']} MACD{res['MACD状态']} 布林{res['布林']}{dk} 价={res['最新价']}")
...
def build_push(df, cluster, hot, spot_now=None):
    total_n = len(df)
    # 【本版·治刷屏】推送阶段白名单过滤(局部变量, 不影响main已存csv的全量df): 默认只推🔴坑底止跌, 砍⚠️刷屏
    if '阶段' in df.columns and PUSH_STAGES:
        df = df[df['阶段'].map(_STAGE_ORDER).fillna(99).isin(PUSH_STAGES)].copy()
    reso = df[df['resonance'] == True] if 'resonance' in df.columns else pd.DataFrame()
    duck_note = "🔴叠周线老鸭头" if REQUIRE_DUCK else "🔴未叠老鸭头"
    L = [f"**🪨 底部坑底/蓄势** | 推{len(df)}只 🎯风口{len(reso)} (全量{total_n}只存csv, 仅推阶段{sorted(PUSH_STAGES)}, {duck_note})",
         f"*(默认只推🔴坑底止跌=跌到底+初现企稳{'＋周线蓄势' if REQUIRE_DUCK else ''}; ⚠️/🟡/不推但存csv; 全推设PUSH_STAGES='0,1,2,3'; 关老鸭头设REQUIRE_DUCK=0; 非买入保证, 必止损)*", ""]
    if hot:
        L.append("🌪️ **风口**: " + "、".join(f"{n}({c}%)" for n, c in hot[:6])); L.append("")
    if cluster:
        L.append("🪨 **底部板块**: " + "、".join(f"{n}({c})" for n, c in cluster)); L.append("")
    if '阶段' in df.columns and df.empty:
        L.append(f"*(今日推送阶段白名单{sorted(PUSH_STAGES)}无命中; 全量{total_n}只多为⚠️接飞刀/🟡横盘, 已存csv; 放宽设 PUSH_STAGES='0,3' 或 '0,1,2,3'; 若🔴被老鸭头砍光设 REQUIRE_DUCK=0)*")
        return "\n".join(L)
    def line(r):
        duck = f" 鸭头{r['鸭头分']}" if r.get('鸭头分') is not None else ""
        return (f"- {r['阶段']} **{r['名称']}({r['代码']})** [{_sec_tag(r.to_dict())}] 距低点{r['距低点%']}% 回撤{r['距高点回撤%']}% "
                f"RSI{r['RSI']} MACD{r['MACD状态']} 布林{r['布林']} 换手{r['换手%']}%{duck}{_align_suffix(r, spot_now)}")
    for stage, head in [("🔴坑底止跌", "### 🔴 坑底止跌 (贴近低点+初现企稳+周线蓄势, 左侧回踩)"),
...
def main():
    ...
    print(f"全扫={'是' if not SCAN_LIMIT else f'限{SCAN_LIMIT}'}; 双源; 多进程{NUM_PROCESSES}; 预筛={'开' if SNAPSHOT_PRE else '关'}; "
          f"非坑底收紧={'开' if STRICT_NON_PIT else '关'}; 坑底≤{PIT_PCT*100:.0f}%无条件进; 不拦交易日")
    print(f"【推送】阶段白名单={sorted(PUSH_STAGES)}(csv全量不丢); 坑底🔴要MACD转强={'开' if REQUIRE_PIT_MACD else '关'}")
    print(f"【周线老鸭头B】🔴硬过滤={'开' if REQUIRE_DUCK else '关'}(score≥{DUCK_SCORE_MIN}; 关设REQUIRE_DUCK=0; 太严设DUCK_SCORE_MIN=3); MIN_DATA={MIN_DATA_LEN}/LOOKBACK={LOOKBACK_DAYS}(周线需≥450日线, 全扫建议SCAN_LIMIT≤1500)")
    print("=" * 70)
    df = run_scan()
    if df is None or df.empty:
        print("\n本次扫描未发现满足 底部坑底/蓄势 的信号。")
        print("可调: REQUIRE_DUCK=0(关老鸭头看全🔴) / DUCK_SCORE_MIN=3(放宽老鸭头) / STRICT_NON_PIT=0 / BOTTOM_ZONE_PCT调大 / PIT_PCT调大")
        sys.exit(0)
    ...
    try:
        ...
        with open(os.path.join(OUTPUT_DIR, f"bottom_acc_{tag}.json"), 'w', encoding='utf-8') as f:
            json.dump({"date": tag, "params": {"BOTTOM_ZONE_PCT": BOTTOM_ZONE_PCT, "PIT_PCT": PIT_PCT,
                                               ...
                                               "BB_NARROW_MAX": BB_NARROW_MAX, "PUSH_STAGES": sorted(PUSH_STAGES),
                                               "REQUIRE_PIT_MACD": REQUIRE_PIT_MACD, "REQUIRE_DUCK": REQUIRE_DUCK,
                                               "DUCK_SCORE_MIN": DUCK_SCORE_MIN, "LOOKBACK_DAYS": LOOKBACK_DAYS,
